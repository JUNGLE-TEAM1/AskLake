...
def create_test_app() -> FastAPI:
    ...
    @app.post("/validate")
    def validate(payload: ValidationPayload) -> dict[str, int]:
...
class ObservabilityContractTests(unittest.TestCase):
    ...
    def test_valid_correlation_id_round_trips_and_is_counted(self) -> None:
        response = TestClient(create_test_app()).get("/ok", headers={CORRELATION_ID_HEADER: "demo-request-42"})
    ...
    def test_invalid_correlation_id_is_replaced(self) -> None:
        response = TestClient(create_test_app()).get("/ok", headers={CORRELATION_ID_HEADER: "invalid value with spaces"})
    ...
    def test_error_envelope_is_redacted_and_diagnostic_is_copyable(self) -> None:
        response = TestClient(create_test_app()).get("/known-error", headers={CORRELATION_ID_HEADER: "diag-123"})
    ...
    def test_unhandled_error_does_not_expose_exception_text(self) -> None:
        response = TestClient(create_test_app(), raise_server_exceptions=False).get("/unhandled")
        self.assertEqual(response.status_code, 500)
        self.assertNotIn("do-not-return", response.text)
        self.assertTrue(response.json()["error"]["diagnosticId"])

    def test_validation_error_returns_422_without_echoing_input(self) -> None:
        response = TestClient(create_test_app(), raise_server_exceptions=False).post(
            "/validate",
            json={"amount": "do-not-return"},
        )
