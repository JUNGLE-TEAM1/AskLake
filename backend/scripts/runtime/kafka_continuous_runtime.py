...
import continuous_sql_runtime as continuous_sql
from continuous_sql_runtime import continuous_output_partition_columns
...
from runtime.config import KafkaWorkerConfig
...
CONTINUOUS_SQL_PLAN = continuous_sql.load_continuous_sql_plan()
...
def on_signal(_signum: int, _frame: Any) -> None:
    ...
    STOP_REQUESTED = True
    SHUTDOWN_COORDINATOR.request_stop(QUERY)
...
def continuous_runtime_contract(output_path: str, iceberg_target: dict[str, Any]) -> dict[str, Any]:
    ...
    contract = {
        "contractSchemaVersion": CHECKPOINT_CONTRACT_SCHEMA_VERSION,
        ...
        **continuous_sql.contract_metadata(CONTINUOUS_SQL_PLAN),
    }
...
def normalize_committed_manifest(
    ...
    manifest.setdefault("manifestSchemaVersion", 0)
...
def write_batch_manifest(spark: SparkSession, root: str, batch_id: int, manifest: dict[str, Any]) -> None:
    ...
    delete_incomplete_output(spark, path)
    versioned_manifest = {
        "manifestSchemaVersion": BATCH_MANIFEST_SCHEMA_VERSION,
        **manifest,
    }
    frame = spark.read.json(spark.sparkContext.parallelize([json.dumps(versioned_manifest)]))
    frame.write.mode("errorifexists").json(path)
    if not output_committed(spark, path):
...
def main() -> None:
    ...
    config = KafkaWorkerConfig.from_environment()
    output_path = config.output_path
    checkpoint_path = config.checkpoint_path
    quarantine_path = f"{output_path.rstrip('/')}/_quarantine"
    trigger_seconds = config.trigger_seconds
    ...
    if config.broker.rsplit(":", 1)[-1] == "9098":
    ...
    def write_batch(batch: DataFrame, spark_batch_id: int) -> None:
        ...
        static_snapshots, source_boundary = continuous_sql.prepare_batch_identity(
            spark, CONTINUOUS_SQL_PLAN, output_path, batch_id,
            normalized_source_ranges(source_ranges), checkpoint_path, config, JOB_ID,
        )
        ...
        CURRENT_BATCH_CONTEXT = {
            ...
            **continuous_sql.batch_context_metadata(source_boundary, static_snapshots),
            "stored_count": 0,
        }
        for item in source_ranges:
        ...
        projected = continuous_sql.execute_continuous_sql_batch(spark, projected, CONTINUOUS_SQL_PLAN, static_snapshots)
        stage_durations["schemaDurationMs"] = max(0, round((time.monotonic() - schema_started_at) * 1000))
        CURRENT_BATCH_CONTEXT.update({
        ...
        stored_count = continuous_sql.validated_output_count(target_frame, CONTINUOUS_SQL_PLAN, total)
        ...
        if stored_count:
            ...
            iceberg_commit = commit_iceberg_table(
                ...
                job_id=JOB_ID,
                run_id=run_id,
                partition_columns=continuous_output_partition_columns(iceberg_target["partitionColumns"], CONTINUOUS_SQL_PLAN),
        ...
        published_manifest = {
            ...
            **continuous_sql.publication_metadata(CONTINUOUS_SQL_PLAN, static_snapshots),
            "consumedCount": total,
