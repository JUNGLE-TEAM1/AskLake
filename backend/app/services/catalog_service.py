...
class CatalogService:
    ...
    def verify_and_register_unique_key(
        ...
        table = verified_iceberg_table(dataset)
        result = execute_trino_rows(
            TrinoClient(),
            unique_key_verification_query(table, columns),
            max_pages=2_000,
            timeout_seconds=600,
        )
        ...
        unique_sets = [list(item) for item in current_sets if isinstance(item, list)] if isinstance(current_sets, list) else []
        ...
        if len(columns) == 1:
            ...
            unique_columns = [str(item) for item in current_columns] if isinstance(current_columns, list) else []
...
def normalized_unique_key_columns(requested_columns: list[str], dataset: CatalogDatasetResponse) -> list[str]:
    schema_columns = {
...
def unique_key_verification_query(table: str, columns: list[str]) -> str:
    ...
    invalid = " OR ".join(
        f"{column} IS NULL OR trim(CAST({column} AS VARCHAR)) = ''" for column in quoted
    )
