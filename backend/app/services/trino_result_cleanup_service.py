    ...
    def cleanup_expired(self, *, now: datetime | None = None) -> TrinoResultCleanupSummary:
        ...
        for payload in self.repository.list_trino_run_payloads():
            ...
            if (
                ...
                or response.status not in {"succeeded", "failed", "cancelled"}
            ):
                continue
            ...
            updated_payload = expired.model_dump(by_alias=True, exclude_none=True, mode="json")
            updated_payload["compiledQuery"] = payload.get("compiledQuery")
            updated_payload["trinoNextUri"] = payload.get("trinoNextUri")
