...
from app.schemas.etl import (
    ...
    SqlJobDatasetInput,
    SqlJobExecutionContract,
    SqlJobStorageSegment,
    TargetDatabaseOption,
    TargetDatabasesResponse,
    UpdatePipelineRequest,
)
from app.schemas.sql import QueryRunResponse as SqlQueryRunResponse
...
def create_pipeline(db: Session, request: CreatePipelineRequest, actor_name: str = "demo-user") -> CreatePipelineResponse:
    ...
    sql_job_execution_contract(db, request.source_type, request.source_config)
...
def update_pipeline(
    ...
    validate_update_request(request, source_type=job.source_type, execution_mode=job.execution_mode)
...
def delete_job(db: Session, job_id: str, actor: ActorContext | None = None) -> str:
    ...
    if job is None:
        raise ApiError(ErrorCode.NOT_FOUND, f"Job not found: {job_id}", status.HTTP_404_NOT_FOUND)
    active_runs = [run for run in etl_repository.list_run_models_for_job(db, job.id) if run.status in ACTIVE_RUN_STATUSES]
    if active_runs:
        raise ApiError(
            ErrorCode.CONFLICT,
    ...
    if active_maintenance:
        raise ApiError(
            ...
            status.HTTP_409_CONFLICT,
            {"maintenanceRunId": active_maintenance[0].run_id, "maintenanceStatus": active_maintenance[0].status},
        )
...
def sql_job_execution_contract(
    db: Session,
    source_type: str | None,
    source_config: list[list[str]] | list[tuple[str, str]] | None,
) -> SqlJobExecutionContract | None:
    if str(source_type or "").strip().casefold() != "sql result":
        return None

    fields = source_config or []
    contract_name = field_value(fields, "SQL Execution Contract")
    if source_config_has_field(fields, "SQL Execution Contract") and contract_name != "catalog_sql_v1":
        raise ApiError(
            ErrorCode.VALIDATION_ERROR,
            f"Unsupported SQL Job execution contract: {contract_name}",
            status.HTTP_422_UNPROCESSABLE_ENTITY,
        )

    source_run_id = field_value(fields, "SQL Run ID")
    if not source_run_id:
        raise ApiError(
            ErrorCode.VALIDATION_ERROR,
            "SQL Result Jobs require a persisted SQL Run ID.",
            status.HTTP_422_UNPROCESSABLE_ENTITY,
        )

    payload = SqlRepository(db).get_run_payload(source_run_id)
    if payload is None:
        raise ApiError(
            ErrorCode.NOT_FOUND,
            f"SQL Run not found: {source_run_id}",
            status.HTTP_404_NOT_FOUND,
        )
    try:
        sql_run = SqlQueryRunResponse.model_validate(payload)
    except ValidationError as exc:
        raise ApiError(
            ErrorCode.CONFLICT,
            f"SQL Run is not executable: {source_run_id}",
            status.HTTP_409_CONFLICT,
            {"validationErrors": exc.errors(include_url=False)},
        ) from exc

    statement = validate_read_only_query(sql_run.query)
    configured_query = field_value(fields, "Query")
    if source_config_has_field(fields, "Query") and (
        not configured_query or validate_read_only_query(configured_query) != statement
    ):
        raise ApiError(
            ErrorCode.CONFLICT,
            "The SQL Job query no longer matches its persisted SQL Run.",
            status.HTTP_409_CONFLICT,
            {"sourceRunId": source_run_id},
        )

    base_dataset_id = str(sql_run.base_dataset_id or sql_run.dataset_id or "").strip()
    if not base_dataset_id:
        raise ApiError(
            ErrorCode.CONFLICT,
            f"SQL Run does not identify a base Catalog dataset: {source_run_id}",
            status.HTTP_409_CONFLICT,
        )
    configured_base_dataset_id = field_value(fields, "Source Dataset ID")
    if source_config_has_field(fields, "Source Dataset ID") and configured_base_dataset_id != base_dataset_id:
        raise ApiError(
            ErrorCode.CONFLICT,
            "The SQL Job base dataset no longer matches its persisted SQL Run.",
            status.HTTP_409_CONFLICT,
            {"configuredDatasetId": configured_base_dataset_id, "persistedDatasetId": base_dataset_id},
        )

    reference_dataset_ids = [
        dataset_id
        for dataset_id in unique_dataset_ids(sql_run.reference_dataset_ids)
        if dataset_id != base_dataset_id
    ]
    configured_reference_ids = parse_sql_job_reference_dataset_ids(field_value(fields, "Reference Dataset IDs"))
    if source_config_has_field(fields, "Reference Dataset IDs") and set(configured_reference_ids) != set(reference_dataset_ids):
        raise ApiError(
            ErrorCode.CONFLICT,
            "The SQL Job reference datasets no longer match its persisted SQL Run.",
            status.HTTP_409_CONFLICT,
            {
                "configuredDatasetIds": configured_reference_ids,
                "persistedDatasetIds": reference_dataset_ids,
            },
        )

    datasets = []
    dataset_inputs = []
    for dataset_id in [base_dataset_id, *reference_dataset_ids]:
        dataset = etl_repository.get_dataset_schema_by_id(db, dataset_id)
        if dataset is None:
            raise ApiError(
                ErrorCode.NOT_FOUND,
                f"Catalog dataset required by SQL Run was not found: {dataset_id}",
                status.HTTP_404_NOT_FOUND,
                {"sourceRunId": source_run_id, "datasetId": dataset_id},
            )
        datasets.append(dataset)
        segments = dataset_storage_segments(dataset)
        if not segments:
            raise ApiError(
                ErrorCode.SERVICE_UNAVAILABLE,
                f"Catalog dataset has no successful physical materialization: {dataset_id}",
                status.HTTP_503_SERVICE_UNAVAILABLE,
                {"sourceRunId": source_run_id, "datasetId": dataset_id},
            )
        unsupported_formats = sorted({storage_format for _, storage_format in segments if storage_format not in {"csv", "json", "jsonl", "parquet"}})
        if unsupported_formats:
            raise ApiError(
                ErrorCode.SERVICE_UNAVAILABLE,
                f"Catalog dataset has unsupported physical storage formats: {dataset_id}",
                status.HTTP_503_SERVICE_UNAVAILABLE,
                {"datasetId": dataset_id, "storageFormats": unsupported_formats},
            )
        dataset_inputs.append(
            SqlJobDatasetInput(
                ...
                name=dataset.name,
                storage_segments=[
                    SqlJobStorageSegment(location=location, format=storage_format)
                    for location, storage_format in segments
                ],
            )
        )

    resolve_referenced_datasets(mask_sql_comments_and_literals(statement), datasets)
    return SqlJobExecutionContract(
...
def parse_sql_job_reference_dataset_ids(value: str | None) -> list[str]:
    normalized = str(value or "").strip()
    if normalized in {"", "-"}:
        return []
    return unique_dataset_ids([item.strip() for item in normalized.split(",") if item.strip()])


def source_config_has_field(fields: list[list[str]] | list[tuple[str, str]], label: str) -> bool:
    return any(field_label == label for field_label, _ in fields)
...
def run_spark_job(db: Session, job: ETLJobModel, command: str, run_id: str) -> dict[str, Any]:
    ...
    sql_execution = sql_job_execution_contract(db, job.source_type, job.source_config)
...
def job_payload_for_spark(
    ...
    sql_execution: SqlJobExecutionContract | None = None,
    *,
    ...
    return {
        ...
        "sqlExecution": sql_execution.model_dump(mode="json", by_alias=True) if sql_execution else None,
        "stats": job.stats or {},
        "target": job.target,
...
def source_incremental_since(db: Session, job: ETLJobModel, current_run_id: str) -> str | None:
    ...
    matching_run = next((
        run
        for run in materialization_runs if isinstance(run, dict)
        and str(run.get("runId") or "") == str(latest_successful_run.run_id)
...
def source_incremental_window(
    ...
    upper_bound = str(current_run.started_at) if current_run and str(current_run.started_at or "").strip() not in {"", "-"} else None
...
def source_uses_incremental_folder_window(job: ETLJobModel) -> bool:
    ...
    if not (source_type.startswith("file / s3") or source_type.startswith("data lake")):
        return False
    fields: dict[str, str] = {}
    for row in getattr(job, "source_config", None) or []:
        if not isinstance(row, (list, tuple)) or len(row) < 2:
            continue
        label, value = row[0], row[1]
        fields[str(label or "").strip().casefold()] = str(value or "").strip().casefold()
    return fields.get("collection scope") == "folder" and fields.get("collection mode", "incremental") == "incremental"
...
def validate_create_request(request: CreatePipelineRequest) -> None:
    validate_target_format(request.source_type, request.execution_mode, request.target_format)
    validate_s3_source_config(
        request.source_type,
...
def validate_update_request(
    ...
    *,
    ...
    validate_target_format(source_type, execution_mode, request.target_format)
    validate_s3_target_path(
        request.storage_type,
        request.storage_path,
        allow_unconfigured=settings.allows_header_auth_fallback,
...
def validate_target_format(source_type: str, execution_mode: str, target_format: str) -> None:
    normalized_source = str(source_type or "").strip().casefold()
    normalized_mode = str(execution_mode or "snapshot").strip().casefold()
    normalized_format = str(target_format or "").strip().casefold()
    if normalized_mode == "continuous":
        allowed_formats = {"parquet"}
    elif "kafka" in normalized_source:
        allowed_formats = {"jsonl"}
    else:
        allowed_formats = {"parquet", "csv", "json"}
    if normalized_format in allowed_formats:
        return
    allowed_label = ", ".join(sorted(allowed_formats))
    raise ApiError(
        ErrorCode.VALIDATION_ERROR,
        f"Unsupported target format '{target_format}'. Allowed formats: {allowed_label}",
        status.HTTP_422_UNPROCESSABLE_ENTITY,
    )
