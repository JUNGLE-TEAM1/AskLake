...
from app.services.clickhouse_client import ClickHouseError
...
def read_dashboard_filter_values(
    ...
    *,
    ...
    try:
        if session.clickhouse_client is not None:
            queries = sqlglot.transpile(query, read="clickhouse", write="clickhouse")
            if len(queries) != 1:
                raise ValueError("Dashboard ClickHouse filter-value query must contain one statement")
            raw_rows = session.clickhouse_client.query(
                queries[0], timeout_seconds=session.query_timeout_seconds
            ).rows
        elif session.trino_client is not None:
