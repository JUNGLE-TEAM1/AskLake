...
class TrinoSqlJobGovernance(CamelModel):
    ...
    permission_summary: str = Field(default="", max_length=2_000)
    ...
    def validate_scope_principal(self) -> "TrinoSqlJobGovernance":
        ...
        if not self.owner:
            raise ValueError("SQL Job owner is required")
        if self.access_scope == "project" and not self.principal_id:
            raise ValueError("Project access requires a real group principal")
        return self
