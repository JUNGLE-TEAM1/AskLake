...
class SqlJobStorageSegment(CamelModel):
    format: Literal["csv", "json", "jsonl", "parquet"]
    location: str


class SqlJobDatasetInput(CamelModel):
    dataset_id: str
    name: str
    storage_segments: list[SqlJobStorageSegment] = Field(min_length=1)


class SqlJobExecutionContract(CamelModel):
    version: Literal[1] = 1
    source_run_id: str
    base_dataset_id: str
    reference_dataset_ids: list[str] = Field(default_factory=list)
    query: str
    validated_read_only: Literal[True] = True
    datasets: list[SqlJobDatasetInput] = Field(min_length=1)
