...
- Issue #1073부터 신규 Kafka Job의 사용자 선택은 `실시간 · ClickHouse`와 `배치 · Spark`다. 신규 Continuous Job은 서버가 `runtimeEngine=kafka_connect_clickhouse_v2`를 영속화하고 Kafka Connect → ClickHouse `raw_events_v2`로만 실행한다. V2 flag/owner가 준비되지 않으면 fail closed하며 Spark로 자동 fallback하지 않는다. marker가 없는 기존 Continuous Job만 legacy Spark/Iceberg V1 경로를 유지한다. start 시 Catalog Dataset은 `preparing`으로 만들어지고, 해당 topic의 첫 offset이 확인되면 `available`과 revision/SSE event를 같은 publication transaction으로 기록한다. 상세 계약은 [Realtime Kafka Job engine routing](realtime-kafka-job-engine-routing.md)을 따른다.
