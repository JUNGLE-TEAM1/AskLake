...
9. 사용자가 Prefix Job을 실행하면 Spark는 같은 제외 규칙으로 prefix의 모든 데이터 파일을 읽고 실제 입력 파일 수·전체 입력 바이트·전체 입력 행 수를 Run manifest에 기록한다. 일반 Snapshot은 원본을 run 전용 Parquet staging에 먼저 한 번 기록한다. staging의 정확한 물리 크기가 운영자가 설정한 안전 한도 이하일 때만 그 staged DataFrame을 memory/disk cache로 재사용하고, 그 밖의 경우와 cache 준비 실패 시에는 staging Parquet를 다시 읽는다. 어느 경로도 후속 Rule·Quality·publish에서 원본 source로 돌아가지 않는다.
