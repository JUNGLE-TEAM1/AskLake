...
- 반복 Trino SQL Job은 결과 page를 복사하지 않고 SQL recipe, 실행 actor, 스케줄, target metadata를 저장한 뒤 첫 `run` command를 보낸다. 생성과 실행은 별도 요청이므로 첫 실행 실패가 durable Job을 삭제하거나 생성 성공을 되돌리지 않으며, UI는 부분 성공을 안내한다. 수동/예약 Run마다 전체 SQL을 다시 실행해 같은 논리 Dataset을 검증된 새 Iceberg table version으로 갱신한다. Base Dataset이 Kafka streaming이고 reference Dataset이 static인 일반 Trino SQL Job은 별도 `실시간 JOIN 만들기` 절차 없이 Kafka Dataset revision 증가를 backend worker가 감지해 같은 저장 SQL을 자동 재실행한다.
...
- Dashboard 보기 모드는 편집 모드와 같은 저장 `x`, `y`, `w`, `h` 레이아웃을 읽기 전용 grid로 렌더링한다. 화면 폭에 따른 반응형 축소는 허용하지만 위젯 순서를 다시 배치하는 별도 보기용 grid는 사용하지 않는다. 저장·게시로 canvas 폭이 바뀌면 차트는 현재 widget content box를 다시 측정해 plot을 카드 전체 크기에 맞춘다. 막대·선·영역 등 Cartesian 차트는 plot 아래에 별도 스크롤 범례 영역을 만들지 않는다.
...
- Dashboard와 SQL 결과의 막대·선·영역 차트는 값 축 범위를 `기본 자동 범위`, `데이터 차이 강조`, `직접 입력` 중에서 선택한다. 데이터 차이 강조는 현재 표시 series의 최솟값·최댓값 주변으로 범위를 좁히고, 직접 입력은 최소 또는 최대 한쪽만 지정할 수 있다. 기존 config에 범위 모드가 없으면 현재 자동 축 동작을 유지한다.
...
4. 사용자는 편집기 상단 `AI로 SQL 작성` Popover를 열고 선택 테이블과 schema context를 기반으로 SQL 초안을 받을 수 있다. 제출 후 입력 폼은 접히고 생성 상태와 적용 action이 Bubble로 표시된다.
