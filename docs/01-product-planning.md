...
| Day 2 | Job 실행 상태를 실행 이력과 실행 단계 상세 모달에 연결하고 Dataset을 SQL context로 전달 | 같은 Run ID가 실행 이력과 상세 모달에 보이고 SQL 화면에 선택 Dataset query가 채워진다. |
...
| Day 2 | Job command 결과를 Run 이력과 실행 단계 상세 모달 상태로 연결 | Dataset을 SQL context로 전달하고 result reset 기준 정리 | `SqlResult`를 Table Widget 초안으로 변환 |
