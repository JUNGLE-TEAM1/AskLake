...
`--loop`는 envelope `event_id`뿐 아니라 원본 `raw.event_id`/`raw.eventId`도 cycle과 stream offset으로 고유하게 만든다. Continuous merge key가 nested source event ID인 파이프라인에서도 반복 데이터가 기존 행 upsert로 소거되지 않고 새 이벤트로 누적된다.
