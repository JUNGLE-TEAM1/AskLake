...
브라우저에서는 생성된 Kafka Job의 `수정`을 열어 broker, topic, consumer group 값이 저장된 값으로 표시되고 편집할 수 없는지 확인한다. 이 시점에는 update API가 아직 없으므로 Review 단계의 생성 요청은 차단되며, 기존 Job을 중복 생성하지 않아야 한다.
