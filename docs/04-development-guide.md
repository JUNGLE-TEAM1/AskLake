...
6. Revision runner 준비 단계는 tree run의 input Dataset revision, parent Run의 static snapshot, output target과 private fence를 typed request로 직렬화한다. Kafka broker/topic/consumer group/offset/trigger는 payload에 넣지 않는다. Dataset revision 대기와 transform 제출은 Phase 5 범위다.
