...
NAT를 선택하면 `single`과 `per_az`의 비용·가용성 차이를 승인 기록에 남긴다. endpoint/hybrid를 선택하면 EC2, ECR API/DKR, Logs, STS interface endpoint와 S3 gateway가 baseline이고 workload/identity/secret/ALB 선택에 따른 추가 endpoint와 non-AWS egress를 별도로 검토한다. NAT를 유지한 채 S3 traffic만 우회할 때는 egress mode를 `hybrid`로 바꾸지 않고 `enable_s3_gateway_endpoint=true`만 사용한다. 이 조합은 기존 NAT 수와 private 기본 route를 유지하고 Interface Endpoint를 만들지 않으며, Terraform이 소유한 모든 private route table만 S3 Gateway Endpoint에 연결한다.

실제 적용 전에는 권위 있는 dev `terraform.tfstate`와 private tfvars를 확보하고 refresh된 전체 plan에서 생성 `1`(S3 Gateway Endpoint), 변경 `0`, 삭제 `0`인지 확인한다. state가 없거나 plan이 기존 VPC/NAT/EKS를 신규 생성·교체 대상으로 보면 apply하지 않는다. 적용 시 active Spark workload가 없어야 한다. Gateway Endpoint route 변경은 기존 S3 TCP 연결을 끊을 수 있으므로 Spark 실행 도중 endpoint를 생성·수정하지 않는다. 적용 후에는 endpoint `available`, EKS subnet과 연결된 private route table 2개의 S3 prefix-list route, NAT 기본 route 보존, Interface Endpoint 증분 0, ALB/RDS/MSK 및 외부 API health를 검증한다. 이어 같은 Raw/Output bucket을 사용하는 10GB bounded smoke 한 번과 NAT Gateway traffic 지표를 같은 UTC window로 수집한다. S3 read/write 성공과 NAT의 S3 대용량 증분 부재가 모두 확인된 뒤에만 100GB campaign을 재개한다.

private-only Kubernetes API를 유지하면 VPN/SSM/VPC runner 같은 `kubectl`·Helm 실행 경로도 배포 전에 확정한다. Phase 11 plan 성공은 network 연결 증거가 아니다. dev는 EKS private Pod에서 S3/STS, MSK `9098`, RDS `5432`, wrong-port와 VPC 외부 negative smoke를 통과했다. Auto Mode Network Policy Controller는 `infra/eks/network/auto-mode-network-policy-controller.yaml`로 활성화하고 NodeClass `DefaultAllow`에서 임시 deny enforcement를 확인했다. 실제 workload 정책은 B의 Service·port 계약 뒤 추가한다. 상세 기준과 증거는 [Phase 11 VPC와 Private Network Foundation](eks-phase-11-network-foundation.md), [7월 15일 Private Network 검증 기록](eks-day15-private-network-evidence.md)을 따른다.
