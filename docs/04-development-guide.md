...
6. Revision runner 준비 단계는 tree run의 input Dataset revision, parent Run의 static snapshot, output target과 private fence를 typed request로 직렬화한다. revision commit에는 exact Iceberg `snapshotId`를 저장하고 pinning은 snapshot이 있는 input만 tree/node에 고정한다. Kafka broker/topic/consumer group/offset/trigger는 payload에 넣지 않는다. transform executor 제출은 다음 단계 범위다.
