...
EKS Frontend image는 `VITE_API_BASE_URL`을 지정하지 않고 빌드해 browser와 같은 origin의 `/api`를 사용한다. `bash scripts/verify-eks-frontend-same-origin.sh`는 API·SSE client의 localhost 기본값과 EKS workflow의 API origin build arg를 배포 전에 차단한다.
