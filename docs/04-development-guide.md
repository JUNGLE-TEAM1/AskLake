...
Semantic Catalog는 dev의 route-owned Catalog loader와 분리된 Explorer 구조를 유지하면서 `데이터 선택 -> 업무 정의 -> AI 검색 -> 권한` 4단계를 제공한다. 시맨틱 화면은 상위 Catalog 가용 폭을 사용하며 공용 `Stepper`, `Tabs`, `Select`, `Table` 컴포넌트를 재사용한다.
