...
Dashboard runtime widget contract는 `metric`, `table`, ApexCharts 차트 8종을 기준으로 둔다. 색상은 문자열 `color`가 아니라 `color: { paletteId, customColors? }` 객체를 사용하며, 향후 AI widget 생성 기능도 같은 type/config 계약을 사용한다.
