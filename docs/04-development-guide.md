...
Kafka Source -> MinIO landing -> Catalog 등록 -> schedule tick 계약까지 한 번에 확인하려면 아래 smoke를 실행한다. 이 스크립트는 고유 `reviews.raw.verify.*` topic에 100건 fixture를 넣고, due 상태의 Kafka ETL Job을 만든 뒤 `/api/etl/schedules/run-due`로 실행해 다음 예약 시각이 advance되는지까지 확인한다. Snapshot Phase 1에서는 partition별 `startOffset/endOffset` metadata와 성공 후 consumer group offset 전진, 같은 group의 다음 empty snapshot도 함께 검증한다.
