...
AI runtime은 MVP에서 `direct`로 선택했다. contract와 정적 검증은 `OPENAI_API_KEY` 한 개를 full-service Backend profile에 추가하지만, 실제 Secrets Manager source에 유효한 key가 없으면 ExternalSecret apply와 Backend rollout을 실행하지 않는다. key가 준비된 뒤 source exact set, staged target byte hash, canonical target handover, FastAPI 2 replica와 ALB/RDS/Trino/AI health를 차례로 검증한다. 값은 command output, evidence 또는 Git에 남기지 않는다.
