...
# Continuous SQL JOIN 자동 유일키 검증 확인

Frontend는 `npm run test:continuous-sql-ui`와 `npm run build`, Backend는 `python -m pytest tests/test_catalog_unique_key_verification.py -q`로 검증한다.
