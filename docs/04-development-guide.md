...
7/17 General/Spark 통합 관찰은 `scripts/capture-eks-day17-autoscaling-evidence.sh`의 `baseline → sample → final` 순서를 사용한다. 동일한 비공개 run token을 유지하고 evidence는 저장소 밖 또는 `infra/eks/delivery/*.day17-autoscaling-evidence.json`에 mode `0600`으로 보관한다. baseline은 blocker를 기록만 하지만 sample의 release identity drift와 final의 placement/blocker/cleanup 미충족은 실패한다. 하네스는 조회 전용이며 실제 부하 생성과 cleanup은 별도 승인된 phase가 소유한다. 세부 계약은 [Day 17 autoscaling 관찰 하네스](eks-day17-a-autoscaling-observer.md)를 따른다.

A 소유 NodePool만 먼저 검증할 때는 confirmation-gated `scripts/run-eks-day17-isolated-nodepool-smoke.sh`를 사용한다. 실행기는 General 1 CPU Pod, Spark 2 CPU Pod와 toleration 없는 Spark 음성 Pod만 만들며 `isolated` evidence scope로 두 pool의 scale-out/in과 cleanup을 검사한다. 이는 FastAPI HPA와 Spark 비즈니스 Job 통합 증거를 대신하지 않는다. 실제 결과는 [Day 17 Pair A 격리 NodePool 검증 기록](eks-day17-a-isolated-nodepool-evidence.md)을 따른다.
