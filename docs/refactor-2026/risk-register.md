...
| R-003 | P1 | `etl_service.py` 9,088 LOC God Service | 외부 subprocess/file/manifest I/O를 Port·Adapter로 분리; command, reconciliation, publication 추출은 05~07에서 계속 | 04~07 | PARTIAL |
| R-004 | P1 | Continuous 상태 권위가 DB, container, report, checkpoint, manifest, Catalog에 분산 | desired/observed/public 상태와 fencing 확정, report/manifest adapter 분리; command/reconciliation/publication은 05~06에서 계속 | 03~06 | PARTIAL |
