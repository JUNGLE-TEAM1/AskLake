...
- Target write or checkpoint failure leaves the previous successful checkpoint authoritative. Each non-empty batch derives a deterministic `sourceBoundary` and `_asklake_run_id` from Job, batch ID, checkpoint, consumer identity and `[startOffset, endOffset)` ranges. If Iceberg commit succeeds but manifest publication fails, retry finds that marker in the table and reuses the committed snapshot instead of appending again. Quarantine and schema-evidence sidecars still require `_SUCCESS` plus their hidden signatures. Backend verifies the exact reported Iceberg snapshot through Trino before reconciling the manifest into an idempotent Catalog run, then acknowledges the highest contiguous Catalog batch.
...
- V1 requires the Continuous worker to be paused or stopped before quarantine inspection or replay. Runtime row locking serializes inspection and persisted maintenance. Replay appends to the same Iceberg table with deterministic `_asklake_run_id`, stores commit/source-boundary evidence, and exposes `catalogApplied`; pending Catalog verification is retried from the successful maintenance result. A persisted maintenance run has a 900-second default lease; expiry marks it failed and removes its named Spark submission/container.

## 12. Compaction Contract

- The former `_batches` Parquet compaction cannot see or safely mutate an Iceberg target and is disabled with `422 KAFKA_CONTINUOUS_ICEBERG_COMPACTION_UNAVAILABLE`.
- Phase 5 will define Iceberg-native data-file rewrite, snapshot expiration and orphan-file cleanup as finite maintenance Jobs. Those Jobs must not mutate the streaming checkpoint or run concurrently with an incompatible writer operation.
