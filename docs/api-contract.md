...
Spark manifest에는 `status`, `runId`, `startedAt`, `endedAt`, `durationMs`, `inputFileCount`, `inputBytes`, `inputRows`, `outputFileCount`, `outputRows`, `outputPath`, `schema`, `transform`, `quality`, `phaseTimings`, `sparkResources`, `failedStage`, `error`가 포함될 수 있다. `phaseTimings`는 `sourceValidation`, `ruleEvaluation`, `qualityAggregation`, `sourcePostValidation`, `targetPublish`별 `startedAt`, `endedAt`, `durationMs`를 보존한다. `sparkResources`는 `cacheStorageLevel`, `defaultParallelism`, `driverCores`, `executorCores`, `executorInstances`, `sqlShufflePartitions`, `outputFrameCacheMode`를 보존한다. `outputFrameCacheMode=source_cache_direct_publish`는 canonical counter로 행 수가 확정되어 write 후 재사용하지 않을 output frame을 다시 persist하지 않고 source cache에서 직접 publish한 경로다. counter fallback과 legacy Quality는 `materialized_output_cache`를 사용한다. `transform.rowPreservingSqlExpressionCount`는 직접 컬럼 복사 또는 `TRIM(CAST(<input> AS STRING))`의 승인된 total·row-preserving subset으로 컴파일되어 규칙별 전체 validation action을 만들지 않은 SQL transform 수다. `transform.preMaterializedTransformCount`는 canonical transform 선두 prefix 중 같은 Spark type의 `String`/`Long`/`Boolean` identity cast·copy·rename과 이 SQL subset을 첫 source projection cache에 포함한 수다. type-changing transform, 그 밖의 SQL expression과 `SELECT`는 기존 validation action과 failure disposition을 유지한다. canonical Rule 결과는 Quality aggregate의 정확한 `evaluatedRowCount`, `droppedCount`, `quarantinedCount`로 최종 행 수를 계산하고 `quality.outputRowCountSource=canonical_quality_counters`를 남겨, final projection·metadata column·repartition 뒤의 중복 `count()`를 실행하지 않는다. 이 counter 근거가 없거나 유효하지 않으면 `quality.outputRowCountSource=spark_count_fallback`과 기존 전체 count를 사용한다. legacy Quality aggregate는 `legacy_quality_aggregate`를 남긴다. counter 재사용 경로는 최종 frame이 아직 전부 materialize되지 않았으므로 source projection cache를 Iceberg/Parquet publish가 끝날 때까지 유지해 원본 재읽기를 막는다. Prefix runtime은 Preview와 같은 객체 제외 규칙으로 실제 경로를 열거하고 이 file/byte/row 수치를 실행 증거로 기록한다. Phase 2는 이 manifest와 물리 Parquet 또는 Iceberg commit까지 저장하지만 Catalog materialization/lineage 갱신은 수행하지 않는다.
