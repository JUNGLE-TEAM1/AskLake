...
`continuousRuntime` additionally exposes `runtimeProvider`, `runtimeApplicationId`, `runtimeJobId`, `runtimeAttempt`, `runtimeState`, `runtimeLogReference`, `runtimeRequestedAction`, `runtimeCancelRequestState`, `runtimeCancelRequestedAt`, `runtimeCancelAcceptedAt`, `runtimeCancelCompletedAt`, `runtimeCancelFailedAt`, `runtimeCancelError`, `lastCatalogAckError`, `lastSuccessfulCheckpoint`, `maxPartitionLag`, `laggingPartitionCount`, `lagAvailable`, `partitionProgress`, `lastBatchDurationMs`, `lastBatchInputRows`, `throughputRowsPerSecond`, optional `endToEndLatency`, `replayedCount`, `schemaVersion`, `schemaFingerprint`, `schemaStatus`, `schemaChanges`, `ruleContractVersion`, `ruleFingerprint`, `runtimeFingerprint`, `ruleMetrics`, and `lastRuleResult`. `endToEndLatency` is `null` before the first successful non-empty batch. Otherwise the runtime value is a checkpoint-recoverable conservative summary with `aggregation: "worst-successful-batch-percentile"`, `method: "kafka-record-timestamp-to-target-commit"`, cumulative `batchCount`, `sampleCount`, `timestampMissingCount`, worst per-batch `p50Ms`, `p95Ms`, `p99Ms`, `measuredAt`, `lastIncludedBatchId`, and the `latest` batch value. Each batch manifest keeps only that batch's percentiles. The runtime P95 is therefore not a mathematically merged global record percentile. Remote/cancellation fields are nullable for Docker/legacy rows. `runtimeCancelRequestState` is `requested | accepted | completed | failed`; only `completed` plus remote `CANCELLED` confirms operator pause/stop. `lastCatalogAckError` is retry evidence and does not replace remote liveness. `ruleMetrics` contains cumulative transform/quality warn, quarantine, drop, set-null, invalid/error, and failed-batch counts. `replayedCount` prevents recovered quarantine rows from being double-counted: `storedCount + quarantinedCount - replayedCount = consumedCount`. Worker logs are limited to 1,000 lines, ANSI-stripped, and redact common key/token/password assignments.
