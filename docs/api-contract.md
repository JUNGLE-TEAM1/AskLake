...
신규 Kafka Continuous Job의 `continuousConfig.runtimeEngine`은 server-owned 값이다. `CLICKHOUSE_REALTIME_V2_ENABLED=true`, `KAFKA_CONNECT_SINK_ENABLED=true`, `CLICKHOUSE_REALTIME_CONSUMER_OWNER=kafka_connect_v2`가 모두 설정된 배포는 `kafka_connect_clickhouse_v2`, 그 외 fail-closed V1-only 배포는 `spark_structured_streaming`을 저장한다. V2 Job은 같은 command API를 유지하되 Spark Structured Streaming을 시작하지 않는다. control-plane worker가 Kafka Connect raw sink를 등록·재개하고 `raw_events_v2_current`의 해당 topic/partition/offset을 관찰한다. `startContinuous` 직후 Catalog Dataset은 pending serving binding과 `status=preparing`으로 저장되며, 첫 offset은 active binding, `status=available`, `dataset_freshness`, `dataset_revision_commits`, schema-v2 `dataset.revision.committed` event를 같은 transaction으로 게시한다. Dashboard reader는 active binding과 `streamingSource.topic`으로 ClickHouse raw view를 제한한다. V2 상태는 Spark runtime report 대신 Connector state와 ClickHouse offset을 `continuousRuntime`에 직접 투영하고 `processingResult.worker`는 `kafka_connect_clickhouse_v2`다. 이 hot-ingest slice는 S3/Iceberg archive를 만들지 않는다.
...
- `basicInformation`은 내부 `id`와 자동 생성용 `jobName`을 제외하고 소스, 처리 방식, 출력 데이터셋 이름, 설명을 반환합니다. `executionMode=snapshot`은 `배치 · Spark`다. Continuous는 effective deployment engine에 따라 `실시간 · Spark (기존 V1)` 또는 `실시간 · ClickHouse`로 표시합니다. 신규 Continuous Job의 response `continuousConfig`에는 server-owned `runtimeEngine`과 `runtimeGeneration=1`이 additive하게 포함됩니다. 기존 marker 없는 Job은 Spark V1 호환 row입니다.
...
Spark manifest에는 `status`, `runId`, `startedAt`, `endedAt`, `durationMs`, `inputFileCount`, `inputBytes`, `inputRows`, `outputFileCount`, `outputRows`, `outputPath`, `schema`, `transform`, `quality`, `phaseTimings`, `sparkResources`, `failedStage`, `error`가 포함될 수 있다. `phaseTimings`는 `sourceValidation`, `materializationStaging`, `ruleEvaluation`, `qualityAggregation`, `sourcePostValidation`, `targetPublish`별 `startedAt`, `endedAt`, `durationMs`를 보존한다. `sparkResources`는 `cacheStorageLevel`, `defaultParallelism`, `driverCores`, `executorCores`, `executorInstances`, `sqlShufflePartitions`, `materializationMode`, `materializationFileCount`, `materializationCleanupStatus`, `outputFrameCacheMode`를 보존한다. 일반 Snapshot batch는 `cacheStorageLevel=NONE`, `materializationMode=run_scoped_parquet_staging`, `outputFrameCacheMode=staged_parquet_reuse`를 사용한다. `materializationFileCount`는 run staging write가 만든 Parquet data file 수고 `materializationCleanupStatus`는 success·quality failure·schema exception 뒤 run staging 정리 결과다. `transform.rowPreservingSqlExpressionCount`는 직접 컬럼 복사 또는 `TRIM(CAST(<input> AS STRING))`의 승인된 total·row-preserving subset으로 컴파일되어 규칙별 전체 validation action을 만들지 않은 SQL transform 수다. `transform.preMaterializedTransformCount`는 canonical transform 선두 prefix 중 같은 Spark type의 `String`/`Long`/`Boolean` identity cast·copy·rename과 이 SQL subset을 source→staging write에 포함한 수다. type-changing transform, 그 밖의 SQL expression과 `SELECT`는 기존 validation action과 failure disposition을 유지한다. canonical Rule 결과는 Quality aggregate의 정확한 `evaluatedRowCount`, `droppedCount`, `quarantinedCount`로 최종 행 수를 계산하고 `quality.outputRowCountSource=canonical_quality_counters`를 남겨, final projection·metadata column·repartition 뒤의 중복 `count()`를 실행하지 않는다. 이 counter 근거가 없거나 유효하지 않으면 `quality.outputRowCountSource=spark_count_fallback`과 staged DataFrame 전체 count를 사용한다. legacy Quality aggregate는 `legacy_quality_aggregate`를 남긴다. 후속 count/quality/sample/publish action은 raw source lineage가 아니라 run-scoped staged Parquet를 읽으므로 원본 physical full read를 한 번으로 제한한다. Prefix runtime은 Preview와 같은 객체 제외 규칙으로 실제 경로를 열거하고 이 file/byte/row 수치를 실행 증거로 기록한다. Phase 2는 이 manifest와 물리 Parquet 또는 Iceberg commit까지 저장하지만 Catalog materialization/lineage 갱신은 수행하지 않는다.
...
- Trino `DESCRIBE`, `$refs`, `$snapshots`와 snapshot summary로 확인한 같은 Iceberg table과 물리 data file. 새 manifest에서는 Spark `outputFileCount`와 `icebergCommit.dataFileCount`도 exact snapshot의 `total-data-files`와 같아야 한다.
...
일반 Spark batch는 logical `outputPath=iceberg://catalog/namespace/table`을 사용한다. writer는 commit 직후 exact snapshot summary의 `total-data-files`를 `outputFileCount`와 `icebergCommit.dataFileCount`에 기록한다. backend는 reported snapshot ID를 Trino `$refs`의 `main` current snapshot과 대조하고, 해당 snapshot의 `$snapshots.summary`에 기록된 `total-data-files`와 `total-files-size`를 저장한다. 세 file count가 다르면 성공으로 확정하지 않는다. EKS bounded fixture도 이 Iceberg 분기를 사용하며, exact snapshot에서 `_asklake_run_id=runId`인 행 수가 persisted `expectedCount`와 정확히 같아야 한다. `$files`는 current table의 보조 물리 확인에만 사용하며 rollback 이후 abandoned history의 newest snapshot을 current로 오인하지 않는다. `outputRows>0`인데 data file 또는 byte 증거가 0이면 reconciliation을 실패시킨다. 기존 non-Iceberg 호환 결과만 S3A/local path의 Parquet object를 직접 검사한다. Catalog `sampleRows`는 Spark가 제공한 제한된 transformed output sample을 사용할 수 있으며, 그런 sample이 없으면 빈 배열을 사용한다. schema나 값이 달라질 수 있는 pre-transform source sample을 output sample로 가장해서는 안 된다.
...
- 실패 시 Catalog partial update는 rollback한다. 검증된 Iceberg snapshot과 성공 `sparkResult`는 삭제하지 않는다. Spark commit 뒤 report 확정 전에 실패한 경우에는 writer가 자신이 방금 commit한 snapshot이 여전히 `main`일 때만 이전 snapshot으로 rollback한다. 다른 writer가 `main`을 이동했거나 같은 Run이 기존 snapshot을 재사용한 경우에는 그 snapshot을 임의로 되돌리지 않는다.
...
- EKS workload 기본값은 `ASKLAKE_CONTINUOUS_CONTROL_PLANE=external_ec2`이며 ClickHouse Realtime V2를 비활성화한다.
...
- 승인된 Realtime owner transfer에서는 EKS web/API를 `ASKLAKE_CONTINUOUS_CONTROL_PLANE=local`, `CONTINUOUS_CONTROL_PLANE=disabled`로 전환하고 별도 EKS Worker만 `CONTINUOUS_CONTROL_PLANE=worker`를 사용한다. V2/sink/owner/private Service URL/TLS Secret은 하나의 schema-validated opt-in이며 EC2 process 0과 canonical ownership manifest 변경 없이 활성화할 수 없다.
