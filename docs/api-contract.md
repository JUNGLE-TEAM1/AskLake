...
Spark manifest에는 `status`, `runId`, `startedAt`, `endedAt`, `durationMs`, `inputFileCount`, `inputBytes`, `inputRows`, `outputFileCount`, `outputRows`, `outputPath`, `schema`, `quality`, `phaseTimings`, `sparkResources`, `failedStage`, `error`가 포함될 수 있다. `phaseTimings`는 `sourceValidation`, `ruleEvaluation`, `qualityAggregation`, `sourcePostValidation`, `targetPublish`별 `startedAt`, `endedAt`, `durationMs`를 보존한다. `sparkResources`는 `cacheStorageLevel`, `defaultParallelism`, `driverCores`, `executorCores`, `executorInstances`, `sqlShufflePartitions`를 보존한다. Prefix runtime은 Preview와 같은 객체 제외 규칙으로 실제 경로를 열거하고 이 file/byte/row 수치를 실행 증거로 기록한다. Phase 2는 이 manifest와 물리 Parquet 또는 Iceberg commit까지 저장하지만 Catalog materialization/lineage 갱신은 수행하지 않는다.
