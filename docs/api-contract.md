...
`DELETE /api/etl/jobs/{jobId}`는 Job 삭제 권한과 active Run/Continuous runtime 검증을 통과한 뒤 하나의 DB transaction으로 처리한다. `catalog_datasets.producer_job_id`가 삭제 Job ID와 일치하는 Dataset은 삭제하지 않는다. 해당 Dataset은 source provenance와 physical binding, `available` status를 유지한 채 `runtimeStatus="producer_deleted"`와 `nextRefresh="예정 없음"`으로 기록된다. 따라서 Catalog, SQL 분석, Dashboard에서는 기존 Dataset을 계속 조회·사용할 수 있지만 자동 갱신은 더 이상 예약되지 않는다.

Job 삭제는 Iceberg/S3/ClickHouse의 물리 결과, Catalog Dataset row, 별도 Dataset deletion receipt를 정리하지 않는다. Dataset을 목록과 SQL/Dashboard 선택지에서 제거하거나 물리 purge가 필요할 때만 이름 확인과 impact 검사를 거치는 `DELETE /api/catalog/datasets/{datasetId}?confirmName=...`를 사용한다. 다른 Job이 게시했거나 `producerJobId`가 없는 legacy Dataset도 추정으로 변경하지 않는다.
