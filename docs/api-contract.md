...
- actor에게 dataset `query` 권한이 있어야 하며, 없으면 `403 FORBIDDEN`을 반환합니다.
- 성공한 materialization 중 가장 최신 버전의 실제 CSV/JSON/JSONL/Parquet 데이터를 읽습니다.
- `limit`은 `1..500`, `offset`은 `0` 이상이며 응답은 한 page만 메모리에 올립니다.
...
GET /api/catalog/datasets/{datasetId}/rows
...
- Trino 운영 정책: Iceberg catalog/metastore 고가용성, 결과 retention/cursor, estimate/guardrail 및 조직별 quota 구체화.
