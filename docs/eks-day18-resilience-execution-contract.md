...
1. 구현 revision이 candidate image revision과 선택한 `dev` base의 조상인가
...
준비기와 binder는 기존 파일을 덮어쓰지 않는다. 최신 `dev`가 바뀌면 기존 bound
...
당시 이전 로컬 candidate 파일은 workflow run `29651079168`의 artifact가 아니라 더 오래된
Git revision을 가리켜 build-input freshness gate에서 거부됐다. 해당 파일을 재사용하지
않고 당시 최신 `pair1` workflow artifact를 새 mode-`0600` 파일로 검증해 바인딩했다. 이는
과거 실행 증거이며 현재 delivery source는 `dev`다.
