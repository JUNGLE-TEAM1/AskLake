...
- `backend/scripts/cleanup-rag-artifacts.py` is dry-run by default and runs as a production cleanup service with `--apply --loop`. It removes retired OpenSearch indexes and, when Trino/Iceberg is enabled, drops the corresponding job-scoped parent/chunk tables. Retention days and previous-index count are configured in Settings.
