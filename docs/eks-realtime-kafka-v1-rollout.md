...
이 문서는 pair1의 유일한 Realtime runtime인 Spark Structured Streaming을 EKS에
...
4. pair1 commit에서 만든 immutable Backend/Spark image receipt를 확인한다.
...
1. foundation/web/workloads를 pair1 values로 순서대로 upgrade한다.
...
`origin/pair1` 기준 전체 diff를 확인해 이 작업 범위 밖 변경 0건, case-insensitive path
