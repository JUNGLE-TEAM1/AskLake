...
| EKS frontend same-origin API | `.github/workflows/eks-image-delivery.yml` + `scripts/verify-eks-frontend-same-origin.sh` | `enabled` | API·SSE client가 localhost를 기본값으로 사용하거나 workflow가 `VITE_API_BASE_URL`을 image에 주입하면 image publish 전에 실패 | maintainer | EKS Frontend는 환경 중립 image를 사용하고 CloudFront/ALB의 same-origin `/api` routing을 따른다. |
