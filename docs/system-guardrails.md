...
| EKS Phase 12 NodePool selection gate | `scripts/verify-eks-auto-mode-node-pools.sh`, Terraform checks and Helm schema | `day17 contract audited` | render zero custom resources by default; reject missing IAM access ownership, subnet/security-group selectors, capacity/instance limits or disruption choices; block load attribution while unrelated temporary Job, active SparkApplication, terminating/Pending workload, ALB drain or release generation drift remains; require intended pool placement in the owning component manifest rather than accepting an incidental scheduler result | Pair A/B | Test fixture values are not deployment recommendations. dev custom pools are Ready and Day 14 synthetic General scale-out/in passed. Web/Collector와 Spark placement는 일치하지만 Airflow selector와 Trino custom General placement는 미정합이다. Actual FastAPI/Spark load, negative taint, limit/cost and final scale-in evidence remain; live HPA source must be reconciled with pair1 before approval. [Baseline](eks-day17-a-autoscaling-baseline.md), [contract audit](eks-day17-a-nodepool-contract-audit.md). |
...
| EKS Phase 14 web/collector workload gate | `scripts/verify-eks-web-workloads.sh`, `scripts/run-eks-day15-backend-rollout-smoke.sh` and receipt/context-gated deploy/destroy scripts | `live passed` | render zero resources by default; require immutable frontend/backend digests, Frontend/FastAPI two replicas, Collector exactly one replica, explicit resources, runtime ConfigMap/Secret, General placement and B runtime-boundary approval; Collector must share the exact Backend digest and identity but expose no HTTP/Service/Ingress or Kubernetes API token; FastAPI termination must cover the live ALB deregistration delay, currently 310-second preStop inside a 360-second grace period for the dev 300-second target-group contract; final rollout must restore all three Deployments and a terminal Query Run | Pair A/B | Collector correctness stays in RDS lease/generation and persisted continuation state. Stuck rows are cancelled through the authenticated API, never deleted manually. 2026-07-17 dev에서 count 100, slot 0, Pod replacement same-run recovery와 duplicate page 0을 통과했다. [Evidence](eks-day17-trino-result-collector-evidence.md). |
...
| Continuous SQL contract | `backend npm run verify:continuous-sql-contract`, `npm run verify:realtime-stack`, `Realtime Quality Gates` | `enabled` | unsupported SQL, unsafe static key, stale generation/fence, invalid lifecycle/publication identity면 matching PR check 실패 | maintainer | 실제 Kafka/Spark/Iceberg fault·restart는 scheduled/manual tier, stream-static JOIN은 operator gate |
...
| Prod compose config | yes on matching realtime paths | `deploy/docker-compose.prod.yml` | `TRINO_ENABLED=false` excludes every Trino-profile service without Trino secrets/files/buckets; `true` plus `COMPOSE_PROFILES=trino` renders the coordinator/bootstrap/workers with internal + outbound networks |
...
| EKS Phase 14 web/collector workloads | PR static + manual runtime apply | Frontend/FastAPI/Collector Helm chart, Phase 6 receipt and Phase 5/8 references | default renders nothing; enabled fixture renders three digest-pinned Deployments and two ClusterIP Services; Collector shares Backend digest with one replica and no HTTP/token; unsafe readiness, replica, identity, image and port overrides fail |
...
| Trino contract verification | matching backend/frontend paths | Query Run, registration, result storage, collector fencing, actor reservation, timeline state | `verify:trino-query-foundation`, `verify:query-engine-registration`, `verify:trino-result-storage`, `verify:trino-collector-resilience`, `verify:trino-submission-guard`, `test:trino-timeline` pass; AWS pre-created Query Result storage must not require bucket-wide `HeadBucket` and proves access with prefix-scoped object writes |
...
| Realtime live runtime | daily schedule or manual opt-in | disposable Kafka/Spark/Iceberg/Trino fault·restart harness | checkpoint recovery, atomic append, Catalog queryability와 duplicate 방지가 통과; production ALB/browser evidence는 별도 |
| EC2 Compose backend image build | no, local/manual until CI exists | `backend/Dockerfile`, `backend/requirements.txt` | backend Docker image builds with production Python base image |
