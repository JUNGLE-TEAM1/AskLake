...
| Storage Layout V1 contract | `cd backend && npm run verify:storage-layout-contract` | `partial` | fail when Node/Python, MinIO/AWS logical paths, checkpoint isolation, retention, or production URI guard drifts | maintainer | repo verifier는 동작하며 실제 bucket lifecycle 적용과 CI required check 연결은 후속 |
...
| Storage Layout identity | matching backend/deploy paths | Batch/Continuous data, checkpoint, manifest, quarantine, log reference | `verify:storage-layout-contract` keeps provider-neutral V1 paths and rejects production local data-plane paths; lifecycle enforcement remains operator-owned |
