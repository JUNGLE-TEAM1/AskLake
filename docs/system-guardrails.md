...
| Production legacy demo auth | backend startup + `scripts/verify-deploy-env.sh` | `enabled` | production preflight rejects legacy demo identity configuration | maintainer | `AUTH_LEGACY_DEMO_USERS_ENABLED`는 false만 허용하고 frontend legacy flag는 production env에 둘 수 없다. Bootstrap admin과 production header-auth 차단은 유지한다. |
...
| ClickHouse Realtime Serving V2 전환 | backend startup validation, `/api/health/realtime`, deploy preflight/regression, `Realtime Quality Gates`의 V2 foundation·Alembic lifecycle·Compose render, PR별 V2 gates | `partial` | 기본 false/disabled, 모순된 V1/V2 owner는 startup 실패, V2 enabled 상태는 live probe가 생길 때까지 health 503, migration/profile/TLS/image/secret wiring drift는 test 실패; receipt/parity/cutover는 후속 gate | data-platform | PR02는 exact image/plugin provenance, private profile과 10-table expand schema까지만 구현한다. connector 미등록·수집 없음, 단일 노드 demo/staging. production canonical 전환은 PR09 operator evidence와 별도 승인 전 금지 |
...
| Production legacy demo rejection | focused auth tests + deploy preflight | backend startup account/session state and production env | known demo identities stay disabled and any production opt-in flag fails before Compose mutation |
