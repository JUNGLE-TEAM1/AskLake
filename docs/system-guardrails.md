...
| ClickHouse Realtime Serving V2 전환 | `docs/codex-clickhouse-realtime-pr-pack/STACKED_PR_PLAN.md`, PR별 V2 contract/integration/full-stack gate | `planned` | 선행 PR 미병합, V2 flag 기본 true, V1/V2 consumer 동시 claim, receipt gap, binding/archive parity 불일치 또는 rollback drill 부재면 merge/cutover 중단 | data-platform | 기존 ClickHouse V1과 realtime event log를 expand-only로 확장한다. production canonical 전환은 PR09 operator evidence와 별도 승인 전 금지 |
