...
| EKS Day 15 final integration gate | `scripts/verify-eks-day15-final-integration.sh`, Terraform no-change plan and redacted evidence | `manual` | require the reviewed Backend commit and immutable ECR digest, two Ready zero-restart Pods, exact steady ALB/Secret/RDS, Backend S3 boundary with cleanup zero, `external_ec2` with zero EKS Continuous processes, a running preserved EC2 runtime and no Terraform resource changes | Pair A/B | this closes Issue #794 runtime evidence only; it does not approve Airflow/Spark/Trino E2E, production cutover or EC2 deletion. [Evidence](eks-day15-final-integration-evidence.md). |
| EKS Phase 14 web workload gate | `scripts/verify-eks-web-workloads.sh`, `scripts/run-eks-day15-backend-rollout-smoke.sh` and receipt/context-gated deploy/destroy scripts | `manual` | render zero resources by default; require immutable frontend/backend digests, two replicas, explicit resources, runtime ConfigMap/Secret, General placement and B runtime-boundary approval; final same-digest rollout must keep every sampled external health request successful and restore two Ready zero-restart Pods with the same imageID, steady ALB/Secret/RDS health and zero EKS Continuous processes | Pair A/B | dev `asklake-web`은 Backend scheduler 수정 image까지 배포됐고 two-replica rollout/RDS health, `external_ec2` isolation과 외부 ALB smoke를 확인했다. Backend S3 경계는 별도 runtime gate로 통과했으며 Spark/Trino runtime 검증은 후속이다. Deploy workloads before Ingress; destroy Ingress before Services. |
