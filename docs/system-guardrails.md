...
| EKS 15.5 Backend image handoff | Backend Catalog rows service and TestClient HTTP tests, Phase 6 formal image receipt, `scripts/preflight-eks-backend-image-rollout.sh`, `scripts/rollout-eks-backend-image.sh`, runtime error verifier and atomic Backend-only rollout | `runtime drift found / replacement image pending` | reject a receipt whose full revision does not contain the Iceberg rows fix, reject mutable/non-AMD64 image, values changes outside `backend.image`, dry-run mutation or Deployment/Pod digest mismatch, and roll back when two-replica health, ALB/RDS or Continuous ownership drifts; require the EKS Auto Mode readiness-gate namespace key and a healthy target condition on every new Backend Pod; accept only an exact approved Backend Secret key set with full source/target hash equality; before Trino deployment require sanitized HTTP 502 `SQL_STORAGE_ERROR` with normalized reason and without enum implementation/private endpoint/query/token leakage | Pair A/B | Phase 3 exposed rolling-update 502 and corrected the managed webhook key to `eks.amazonaws.com/pod-readiness-gate-inject`. The first fix image is currently 2/2 and ALB steady. Phase 4 confirmed sanitized HTTP 502 but found `ErrorCode.BACKEND_TIMEOUT` reason drift; source now emits `BACKEND_TIMEOUT`, with replacement image/live rerun pending. Trino snapshot HTTP 200 remains separate. [Handoff](eks-day15-5-backend-image-handoff.md), [evidence](eks-15-5-runtime-remediation-evidence.md). |
