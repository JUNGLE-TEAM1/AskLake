...
| AWS staging Phase 0~4 contract | `.github/workflows/aws-staging-contract-checks.yml`, `cd backend && npm run verify:aws-staging-contract && npm run verify:aws-staging-terraform && npm run verify:aws-staging-runtime && npm run verify:aws-staging-workflows && npm run verify:aws-staging-smoke` | `partial` | fail when staging region/no-NAT endpoint/state/cost/cap, Terraform schema, Runtime mapping/redaction, manual-only OIDC trigger, reviewed plan fingerprint, protected mutation approval, actual quota gate, cleanup independence, private artifact cleanup, conditional/checksummed activation, SSM runner bundle, exact count/lag/quarantine/checkpoint/Job identity evidence drifts | maintainer | AWS credential 없는 PR check, Terraform/mock plan, private Runtime 변환과 수동 plan/apply/artifact/smoke/destroy 코드는 동작한다. 실제 AWS workflow evidence와 TTL sweep은 후속이며 일반 deploy에는 연결하지 않는다 |
