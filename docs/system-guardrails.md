...
| AWS staging Phase 0 contract | `cd backend && npm run verify:aws-staging-contract` | `partial` | fail when staging region, network isolation, Terraform state, OIDC/IAM, budget/TTL, EMR cap, functional smoke, or lifecycle boundary drifts | maintainer | versioned contract와 negative verifier는 동작한다. Terraform resource, GitHub workflow, 실제 AWS smoke와 TTL sweep은 후속이며 일반 deploy에는 연결하지 않는다 |
