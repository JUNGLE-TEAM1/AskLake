...
| AWS staging Phase 0/1/2 contract | `cd backend && npm run verify:aws-staging-contract && npm run verify:aws-staging-terraform && npm run verify:aws-staging-runtime` | `partial` | fail when staging region, network isolation, Terraform state, OIDC/IAM, budget/TTL, EMR cap, functional smoke, module/provider schema, output mapping, private file mode, broker redaction, or lifecycle boundary drifts | maintainer | versioned contract, Terraform module/mock plan과 stack별 private env/redacted manifest 변환은 동작한다. 실제 AWS plan/apply, artifact upload, GitHub workflow, smoke와 TTL sweep은 후속이며 일반 deploy에는 연결하지 않는다 |
