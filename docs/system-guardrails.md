...
| AWS staging Phase 0~5 contract | `.github/workflows/aws-staging-contract-checks.yml`, `cd backend && npm run verify:aws-staging-contract && npm run verify:aws-staging-terraform && npm run verify:aws-staging-runtime && npm run verify:aws-staging-workflows && npm run verify:aws-staging-smoke && npm run verify:aws-staging-lifecycle` | `partial` | fail when staging region/no-NAT endpoint/state/cost/cap, Terraform schema, Runtime mapping/redaction, manual OIDC trigger, reviewed plan fingerprint, protected mutation approval, private artifact cleanup, checksum activation, SSM runner bundle, exact count/lag/quarantine/checkpoint/Job identity, evidence-before-cleanup, or TTL tag/state audit drifts | maintainer | AWS credential 없는 PR check, Terraform/mock plan, private Runtime 변환, smoke cleanup과 scheduled TTL audit 코드는 동작한다. 실제 AWS workflow evidence와 TTL sweep evidence는 후속이며 일반 deploy에는 연결하지 않는다 |
