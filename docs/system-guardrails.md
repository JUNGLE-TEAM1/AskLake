...
| Frontend API transport error classification | `frontend npm run test:sql-transport-errors` inside `verify:ui-regressions` | `enabled` | browser `fetch` network failure가 raw `Failed to fetch`로 노출되거나 SQL 화면에서 Trino validation failure로 오분류되면 matching test 실패 | analytics-experience | network/timeout은 API transport failure로 표시하고 backend structured `ApiError`의 code·stage·diagnostic context는 보존한다. |
...
| Dev public entrypoint ownership | EKS Ingress status, ALB target health, same-origin `/`·`/api/health` manual smoke and deployment handoff | `partial` | current ALB smoke가 없는 legacy EC2/`sslip.io` URL을 canonical demo URL로 전달하지 않는다 | maintainer | 실제 ALB hostname은 private handoff에서 관리한다. Frontend가 cached page를 표시해도 같은 origin `/api`가 연결되지 않으면 정상 진입점이 아니며 SQL/Trino 오류로 판정하지 않는다. |
...
| Production legacy demo auth | backend startup + `scripts/verify-deploy-env.sh` | `enabled` | default production disables legacy identities; explicit demo deployment requires matching backend/frontend opt-in flags | maintainer | paired `true` opt-in은 이전 deploy에서 disabled된 demo 계정을 복구한다. 기본값 false, Bootstrap admin 요구와 production header-auth 차단은 유지한다. |
...
| ClickHouse Realtime Serving V2 전환 | backend startup/live probe, `/api/health/realtime`, deploy preflight 58 checks, `Realtime Quality Gates`의 54-test V2 release suite·Alembic 0016~0018 lifecycle·PostgreSQL fencing·frontend 144-check regression·Compose render | `enabled` | 기본 false/disabled, 모순된 owner와 receipt gap/parity mismatch/stale pointer/incomplete cutover gate는 fail closed; 동일 switch retry는 한 epoch/revision/event만 허용 | data-platform | PR01~09 누적 branch 구현 gate 상태다. 실제 10만 건·72시간·P95·chaos·security·browser rollback·HA/backup evidence와 별도 승인 전 production canonical 전환 금지. 단일 노드 demo/staging |
...
| ClickHouse Realtime V2 foundation | matching V2 backend/deploy paths | feature-flag and Alembic unit tests, local/prod Compose profile render, artifact provenance review | profile is absent by default; exact pins, five config fields, health 503 and 10-table migration remain consistent. Isolated TLS start/restart/RBAC passed; live connector, actual EC2 TLS/reboot/backup are not a PR02 PASS |
...
| Production legacy demo boundary | focused auth tests + deploy preflight | backend startup account/session state and production env | known demo identities stay disabled by default; mismatched or invalid opt-in flags fail before Compose mutation |
