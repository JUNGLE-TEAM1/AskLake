...
| EKS foundation contract check | `scripts/verify-eks-foundation.sh` | `manual` | block Pair A/B handoff when Terraform/Helm files, service accounts, namespace RBAC, Replay exclusion, Trino handoff, runtime boundary, or credential scan fails | Pair A | Terraform CLI가 없는 환경은 documented Docker validation을 추가 실행한다. 실제 AWS apply는 수행하지 않는다. |
...
| EKS data-plane safe defaults | Terraform mode checks and mock-provider tests | `manual` | keep MSK/RDS/S3 disabled by default; reject create for shared/external lifecycle, missing private network inputs, broad Kafka/S3 actions, or incomplete KMS reference | Pair A | [Phase 3 Data Plane 계약](eks-phase-3-data-plane.md) 검증은 AWS를 변경하지 않으며 실제 apply는 Phase 2 gate와 별도 승인이 필요하다. |
| EKS workload identity selection | Terraform mode checks, pure policy module and Helm/mock tests | `manual` | keep identity disabled until IRSA OIDC provider or Pod Identity Agent ownership is confirmed; keep resource keys plan-known and create four isolated roles only with exact MSK/S3 boundaries | Pair A | Frontend/Airflow와 외부 fixture producer를 EKS role에 섞지 않으며 IRSA/Pod Identity render를 분리한다. [Phase 4 identity 계약](eks-phase-4-identity-rds-bootstrap.md)을 따른다. |
...
| EKS ALB/private network decision gate | Terraform checks and `scripts/verify-eks-network-ingress.sh` | `manual` | render zero Ingress by default; reject partial controller/exposure/target/DNS/ACM inputs; keep NAT/endpoints and Pod enforcement undecided | Pair A | application SAs receive no Ingress mutation. Restrict ALB IngressGroup membership before apply. No controller, CNI, ALB, DNS, certificate or egress resource is created by Phase 7. |
| EKS runtime Secret delivery gate | canonical JSON, Terraform checks, runtime/combined verifiers | `manual` | reject Secret/env/file/shared-binding drift, incomplete delivery metadata and Phase 5/8 selection mismatch; keep full-service contract closed while Airflow/AI decisions are unresolved | Pair A/B | Secret values never enter Git/Terraform/output. Static AWS keys and application SA Secret read RBAC are forbidden. Actual sync, workload injection and rotation smoke remain environment work. |
...
| EKS foundation contract | no, local/manual until CI exists | `infra/eks/terraform`, `infra/eks/helm`, Phase 1 handoff | `scripts/verify-eks-foundation.sh` passes Terraform mock-provider tests, Helm schema/lint/render, backend/Spark least-privilege RBAC, Replay `create=false`, Trino handoff, runtime boundary and credential-pattern checks without AWS mutation |
...
| EKS network and ingress | no, local/manual until actual environment apply | Terraform Phase 7 handoff and `asklake-ingress` chart | default renders no Ingress; reviewed fixture renders exactly two HTTPS ALB rules with distinct FastAPI/Frontend health paths; Terraform planning tests reject partial choices |
| EKS runtime Secret contract | no, local/manual until delivery mechanism is selected | `infra/eks/secrets`, Terraform Phase 8 handoff and combined Phase 5/8 verifier | exact Secret names/keys, env injection, shared bindings and read-only mounts pass 20 negative/positive scenarios; default examples remain planning-only and selection mismatch blocks deploy readiness |
