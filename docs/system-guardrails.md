...
| EKS 15.5 Backend image handoff | Backend Catalog rows service and TestClient HTTP tests, Phase 6 formal image receipt, `scripts/preflight-eks-backend-image-rollout.sh`, provenance check and atomic Backend-only rollout | `preflight passed / rollout pending` | reject a receipt whose full revision does not contain the Iceberg rows fix, reject mutable/non-AMD64 image, values changes outside `backend.image`, dry-run mutation or Deployment/Pod digest mismatch, and roll back when two-replica health, ALB/RDS or Continuous ownership drifts; accept only an exact approved Backend Secret key set with full source/target hash equality; before Trino deployment require sanitized HTTP 502 `SQL_STORAGE_ERROR` without private endpoint/query/token leakage instead of `NameError`/generic 500 | Pair A/B | Issue #798 Phase 0 confirmed the healthy pre-fix baseline and rollback. Phase 1 accepted a distinct immutable AMD64 Backend digest whose exact receipt revision contains `f556e95e`. Phase 2 changed only a private candidate, passed server dry-run and proved zero cluster mutation while preserving a concurrent approved Secret expansion. EKS is still unchanged. Trino snapshot HTTP 200 remains separate. [Handoff](eks-day15-5-backend-image-handoff.md), [evidence](eks-15-5-runtime-remediation-evidence.md). |
