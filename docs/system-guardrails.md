...
| Metadata schema bootstrap | Helm `asklake-backend-migration`, `backend npm run migrate:metadata-schema`, shared schema-management guard, and EC2 deploy pre-bootstrap | `enabled` | block application startup/deploy when metadata preparation fails; reject PostgreSQL DDL outside the explicit runner; skip every helper when runtime schema management is disabled | data-platform | EKS runs Alembic and one metadata bootstrap runner before rollout. Auth, Audit, Permission, Governance, ETL, Catalog, SQL, Dashboard, Continuous, Realtime, and Semantic helpers are covered; SQLite/local compatibility remains. |
