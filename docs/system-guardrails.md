...
| Kafka deploy runtime dependency | manual deploy audit, future deploy dependency check | `enabled (manual)` | warn when Kafka source demos are claimed but no Kafka-compatible broker and seeded topic exist in deploy stack | maintainer | EC2 Compose declares Redpanda at `redpanda:9092`; seed script creates `reviews.raw`; EC2 source test, MinIO landing, Catalog registration, and `sourceKind: kafka` were verified. Scheduled trigger remains separate follow-up work. |
