...
| AWS staging Phase 0~6 contract | `.github/workflows/aws-staging-contract-checks.yml`, `cd backend && npm run verify:aws-staging-contract && npm run verify:aws-staging-terraform && npm run verify:aws-staging-runtime && npm run verify:aws-staging-workflows && npm run verify:aws-staging-smoke && npm run verify:aws-staging-lifecycle && npm run verify:aws-staging-handoff` | `partial` | fail when staging region/no-NAT endpoint/state/cost/cap, Terraform schema, Runtime mapping/redaction, manual OIDC trigger, reviewed plan fingerprint, protected mutation approval, private artifact cleanup, checksum activation, SSM runner bundle, exact count/lag/quarantine/checkpoint/Job identity, evidence-before-cleanup, TTL audit, or cross-evidence handoff/redaction boundary drifts | maintainer | AWS credential 없는 PR check, Terraform/mock plan, private Runtime 변환, smoke cleanup, scheduled TTL audit와 handoff 생성 코드는 동작한다. 실제 AWS workflow evidence와 handoff bundle은 후속이며 일반 deploy에는 연결하지 않는다 |
...
| AWS staging Runtime render failed | `terraform output -json`의 non-sensitive infrastructure contract와 sensitive broker wrapper, stack/account/region, application/role/bucket/capacity 일치를 확인한다. sensitive JSON을 log/`tee`로 출력하지 말고 생성 env는 `deploy/generated/`의 mode `0600`으로 유지한다. checksum JAR bundle 전에는 Continuous flag를 켜지 않는다. |
| AWS staging workflow contract failed | 네 mutation workflow가 `workflow_dispatch` 전용인지, OIDC job의 `id-token: write`/예상 account 제한, mutation별 보호 Environment와 정확한 `<operation>:<stackId>` 확인, 검토/re-plan fingerprint 동일성, 실제 EMR quota 조회, cleanup의 Budget/quota 독립성, private backend/tfvars/env/binary plan 제거, GitHub artifact redaction을 확인한다. smoke는 checksum bundle, private SSM, exact count/checkpoint/Job identity evidence를 우회할 수 없어야 한다. JAR POM 버전, `If-None-Match`, S3 SHA-256/size/metadata 검증을 완화하거나 일반 deploy trigger를 추가하지 않는다. PR contract check에는 AWS credential/id-token을 주지 않는다. |
