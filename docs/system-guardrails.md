...
| pair1 shared EKS live validation | exact receipt rollout, runtime identity guard, bounded E2E/evidence/retry verifiers | `bounded E2E and retry passed; recovery pending` | apply runtime ConfigMap before Web consumers, roll FastAPI/Collector on receipt revision, require active Pod process Spark image equality, fail on Helm/Deployment drift, and never reuse a batch rejected by provenance or shared-runtime gates | Pair A/B | exact 100 rows, one materialization and idempotent retry passed in a fixed window. A later unrelated Web upgrade changed the live Backend before Pod recovery verification, so that recovery gate remains pending and must not be forced over another active workload. [Evidence](eks-pair1-live-validation.md). |
