...
| EKS Trino MVP readiness | EKS deploy-time | 신규 단일 coordinator HTTPS `8443` Service, RDS `iceberg_catalog`, S3 warehouse, FastAPI TLS connection, Spark commit physical verification | `/v1/info` Ready → FastAPI query → Spark Iceberg commit/Trino verify/Catalog materialization이 순서대로 성공한다. 기존 EC2/공용 endpoint·static AWS key는 사용하지 않는다. |
