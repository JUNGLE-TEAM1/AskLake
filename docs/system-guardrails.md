...
| ETL CSS reviewed cascade | `frontend npm run test:css-catalog-boundary` | `enabled` | feature CSS hash, 정확한 selector inventory, block 완결성 또는 통합된 `.s3-tree-panel` declaration 순서가 drift하면 실패 | analytics-experience | 인접 중복 1개만 통합해 1,249 definitions/1,183 unique/중복 66개. 비인접 중복은 visual·computed-style evidence 없이 제거 금지 |
...
| ETL Job query ownership | `Refactor Quality Gates` + `tests.test_etl_job_queries` | `enabled` | Job refresh/hydrate/permission/facet 순서 또는 404/403 audit 계약이 바뀌면 실패 | data-platform | `etl_service.list_jobs/get_job`은 façade로 유지하고 application module이 조회 정책을 소유 |
...
| Session cookie transport | backend auth tests + workload Helm schema | `enabled` | production and HTTPS deployments keep `AUTH_SESSION_COOKIE_SECURE=true`; only an explicitly approved HTTP dev ALB may set it to `false` | maintainer | HTTP dev exception keeps `HttpOnly`, `SameSite=Lax`, production auth mode, header-auth blocking, and public signup blocking. Restore `true` when HTTPS is available. |
