...
| EKS foundation contract check | `scripts/verify-eks-foundation.sh` | `manual` | block Pair A/B handoff when EKS Auto Mode/IAM/access-entry contract, Terraform/Helm files, service accounts, namespace RBAC, Replay exclusion, Trino handoff, runtime boundary, or credential scan fails | Pair A | 일반 계약은 existing cluster의 외부 Auto Mode 확인과 신규 cluster의 명시적 admin principal을 요구하며, Terraform CLI가 없으면 documented Docker validation을 추가 실행한다. 검증 명령 자체는 AWS를 변경하지 않는다. dev에는 별도 승인된 Terraform/Helm apply로 foundation revision 2, Pod Identity/RBAC와 Spark Operator 2.5.1이 적용됐으며 실제 workload/Secret/Ingress rollout은 별도 gate다. |
...
| EKS workload identity selection | Terraform mode checks, pure policy module and Helm/mock tests | `manual` | keep identity disabled until IRSA OIDC provider or Pod Identity Agent ownership is confirmed; keep resource keys plan-known and create four isolated roles only with exact MSK/S3 boundaries | Pair A | dev는 EKS Pod Identity를 선택해 Backend/MSK smoke/Spark/Trino association을 분리했다. Frontend/Airflow와 외부 fixture producer를 EKS role에 섞지 않으며 일반 계약은 IRSA/Pod Identity render를 계속 분리한다. [Phase 4 identity 계약](eks-phase-4-identity-rds-bootstrap.md)을 따른다. |
