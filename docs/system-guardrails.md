...
| EKS 15.5 Backend image handoff | Backend Catalog rows service and TestClient HTTP tests, Phase 6 formal image receipt, provenance check and atomic Backend-only rollout | `baseline verified / rollout pending` | reject a receipt whose full revision does not contain the Iceberg rows fix, reject mutable/non-AMD64 image or Deployment/Pod digest mismatch, and roll back when two-replica health, ALB/RDS or Continuous ownership drifts; before Trino deployment require sanitized HTTP 502 `SQL_STORAGE_ERROR` without private endpoint/query/token leakage instead of `NameError`/generic 500 | Pair A/B | Issue #798 Phase 0 confirmed the current pre-fix image, healthy runtime and rollback revision/digest without mutation. A new image must come from the workload image owner; Trino snapshot HTTP 200 remains separate. [Handoff](eks-day15-5-backend-image-handoff.md), [evidence](eks-15-5-runtime-remediation-evidence.md). |
