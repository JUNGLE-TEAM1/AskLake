...
| EKS application workload | yes on matching PR paths | web-only and full Helm schema, staged one-replica rollout, same-origin empty CORS or explicit HTTPS cross-origin, digest image, AMD64 scheduling, ClusterIP, DB-aware readiness/TCP liveness, conditional ConfigMap/Secret refs, RBAC, Kubernetes Spark recovery, MSK IAM, EC2-owned Continuous boundary | `scripts/verify-eks-workloads.sh`, Node provider tests, focused FastAPI tests and four B-owned AMD64 image builds pass; disabled Airflow/Trino render no resource or Secret refs, and chart contains no Secret, static AWS key, replay producer, LoadBalancer, StatefulSet, or mutable image tag |
