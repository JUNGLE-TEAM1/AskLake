...
| EKS Auto Mode ALB decision/apply gate | Terraform checks, `scripts/verify-eks-network-ingress.sh` and confirmation-gated deploy/destroy scripts | `manual` | render zero resources by default; reject partial exposure/target/address/listener/subnet inputs, reject HTTP with DNS/ACM values and HTTPS without them, and require server-side dry-run before class or paid ALB creation | Pair A | dev HTTP Ingress와 internet-facing ALB를 적용해 `/`와 `/api/health` smoke를 통과했다. Application SAs receive no Ingress mutation. DNS/ACM/HTTPS는 후속이며 Ingress/finalizer cleanup must precede cluster destroy. |
| EKS Phase 14 web workload gate | `scripts/verify-eks-web-workloads.sh` and receipt/context-gated deploy/destroy scripts | `manual` | render zero resources by default; require immutable frontend/backend digests, two replicas, explicit resources, runtime ConfigMap/Secret, General placement and B runtime-boundary approval | Pair A/B | dev `asklake-web`은 Backend scheduler 수정 image까지 revision 2로 배포됐고 두 replica/RDS health, restart/Continuous isolation과 외부 ALB smoke를 확인했다. S3 positive smoke는 별도 gate다. |
| EKS Spark Operator install gate | `scripts/verify-eks-spark-operator.sh`, `scripts/verify-eks-spark-rbac.sh` and confirmation-gated deploy/destroy scripts | `manual` | verify official chart archive SHA-256 before local render/install; pin image digests; watch only `asklake-dev`; require admission fixture server dry-run and explicit RBAC allow/deny matrix without creating a workload; uninstall preserves CRDs unless a separate ownership-and-empty confirmation is supplied | Pair A | dev 2.5.1 revision 2 is deployed. One General node remains required while controller/webhook run. All three workload kinds must be globally empty before uninstall or CRD deletion. [Evidence](eks-day15-spark-operator-evidence.md). |
...
| EKS runtime Secret delivery gate | canonical JSON, Terraform checks, Helm render and runtime/combined verifiers | `manual` | reject Secret/env/file/shared-binding drift, incomplete delivery metadata and Phase 5/8 selection mismatch; scope ESO to `asklake-dev`; allow its Pod Identity only three read actions on `asklake/dev/*`; keep full-service contract closed while a live mapping is incomplete | Pair A/B | Secret values never enter Git/Terraform/output. Static AWS keys, cluster-wide store/PushSecret and application SA Secret read RBAC are forbidden. 네 workload source/target과 공유 binding은 dev에서 검증했지만 live Backend ExternalSecret의 Trino mapping은 적용 전이다. Issue #828의 임시 Trino-only target은 정식 mapping 뒤 제거한다. |
...
| EKS Auto Mode ALB ingress | foundation applied; routes pending | Phase 13 handoff, IngressClassParams/Class and two optional Ingress rules | dev class/params each exist once with internet-facing/ip/IPv4/HTTP contract; no Ingress or ALB exists until final FastAPI/Frontend Services are ready; route fixture keeps distinct health paths and apply/destroy require exact confirmations |
...
| EKS Spark Operator | dev foundation applied; application pending | verified Kubeflow 2.5.1 chart checksum/image digests, owned `v1beta2` CRDs, controller/webhook and namespace-scoped admission | revision 3 controller/webhook Ready, CRD Established, repo fixture server dry-run and RBAC allow/deny matrix pass, all three workload kinds remain zero; General node cost continues while operator runs |
