...
| Runtime promotion contract failed | approved Phase 7 원본과 SHA, Phase 8 policy/evidence/report, baseline `spark-rest + redpanda`, 분리된 consumer group/output/checkpoint, Shadow 정합성/checksum, 관측 임계치, rollback owner/runbook을 확인한다. 배포 실패 시 보고서를 우회하지 말고 기본 Runtime으로 롤백한다. |
...
| Prod compose config failed | `deploy/.env.example`의 필수 env key, `deploy/docker-compose.prod.yml`, 기본 `ASKLAKE_SPARK_RUNTIME=spark-rest` 또는 승인된 `emr-serverless`/`msk` opt-in, Phase 8/Phase 7 리포트 절대 경로, Dockerfile path를 확인한다. |
