...
| Production legacy demo auth | backend startup + `scripts/verify-deploy-env.sh` | `enabled` | legacy demo users are disabled by default and the frontend credential switch is not supported | maintainer | backend test 환경의 `AUTH_LEGACY_DEMO_USERS_ENABLED`만 저장된 status/session 호환에 사용한다. frontend는 demo credential을 주입하지 않으며 Bootstrap admin과 production header-auth 차단을 유지한다. |
