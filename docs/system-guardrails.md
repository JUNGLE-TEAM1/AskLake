...
| EKS Spark Operator install gate | `scripts/verify-eks-spark-operator.sh` and confirmation-gated deploy/destroy scripts | `manual` | pin official Kubeflow chart and image digests; watch only `asklake-dev`; reuse Foundation Spark SA/RBAC; require CRD/webhook/controller readiness and B manifest server dry-run without submitting an application | Pair A | dev 2.5.1 revision 2 is deployed. One General node remains required while controller/webhook run. Empty application check precedes release/CRD deletion. [Evidence](eks-day15-spark-operator-evidence.md). |
...
| EKS Spark Operator | dev foundation applied; application pending | official Kubeflow 2.5.1 chart/image digest, `v1beta2` CRDs, controller/webhook and namespace-scoped admission | controller/webhook Ready, CRD Established, B SparkApplication server dry-run passes and actual application count remains zero; General node cost continues while operator runs |
