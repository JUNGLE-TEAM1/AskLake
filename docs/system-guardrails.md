...
| --- | --- | --- | --- | --- | --- |
| Frontend UI checks before merge | GitHub Actions workflow running `cd frontend && npm run verify:ui-regressions && npm run build` | `enabled` | block merge when required check is enabled and the workflow fails | maintainer | PR에서 compact workspace header, SQL action/section marker와 방향성 panel toggle, Catalog PROCESS projection, Dashboard panel toggle·값 축 범위, Vite build를 함께 확인 |
...
| Dashboard schema preparation | explicit metadata migration runner plus `backend npm run migrate:dashboard-schema` and `npm run verify:dashboard-storage` | `enabled` | versioned Dashboard schema preparation fails before Dashboard API requests are served; request paths must not issue Dashboard DDL | data-platform | `dashboard_schema_migrations` records Dashboard card/runtime and batch-result-cache versions; a PostgreSQL advisory lock serializes migration execution. EKS API/worker startup does not own this DDL. |
| Metadata schema bootstrap | Helm `asklake-backend-migration`, `backend npm run migrate:metadata-schema`, shared schema-management guard, and EC2 deploy pre-bootstrap | `enabled` | block application startup/deploy when metadata preparation fails; reject PostgreSQL DDL outside the explicit runner; skip every helper when runtime schema management is disabled | data-platform | EKS runs the single metadata compatibility bootstrap before existing Alembic revisions because some revisions reference bootstrap-owned parent tables. Auth, Audit, Permission, Governance, ETL, Catalog, SQL, Dashboard, Continuous, Realtime, and Semantic helpers are covered; SQLite/local compatibility remains. |
| Spark external-wait DB connection release | `airflow_execution` prepare/run/finalize boundary plus `test_parallel_spark_waits_leave_pool_available_for_status_polling` | `enabled` | fail regression if an external Spark wait receives a DB Session/ORM row, keeps a pool connection checked out, loses attempt fencing, or blocks parallel status reads | data-platform | DB-dependent input is detached and committed before Spark wait; finalization reopens a short transaction and verifies the same `attemptId`. Live RDS metrics remain a post-deploy check. |
...
| ETL Job delete transaction | `Refactor Quality Gates` + `tests.test_etl_job_commands` + `tests.test_etl_job_delete` | `enabled` | permission보다 workload identity를 먼저 조회하거나 active workload 차단, 종속 운영 레코드 삭제 순서, 연결 Dataset 보존·자동 갱신 종료 기록, audit·commit/rollback이 바뀌면 실패 | data-platform | `etl_service.delete_job`은 façade로 유지하고 application command가 transaction을 소유 |
...
| EKS·EC2 Continuous control-plane owner | `Refactor Quality Gates / structural-ratchet` + `deploy/control-plane-ownership.json` + `scripts/refactor_audit/control_plane_ownership.py` + EC2 Compose preflight | `enabled` | required control plane의 active owner 누락·중복, inactive/unknown claim, repository evidence drift 또는 runtime env 전달 누락 시 실패 | data-platform | 현재 EKS Realtime V1 worker가 Kafka Continuous·Continuous SQL reconciliation을 claim하고 EC2 worker는 rollback standby다. EC2 Compose는 control-plane mode, Spark execution lease, worker scope/owner/generation을 명시적으로 전달한다. EC2 프로필로 owner를 옮길 때는 EKS fence와 manifest/evidence를 같은 변경에 포함한다. |
...
| EKS Spark gateway | `ASKLAKE_SPARK_RUNNER=kubernetes`, `ASKLAKE_CONTINUOUS_SPARK_RUNNER=kubernetes` + Spark Operator RBAC | required when enabling EKS batch/Continuous | mutable image tag, missing inline/runtime document manifest, UID 185 report 경로 쓰기 실패, SparkApplication CRUD 누락, 또는 SparkApplication에 JDBC secret 평문 삽입 금지 | data-platform | image digest, worker `sparkapplications` CRUD, Driver report write, driver/executor S3 access와 JDBC `secretKeyRef` 단일 주입을 rollout 전 확인 |
...
| Continuous SQL contract | `backend npm run verify:continuous-sql-contract`, execution-tree migration/unit suite, ClickHouse unit/E2E, `npm run verify:realtime-stack`, `Realtime Quality Gates` | `enabled` | CTAS 전 durable reservation 누락, 자동·수동 실행 간 unfinalized fence 누락, terminal-but-unfinalized 상태의 중복 refresh, stale claim/Run/revision publication, malformed Kafka parser contract, stale generation/fence, invalid lifecycle/publication identity면 matching PR check 실패 | maintainer | Dataset-revision Trino refresh는 CTAS 전에 Run reservation을 commit하고 terminal payload만으로 claim을 풀지 않는다. 제출 결과가 불명확하면 fail closed한다. matching Catalog·Run finalized·revision cursor가 한 transaction으로 commit된 뒤 다음 worker cycle에서만 후속 revision을 시작하며 실패 시 마지막 published Gold를 유지한다. EKS 적용 완료에는 `trino-result-collector`와 `realtime-v1-worker`가 같은 승인 Backend digest라는 증거가 필요하다. Dashboard는 수동 Widget query만 사용한다. 실제 Kafka/Spark/Iceberg fault·restart는 scheduled/manual tier |
| Realtime architecture regression | `backend/scripts/verify-realtime-quality-gates.py` + frontend `npm run test:dashboard-realtime-v2` | `enabled` | silent polling interval, direct in-memory event publish, SQL matrix·frontend cache/route/compatibility 계약 유실, God-file 신규 결합을 실패 처리 | maintainer | allowlist 없는 `refetchInterval`/`setInterval` 추가 금지. Workflow와 복구 runbook은 현재 package script 이름을 사용한다. |
...
| Failure | How to fix |
| --- | --- |
| `npm run verify:ui-regressions` failed | 주요 workspace의 compact header, SQL action/탭의 텍스트 label, SQL section marker·좌우 panel 화살표, editor 불변 높이·Nessie Popover/Bubble/Collapsible·Dashboard WidgetConfigPanel 재사용·값 축 범위·차트/데이터/실행 정보 전환·Trino timeline/cursor pagination/server CSV·Job wizard, Source/Catalog 밀도, Catalog PROCESS projection, Dashboard panel toggle, Dashboard 목록, ApexCharts 위젯의 최근 회귀 방지 계약을 확인하고 관련 파일을 수정한다. |
...
- FastAPI와 collector의 `STARTUP_SCHEMA_MANAGEMENT_ENABLED`는 production EKS에서
  ...
  소유하며 no-op `ALTER COLUMN TYPE`을 실행하지 않는다. 공통 schema-management guard는
  PostgreSQL의 명시적 migration 문맥 밖 DDL을 거절한다.
- Airflow Spark 실행은 DB-dependent 준비 transaction을 외부 실행 전에 끝낸다. 외부
  SparkApplication 대기 중 checked-out connection은 0이어야 하며, 완료 후 새
  transaction에서 동일 `sparkExecution.attemptId`를 확인한 뒤 결과를 저장한다.
