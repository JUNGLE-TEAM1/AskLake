...
| AWS staging Phase 0~3 contract | `.github/workflows/aws-staging-contract-checks.yml`, `cd backend && npm run verify:aws-staging-contract && npm run verify:aws-staging-terraform && npm run verify:aws-staging-runtime && npm run verify:aws-staging-workflows` | `partial` | fail when staging region/no-NAT endpoint/state/cost/cap, Terraform schema, Runtime mapping/redaction, manual-only OIDC trigger, reviewed plan fingerprint, protected mutation approval, actual quota gate, cleanup independence, private artifact cleanup, or conditional/checksummed JAR activation boundary drifts | maintainer | AWS credential 없는 PR check, versioned contract, Terraform/mock plan, private Runtime 변환과 수동 plan/apply/artifact/destroy workflow는 동작한다. 실제 AWS workflow 실행, S3/MSK/EMR smoke와 TTL sweep은 후속이며 일반 deploy에는 연결하지 않는다 |
...
| AWS staging workflow contract failed | 세 mutation workflow가 `workflow_dispatch` 전용인지, OIDC job의 `id-token: write`/예상 account 제한, mutation별 보호 Environment와 정확한 `<operation>:<stackId>` 확인, 검토/re-plan fingerprint 동일성, 실제 EMR quota 조회, cleanup의 Budget/quota 독립성, private backend/tfvars/env/binary plan 제거, GitHub artifact redaction을 확인한다. JAR POM 버전, `If-None-Match`, S3 SHA-256/size/metadata 검증을 완화하거나 일반 deploy trigger를 추가하지 않는다. PR contract check에는 AWS credential/id-token을 주지 않는다. |
