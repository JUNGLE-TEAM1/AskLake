...
| EKS Auto Mode ALB decision/apply gate | Terraform checks, `scripts/verify-eks-network-ingress.sh`, `scripts/verify-eks-day15-alb-runtime.sh` and confirmation-gated deploy/destroy scripts | `manual` | render zero resources by default; reject partial exposure/target/address/listener/subnet inputs, reject HTTP with DNS/ACM values and HTTPS without them, preserve Helm field ownership in server dry-run, and require shared-ALB target/HTTP/RDS health after paid ALB creation | Pair A | dev HTTP class/params and two Ingress share one active ALB; `/`, `/api/health`, group별 healthy target 2개와 RDS health가 통과했다. Application SAs receive no Ingress mutation; Ingress/finalizer cleanup must precede cluster destroy. [Evidence](eks-day15-alb-runtime-evidence.md). |
...
| EKS Day 15 Backend runtime Secret | `infra/eks/secrets/backend-runtime-external-secret.yaml` and `scripts/verify-eks-day15-backend-secret-runtime.sh` | `manual` | require exact two-key source/target mapping and equal value hash, ESO target ownership, FastAPI 2/2, active ALB routes and RDS-aware health without printing values or identifiers | Pair A | never delete the manual target until a staged ESO target matches; credential value rotation is separate from refresh/rollout wiring smoke. |
| EKS Day 15 Backend S3 boundary | bucket-isolated Terraform statements and `scripts/run-eks-backend-s3-smoke.sh` | `manual` | require one Backend association/policy, immutable current image, allowed result object round trip, denied read-only write, denied existing out-of-prefix object read/list and denied bucket metadata; always delete sentinel/object/Pod/ConfigMap | Pair A | never combine bucket-wide `*` and restricted prefixes in one multi-bucket ListBucket statement. Backend evidence does not approve Spark/Trino policies. [Evidence](eks-day15-backend-s3-runtime-evidence.md). |
