...
| EKS Day 15 Backend runtime Secret | `infra/eks/secrets/backend-runtime-external-secret.yaml`, `scripts/migrate-eks-backend-runtime-secret.sh`, `scripts/verify-eks-day15-backend-secret-runtime.sh` and the approved runtime contract | `manual` | require an exact approved two-key web baseline or five-key integrated-runtime source/target mapping and equal full value hash, staged ESO match before an explicitly confirmed handover, idempotent verification of an existing Ready owner, and fail-closed manual restoration; rollback must verify recreated key/hash/owner, FastAPI 2/2, steady ALB and RDS-aware health and must surface cleanup/delete/apply/rollout failure | Pair A | never delete the manual target until a staged ESO target matches; do not treat a partial key superset as valid. Credential value rotation is separate from refresh/rollout wiring smoke. |
...
| EKS 15.5 Backend image handoff | Backend Catalog rows service and TestClient HTTP tests, Phase 6 formal image receipt, `scripts/preflight-eks-backend-image-rollout.sh`, `scripts/rollout-eks-backend-image.sh`, runtime error verifier and atomic Backend-only rollout | `runtime HTTP 502 verified / Trino HTTP 200 pending` | reject a receipt whose full revision does not contain the Iceberg rows fix, reject mutable/non-AMD64 image, values changes outside `backend.image`, dry-run mutation or Deployment/Pod digest mismatch, and roll back when two-replica health, ALB/RDS or Continuous ownership drifts; require the EKS Auto Mode readiness-gate namespace key and a healthy target condition on every new Backend Pod; accept only an exact approved Backend Secret key set with full source/target hash equality; before Trino deployment require sanitized HTTP 502 `SQL_STORAGE_ERROR` with normalized reason and without enum implementation/private endpoint/query/token leakage | Pair A/B | Phase 3 exposed rolling-update 502 and corrected the managed webhook key to `eks.amazonaws.com/pod-readiness-gate-inject`. Phase 4 replacement rollout passed two Pod target-health gates, 343 final health samples with zero failure, exact digest and unchanged Frontend/Secret. Live rows returns HTTP 502 `SQL_STORAGE_ERROR` with normalized `BACKEND_TIMEOUT` and no private marker. Trino snapshot HTTP 200 remains separate. [Handoff](eks-day15-5-backend-image-handoff.md), [evidence](eks-15-5-runtime-remediation-evidence.md). |
...
| EKS Day 16 A private runtime input | `scripts/prepare-eks-day16-runtime-secret-input.sh`, `scripts/verify-eks-day16-runtime-secret-input.mjs` and Git ignore review | `private input ready` | require mode 0600 and untracked input; reuse the isolated Iceberg credential; enforce exact Backend patch/Spark/Trino keys, shared JDBC values, distinct Trino identities/signing secrets, bcrypt cost, JKS readability, matching CA fingerprint and Service SAN; never print or commit values | Pair A | Existing Airflow and AWS/Kubernetes sources remain unchanged in Phase 1. Base64 JKS/password DB must be decoded only by their Phase 2 ExternalSecret properties. [Evidence](eks-day16-a-runtime-secret-input.md). |
| EKS Day 16 A runtime Secret delivery | `scripts/deploy-eks-day16-runtime-secrets.sh`, `scripts/verify-eks-day16-runtime-secret-delivery.sh`, exact manifests and private input | `Spark/Trino source-target Ready` | preserve Backend/Airflow; require server dry-run, absent-before-create gate, staged source-target hash, Base64 decode only for Trino files, exact final owner/Ready/hash and application SA Secret read deny; clean this run's source/target on failure | Pair A | Spark 3-key and Trino 7-key delivery passed with stage residue 0 and Web/Airflow restart 0. Backend Trino client patch waits for file-mount workload integration. [Evidence](eks-day16-a-runtime-secret-delivery.md). |
| EKS Day 16 A Trino data plane | private values verifier and `scripts/run-eks-day16-trino-data-plane-smoke.sh` | `live boundary passed` | require one Trino Pod Identity association/policy, no inline/wildcard action, immutable receipt, exact RDS role/database, Warehouse/Query Result positive and outside-prefix negative smoke, DNS, Secret read deny and fail-closed version/Kubernetes residue cleanup | Pair A | Trino resources only passed server dry-run; live identity Job passed STS/RDS/S3/DNS with residue 0. Coordinator startup, final Service TLS and Backend client patch remain integration gates. [Evidence](eks-day16-a-trino-data-plane.md). |
| EKS Day 16 A handoff audit | `scripts/prepare-eks-day16-a-handoff.sh`, `scripts/verify-eks-day16-a-handoff.sh` and private contracts | `integration blocked` | require latest pair1 ancestry, actual Terraform/image equality, planning contract, full lint/render, Trino-only server dry-run and zero mutation; do not mark ready while fixture receipt, release ownership, Backend full Secret/CA mount or full-service decisions are missing | Pair A/B | Four blocker classes remain; the verifier reproduces ownership failure without applying resources. [Evidence](eks-day16-a-handoff.md). |
