...
| EKS 15.5 Backend image handoff | Backend Catalog rows focused test, Phase 6 formal image receipt, provenance check and atomic Backend-only rollout | `source ready / runtime pending` | reject a receipt whose full revision does not contain the Iceberg rows error fix, reject mutable/non-AMD64 image or Deployment/Pod digest mismatch, and roll back when two-replica health, ALB/RDS or Continuous ownership drifts; before Trino deployment require sanitized HTTP 502 `SQL_STORAGE_ERROR` instead of `NameError`/generic 500 | Pair A/B | Current deployed Backend image predates the source fix. A new image must come from the workload image owner; Trino snapshot HTTP 200 remains a separate gate. [Handoff](eks-day15-5-backend-image-handoff.md). |
