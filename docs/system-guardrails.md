...
| EKS foundation contract check | `scripts/verify-eks-foundation.sh` | `manual` | block Pair A/B handoff when Terraform/Helm files, service accounts, runtime boundary, or credential scan fails | Pair A | Terraform CLI가 없는 환경은 documented Docker validation을 추가 실행한다. 실제 AWS apply는 수행하지 않는다. |
...
| EKS foundation contract | no, local/manual until CI exists | `infra/eks/terraform`, `infra/eks/helm`, Phase 1 handoff | `scripts/verify-eks-foundation.sh` passes Terraform mock-provider tests, Helm schema/lint/render, runtime boundary and credential-pattern checks without AWS mutation |
