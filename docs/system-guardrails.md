...
| EKS 15.5 Catalog physical read | `scripts/run-eks-catalog-physical-read-smoke.sh`, `scripts/test-eks-catalog-physical-read-smoke.sh` and private Git-ignored inputs | `static ready / live rerun pending` | reject tracked/unignored inputs, non-AMD64 or mutable receipt, object outside materialization root, missing confirmation/context, empty/malformed result, leaked private marker, cleanup failure or any exact-run label/name residue; never print URI or rows | Pair A/B | The earlier representative live read remains evidence. The reusable runner passed validate-only and mocked success/failure/timeout/signal/residue scenarios; a new live run belongs to the next gated phase. [Receipt](eks-day15-data-copy-receipt.md). |
