...
| Kafka deploy runtime dependency | manual deploy audit, future deploy dependency check | `partial` | warn when Kafka source demos are claimed but no Kafka-compatible broker and seeded topic exist in deploy stack | maintainer | EC2 Compose declares Redpanda at `redpanda:9092`; `scripts/seed-kafka-demo-data.sh` seeds `reviews.raw`; UI/job end-to-end ingest verification remains follow-up work |
