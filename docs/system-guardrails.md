...
| EKS runtime Secret delivery gate | Terraform checks and `scripts/verify-eks-runtime-secrets.sh` | `manual` | reject Secret name/key/shared-binding drift and block sync while delivery mode, controller/source or rotation owner is incomplete | Pair A/B | Secret values never enter Git/Terraform/output. Static AWS keys and application SA Secret read RBAC are forbidden. Actual controller/workflow selection and runtime rotation smoke remain environment work. |
...
| EKS runtime Secret contract | no, local/manual until delivery mechanism is selected | `infra/eks/secrets`, Terraform Phase 8 handoff | exact workload Secret names/keys, shared token/JDBC bindings and read-only mounts pass planning validation; default example fails deploy-ready validation and contains no values/static AWS credentials |
