...
- 2026-07-21 기준표는 이미 `dev`에 병합된 구조와 일치하도록 `1f695b73`에서 다시 동기화했다. 이 갱신은 최신 `dev`의 기존 구조 부채만 반영하며, #1114가 수정한 대형 파일은 최신 `dev` 크기 이하로 분리해 새 증가를 허용하지 않는다. 신규 파일·함수는 예외 없이 1,000줄/100줄 제한을 지키고, 현재 크기를 넘는 추가 증가는 계속 차단하며, 기준 갱신을 기능 PR의 검사 우회 수단으로 사용하지 않는다.
