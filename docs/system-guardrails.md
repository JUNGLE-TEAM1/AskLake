...
| EKS Phase 12 NodePool selection gate | `scripts/verify-eks-auto-mode-node-pools.sh`, Terraform checks and Helm schema | `day17 baseline captured` | render zero custom resources by default; reject missing IAM access ownership, subnet/security-group selectors, capacity/instance limits or disruption choices; block load attribution while unrelated temporary Job, active SparkApplication, terminating/Pending workload, ALB drain or release generation drift remains | Pair A/B | Test fixture values are not deployment recommendations. dev custom pools are Ready and Day 14 synthetic General scale-out/in passed, but actual FastAPI/Spark load, negative taint, limit/cost and final scale-in evidence remain. Live HPA source must be reconciled with pair1 before approval. [Day 17 A baseline](eks-day17-a-autoscaling-baseline.md). |
