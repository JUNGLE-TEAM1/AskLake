...
| Prod compose config check | `Realtime Quality Gates / realtime-contracts`, `docker compose --env-file deploy/.env.example -f deploy/docker-compose.prod.yml config --quiet`, `backend/scripts/verify-production-spark-contract.mjs` | `enabled` | matching realtime PR에서 config render 실패, 배포 전 Spark/UID 계약 실패 시 중단 | maintainer | GitHub required check 등록은 repo admin 확인 필요 |
