...
| EKS Spark Operator install gate | `scripts/verify-eks-spark-operator.sh`, `scripts/verify-eks-spark-rbac.sh` and confirmation-gated deploy/destroy scripts | `manual / revision 3 verifier sync pending` | verify official chart archive SHA-256 before local render/install; pin image digests; watch only `asklake-dev`; require admission fixture server dry-run and explicit RBAC allow/deny matrix without creating a workload; uninstall preserves CRDs unless a separate ownership-and-empty confirmation is supplied | Pair A | dev 2.5.1 revision 3 is deployed. Representative Spark physical read passed, but the repository RBAC verifier still needs the full deletecollection/PVC cleanup matrix. All three workload kinds must be globally empty before uninstall or CRD deletion. [Evidence](eks-day15-spark-operator-evidence.md). |
