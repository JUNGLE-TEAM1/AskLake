...
| EKS workload identity selection | Terraform mode checks and mock-provider tests | `manual` | keep identity disabled until IRSA OIDC provider or Pod Identity Agent ownership is confirmed; create four isolated workload roles only with exact MSK/S3 boundaries | Pair A | Frontend/Airflow와 외부 fixture producer를 EKS role에 섞지 않으며 [Phase 4 identity 계약](eks-phase-4-identity-rds-bootstrap.md)을 따른다. |
| EKS RDS logical bootstrap | confirmation-gated script and SQL contract | `manual` | fail before DB mutation when admin/three role passwords, postgres maintenance DB, confirmation, or psql is missing | Pair A | bootstrap은 3개 database/user 분리만 수행하며 schema migration, EC2 데이터 이전과 rollback 승인은 별도다. |
