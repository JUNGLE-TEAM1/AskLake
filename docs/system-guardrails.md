...
| EMR Continuous contract | `cd backend && npm run verify:emr-serverless-continuous-contract` | `partial` | fail when STREAMING mode, retry threshold, MSK IAM manifest, S3 report/ack, restart reconnect, pause/resume, or secret guard drifts | maintainer | fake EMR/S3 verifier는 동작한다. 실제 VPC/MSK/EMR 부하 검증과 CI required check 연결은 후속 |
...
| EMR Continuous contract failed | Continuous feature flag, EMR 7.1+, MSK Runtime/IAM bootstrap/VPC reachability, streaming entry/helper artifact, execution role의 S3·Kafka 권한, S3 checkpoint/report, `maxFailedAttemptsPerHour` 1~10을 확인한다. Streaming 제출에 Batch timeout이나 static AWS key를 추가하지 않는다. |
