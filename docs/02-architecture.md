...
- 독립 `AI 활용` 메뉴와 `/ai` route는 제거했다. SQL Query AI, Dashboard Assistant, ETL 변환 AI, Semantic RAG, 리뷰 분석은 각각 사용자가 작업 중인 화면 안에서 private AI Gateway를 공유하며, 진입점 계약은 [AI 진입점 계약](ai-chat-ui-contract.md)을 따른다.
- 수집/처리 Transform 화면의 field transform과 SQL transform 보조는 `POST /api/ai/generate-sql`을 사용한다. Gateway 응답은 입력 schema 범위와 read-only/scalar Spark·Trino SQL 안전 검사를 통과해야만 편집기에 적용할 수 있다.
