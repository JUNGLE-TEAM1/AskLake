...
- Phase 1 foundation은 `infra/eks/terraform`의 existing/create cluster 입력, optional managed node group과 ECR output, `infra/eks/helm/asklake-foundation`의 namespace/service account 계약으로 구성한다. workload IAM action과 network destination은 Pair B가 [Phase 1 인수 계약](eks-msk-mvp-phase-1-handoff.md)에 따라 반환하기 전까지 추정해 부여하지 않는다.
