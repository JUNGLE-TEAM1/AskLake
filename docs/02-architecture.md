...
ClickHouse Dataset의 집계 widget은 `FINAL` 뒤에 안정적인 source alias를 붙이고 dimension/value 식을 그 alias로 한정한다. 특히 `date_trunc(...) AS event_time`처럼 output alias가 원본 column과 같은 시간축에서도 SELECT와 GROUP BY가 원본 column을 혼동하지 않도록 한다.
...
ClickHouse Kafka table은 source payload 형식을 추정하지 않고 메시지 전체를 `RawBLOB` 한 열로 소비한다. ingest materialized view가 Catalog에 저장된 `recordParsing`과 `schemaColumns`를 적용해 공백 원문 또는 nested JSON을 typed raw table로 투영한다. 정적 relation은 SQL이 참조한 열만 exact snapshot에서 page 단위로 적재한다. 검증 identity는 Dataset, snapshot, schema fingerprint, query-engine mapping, 참조 열, compiled JOIN key를 모두 포함하며 `asklake_static_cache_registry`가 검증 완료 table과 row count를 기록한다. 같은 identity는 pause/resume뿐 아니라 다른 Continuous SQL Job에서도 local table을 재사용하고 Trino 재조회와 중복 적재를 건너뛴다. 첫 적재는 ClickHouse disk reserve를 선검사하고, 정렬된 JOIN key의 `GROUP BY ... HAVING count() > 1`을 `optimize_aggregation_in_order`, query memory/thread 상한으로 실행해 null·빈 값·exact duplicate를 검사한다. 검증 registry가 없는 cache만 이 scan을 수행하므로 대형 dimension에 메모리 비례 `uniqExact`를 매 시작마다 반복하지 않는다. worker readiness는 table 존재뿐 아니라 `system.kafka_consumers`의 active consumer와 복구되지 않은 parser exception까지 확인한다.
