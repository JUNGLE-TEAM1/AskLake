...
- 일반 Snapshot Spark runtime은 확정 schema projection과 지원되는 row-preserving transform 선두 prefix를 run 전용 Parquet materialization 경로에 먼저 기록한다. 이 write가 raw source 전체를 한 번 읽는 경계이며, schema count/required-null, 나머지 Rule/Quality, sample과 target publish는 원본 lineage가 아니라 새로 읽은 staged DataFrame을 기준으로 실행한다. `ASKLAKE_SPARK_STAGED_CACHE_MAX_BYTES`가 양수이고 모든 staged file의 정확한 합계가 이 값 이하일 때만 staged DataFrame을 `MEMORY_AND_DISK`로 persist한다. 한도가 0이거나 크기를 정확히 읽지 못했거나 한도를 넘으면 staging-only를 유지한다. cache 준비 action이 실패하면 cache를 해제하고 같은 run staging을 새 DataFrame으로 다시 읽으며 raw source로 돌아가지 않는다. 같은 Spark type의 `String`/`Long`/`Boolean` identity cast·copy·rename 또는 직접 컬럼 복사와 `TRIM(CAST(<input> AS STRING))`으로만 구성된 canonical 선두 prefix는 staging 전에 적용하고 `preMaterializedTransformCount`로 남긴다. 품질 실패 전에는 Iceberg current snapshot이나 Catalog를 변경하지 않으며, 성공·품질 실패·schema 예외 경로는 run 전용 materialization 경로와 cache를 정리한다.
- Spark manifest는 `phaseTimings`로 Source 검증, Parquet materialization, Rule 평가, Quality 집계, source 사후 검증, Target publish 시간을 분리한다. `sparkResources`에는 driver/executor core, executor 수, default parallelism, shuffle partition과 함께 실제 `cacheStorageLevel`, `materializationMode=run_scoped_parquet_staging`, staged file·byte 수와 크기 판정 상태, cleanup 상태, cache 한도·선택 이유·fallback 횟수와 실제 `outputFrameCacheMode`를 남긴다. Parquet 물리 byte는 deserialized cache 크기와 같지 않으므로 이 한도는 scheduler나 autoscaler가 자동 계산하지 않으며, 기본값 0은 staging-only다.
