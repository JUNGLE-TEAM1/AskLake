...
- Phase 8 runtime Secret handoff는 FastAPI, Airflow, Spark, Trino가 소비하는 네 Kubernetes Secret의 이름·key, 공유 token/JDBC binding과 읽기 전용 file mount를 값 없이 고정한다. Terraform은 Secret resource나 외부 secret version을 소유하지 않고 `disabled`, `external_secrets`, `workflow_sync` 선택 gate만 제공한다. AWS static credential은 금지하고 IRSA/Pod Identity를 사용한다. 실제 전달 방식은 controller/source/rotation owner와 rollback 절차를 학습·확정하기 전까지 선택하지 않는다. 이 gate는 core workload의 Secret sync 준비만 나타내며, Airflow API 인증 방식과 아직 foundation에 없는 AI provider workload까지 포함한 전체 서비스 production readiness는 별도 계약이다. 상세 기준은 [Phase 8 런타임 Secret 전달 계약](eks-phase-8-runtime-secrets.md)을 따른다.
