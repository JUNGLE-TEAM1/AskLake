...
- Phase 7 network handoff는 ALB controller owner, internal/internet-facing exposure, ip/instance target, exact DNS/ACM, private egress와 Pod network enforcement를 선택 전 `disabled`/`undecided`로 유지한다. 선택이 완전해진 경우에만 별도 chart가 HTTPS ALB group에 FastAPI `/api`와 Frontend `/` Ingress를 분리해 각 target group의 실제 health path를 적용한다. 이 state는 shared controller/CNI, NAT Gateway, VPC endpoint, DNS, certificate나 ALB를 직접 생성하지 않는다. IngressGroup 참여 권한은 배포 주체로 제한해야 하며 application ServiceAccount에는 Ingress 변경 권한을 주지 않는다. 상세 기준은 [Phase 7 Network와 ALB Ingress 계약](eks-phase-7-network-ingress.md)을 따른다.
