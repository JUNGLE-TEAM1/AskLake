...
- SQL 분석 구현은 `SqlAnalysisPage.tsx`가 화면 상태와 큰 레이아웃을 맡고, `sqlLogic.ts`가 SQL 검증/자동완성/format helper를, `SqlPreviewTable.tsx`, `SqlDashboardPreview.tsx`, `SqlSchemaPanel.tsx`, `SqlDatasetRow.tsx`가 표시 컴포넌트를 맡는다.
