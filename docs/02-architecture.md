...
- Source, Schema, Create, Run은 명시된 `VITE_API_BASE_URL` 또는 기본 same-origin `/api`를 통해 live backend를 호출한다. EKS Frontend image에는 API hostname을 고정하지 않고 ALB가 `/api`를 FastAPI로 분기한다.
