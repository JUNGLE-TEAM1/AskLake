...
- Issue #735의 EKS + MSK MVP는 아직 현재 runtime이 아니라 후보 배포 경로다. 배포 broker는 MSK Serverless + IAM을 사용하고, frontend/FastAPI/Airflow/단일 Trino coordinator/Spark batch를 EKS로 옮기는 동안 기존 EC2 Kafka Continuous control plane과 worker는 유지한다. 테스트 fixture producer는 EKS 밖에서 격리된 MSK topic에 입력하며 Kafka/Redpanda broker를 EKS 안에 운영하지 않는다. 로컬 Redpanda는 fixture/replay 경로로 남긴다.
...
- Phase 1 foundation은 `infra/eks/terraform`의 existing/create cluster 입력, ECR와 Trino handoff output, `infra/eks/helm/asklake-foundation`의 namespace/service account/RBAC 계약으로 구성한다. Phase 10부터 신규 cluster compute는 EKS Auto Mode이고 표준 Managed Node Group은 만들지 않는다. `asklake-backend`는 `asklake-dev` namespace의 SparkApplication 제출·조회·취소와 Pod log/Event 조회에 한정된 Role만 사용하고, `asklake-spark`도 driver lifecycle에 필요한 namespace Role만 사용한다. Replay Producer 입력은 호환 value에서 `create=false`로 유지하며 EKS ServiceAccount/ECR/workload를 만들지 않는다. 그 밖의 workload IAM action과 network destination은 Pair B가 [Phase 1 인수 계약](eks-msk-mvp-phase-1-handoff.md)에 따라 반환하기 전까지 추정해 부여하지 않는다.
- Phase 3 data plane은 MSK, RDS와 S3를 각각 `disabled`/`existing`/`create` 모드로 분리하고 기본값을 `disabled`로 둔다. 배포 broker는 IAM 인증 MSK Serverless만 허용하며 test topic/group은 기존 EC2 Continuous consumer와 격리한다. RDS create는 private PostgreSQL과 `asklake_app` 초기 database까지만 소유하고 Airflow/Iceberg database·user·grant는 별도 멱등 bootstrap이 담당한다. S3는 Raw/Output/Warehouse/Query Result bucket과 안정적인 prefix를 분리한다. workload IAM은 최소 권한 policy document까지만 만들며 IRSA 또는 Pod Identity 선택 전에는 role에 연결하지 않는다. 실제 apply 경계는 [Phase 3 Data Plane 계약](eks-phase-3-data-plane.md)을 따른다.
- Phase 4 workload identity는 IRSA와 EKS Pod Identity를 선택형으로 지원하고 기본값을 `disabled`로 둔다. Backend, Trino, MSK smoke와 Spark는 각자 전용 role만 사용하며 Frontend/Airflow와 EKS 밖 fixture producer에는 EKS role을 만들지 않는다. policy와 trust resource key는 생성 예정 ARN/issuer와 분리된 정적 workload key를 사용하고 Spark checkpoint/quarantine 및 Backend evidence의 읽기 복구 경계를 포함한다. 신규 EKS의 IRSA는 cluster와 OIDC provider 확인 뒤 두 번째 plan에서 활성화한다. RDS는 host 일치와 `verify-full` TLS gate 뒤 `asklake_app`, `airflow_metadata`, `iceberg_catalog` database와 전용 login role을 멱등 bootstrap으로 분리하되 application schema migration과 EC2 데이터 이전은 별도 단계다. 선택·secret·실행 gate는 [Phase 4 Workload Identity와 RDS Bootstrap](eks-phase-4-identity-rds-bootstrap.md)을 따른다.
...
- Phase 8 runtime Secret handoff는 한 machine-readable JSON을 정적 기준으로 사용해 FastAPI, Airflow, Spark, Trino가 소비하는 네 Kubernetes Secret의 이름·key, 공유 token/JDBC binding, env injection과 읽기 전용 file mount를 값 없이 고정한다. Terraform은 같은 JSON을 읽어 전체 계약을 output하며 Secret resource나 외부 secret version을 소유하지 않는다. `ready_for_sync`와 Airflow API 인증·AI runtime까지 선택한 `full_service_secret_contract_ready`를 분리하고 Phase 5 선택값과 결합 검증한다. AWS static credential은 금지하고 IRSA/Pod Identity를 사용한다. 실제 전달 방식과 미결정 runtime은 학습·확정하기 전까지 선택하지 않는다. 상세 기준은 [Phase 8 런타임 Secret 전달 계약](eks-phase-8-runtime-secrets.md)을 따른다.
...
- Phase 11 network foundation은 기본 `external` 경로에서 기존/shared VPC를 참조만 하고, 신규 MVP-owned EKS에 한해서만 전용 VPC와 최소 2개 AZ의 public/private subnet을 생성할 수 있다. 생성 subnet은 VPC CIDR의 `newbits/netnum`으로 계산해 중복을 막고 EKS/MSK/RDS는 private subnet을 공유한다. private egress는 NAT Gateway의 single/per-AZ 또는 최소 AWS interface endpoint+S3 gateway를 실제 비용·destination 검토 후 명시해야 하며 미선택 상태에서는 plan이 실패한다. 생성형 MSK `9098`과 RDS `5432`는 EKS cluster security group만 source로 허용한다. 공개 ALB·DNS·ACM은 Phase 13, custom NodePool·NodeClass는 Phase 12 범위이며 Security Groups for Pods는 별도 network enforcement 선택과 runtime 검증 전에는 적용하지 않는다. 상세 기준은 [Phase 11 VPC와 Private Network Foundation](eks-phase-11-network-foundation.md)을 따른다.
...
- Phase 13은 `eks.amazonaws.com/v1` IngressClassParams와 `eks.amazonaws.com/alb` IngressClass를 사용해 Auto Mode가 ALB를 직접 reconcile하게 한다. 환경별 class는 exact namespace label로 사용 범위를 제한하고, scheme·address type·subnet·ACM·group은 class params, target type·HTTPS redirect·health path는 두 Ingress에 둔다. 별도 AWS Load Balancer Controller를 설치하지 않으며 실제 DNS record도 이 state에서 만들지 않는다. server-side dry-run, 비용 confirmation과 Ingress-first 삭제 gate를 통과한 뒤에만 적용하고 실제 ALB/HTTPS/target/cost evidence 전에는 runtime 완료로 간주하지 않는다. 상세 기준은 [Phase 13 Auto Mode ALB 진입 경로](eks-phase-13-auto-mode-alb.md)를 따른다.
...
EKS 애플리케이션 workload는 `infra/eks/helm/asklake-workloads` chart가 소유한다. chart는 `asklake-dev` namespace에 Frontend/FastAPI 2 replica, Airflow API server/scheduler/DAG processor, Trino coordinator를 `Deployment`로 배포하고 외부에 직접 노출하지 않는 `ClusterIP` Service를 만든다. Airflow DB migration은 Helm pre-install/pre-upgrade hook Job으로 실행한다. namespace, Spark operator, EKS Pod Identity ServiceAccount/RBAC, RDS/MSK/S3/ECR과 External Secrets Operator 전달 기반은 EKS foundation 범위가 제공한다. 현재 ALB는 IngressClass/IngressClassParams까지만 적용됐고 실제 Ingress와 ALB는 없다.

일반 설정은 `ConfigMap`으로 전달한다. credential과 TLS 파일은 chart가 만들지 않으며 기존 `asklake-backend-runtime`, `asklake-airflow-runtime`, `asklake-spark-runtime`, `asklake-trino-runtime` Secret의 확정된 key만 참조한다. External Secrets Operator controller/store 준비는 실제 네 Secret 동기화 완료와 구분한다. workload image는 모두 `repository@sha256:digest`로 고정하고 static AWS access key는 허용하지 않는다. FastAPI와 Spark driver의 최소 권한 RBAC는 foundation chart가 단독 소유한다. workload chart는 해당 Role/RoleBinding을 다시 만들지 않고, `asklake-backend`에는 `SparkApplication` create/get/list/watch/delete와 Pod log 조회, `asklake-spark`에는 executor Pod/Service/ConfigMap 관리 권한이 제공됐다는 계약만 소비한다. Frontend/FastAPI Service는 A/B 원격 계약의 `frontend:80`, `fastapi:8080`을 사용한다. 다른 Service 이름을 선택하면 Ingress values와 foundation handoff를 같은 변경에서 갱신한다.

Airflow MVP는 `LocalExecutor`와 image에 bake한 DAG를 사용하고 metadata를 RDS `airflow_metadata`에 저장한다. API server, scheduler, DAG processor는 각각 1 replica이며 EFS/PVC와 shared DAG/log volume은 만들지 않는다. 따라서 Pod-local task log의 재시작 후 보존이나 cross-Pod 공유는 보장하지 않는다. 이 제한은 MVP에서 수용하고 durable log, scheduler HA 또는 동적 DAG 배포가 필요할 때 storage/executor 설계를 다시 연다.
...
`ASKLAKE_SPARK_RUNNER=kubernetes`는 FastAPI의 Node bridge가 in-cluster Kubernetes API를 호출하는 provider다. provider는 `runId`에서 결정적인 `SparkApplication` 이름을 만들고 run/job/image digest annotation을 함께 저장한다. create 응답 유실 또는 `409`가 발생하면 같은 이름을 조회해 annotation과 image가 모두 일치할 때만 기존 실행을 복구하므로 다른 실행을 오인하지 않는다. terminal state까지 polling한 뒤 driver log의 `ASKLAKE_SPARK_JOB_RESULT` marker를 읽고, timeout이면 해당 application을 삭제한다. 실행 결과에는 application name/UID, driver Pod, image digest, recovery 여부와 최종 state가 남는다.

배포용 opt-in smoke는 두 단계다. MSK smoke Job은 `asklake-msk-smoke` Pod Identity로 TLS/OAUTHBEARER metadata 조회만 확인한다. Spark smoke는 `asklake-spark` Pod Identity로 fixture topic의 `earliest`부터 실행 시점 `latest`까지 bounded read하고 전용 `iceberg.asklake.eks_mvp_fixture` table을 RDS JDBC catalog와 S3 warehouse에 replace commit한다. replay producer와 Continuous worker는 chart에 포함하지 않는다. 15일차 MVP는 Spark driver/executor가 `asklake-spark`를 공유하므로 executor도 Kubernetes API token과 driver RBAC을 받는 잔여 과권한을 수용한다. 운영 전에는 driver를 token/RBAC 사용 ServiceAccount, executor를 token/RBAC 없는 별도 ServiceAccount로 분리하되 양쪽의 Spark MSK/S3 Pod Identity 권한은 유지한다. 실제 Secret 값과 image digest가 주입된 live smoke는 별도 배포 gate다.
