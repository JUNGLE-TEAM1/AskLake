...
Estimate & Guardrail phase는 Catalog size와 JOIN complexity로 실행 전 예상 처리량/시간을 계산한다. 유효한 SQL은 editor 아래에서 자동 평가하며, submit 시점 snapshot은 Query Run metadata에 보존하되 재실행 승인용 confirmation token은 저장하지 않는다. warning threshold를 넘으면 backend가 actor/query/dataset/TTL-bound confirmation token을 요구하고, optional hard byte limit은 서버에서 실행을 차단한다. 이 heuristic은 실제 Trino 처리량·시간·비용의 대체값이 아니라 실행 전 의사결정 보조이며, 완료 run stats가 source of truth다. status polling은 Trino `progressPercentage` 또는 split 완료 비율만 progress로 표시하고, 값이 없으면 indeterminate 상태로 둔다. query terminal 뒤 result object를 쓰는 collector 시간은 별도 결과 수집 단계다.
...
- Trino Query Run의 1회성 Dataset 생성은 별도 `DialogShell`에서 metadata를 받고 `POST /api/catalog/trino-runs/{runId}/materializations`를 호출한다. backend는 저장된 compiled SQL로 Iceberg CTAS를 수행하고 table 확인 후 Catalog 등록 상태를 갱신한다.
...
- SQL 분석 화면은 오른쪽 `선택 테이블`/schema 사이드바 없이, 왼쪽 `분석 테이블` 트리에서 테이블 행을 클릭해 선택·해제한다. 선택된 행에는 `선택됨` 상태를 표시하고, SQL editor의 사용자가 직접 작성한 query text가 실행 기준 source of truth이며 UI 선택 상태로 역동기화하지 않는다. 테이블을 해제해도 SQL text는 자동 재작성하지 않고, 해제된 table을 계속 참조하면 실행 전 table context 검증에서 차단한다. UI에서는 base/reference를 구분하지 않고, 내부 API payload만 기존 `sourceDatasetId`/`referenceDatasetIds` 계약을 유지한다.
- SQL 분석 구현은 `SqlAnalysisPage.tsx`가 화면 상태와 큰 레이아웃 및 Trino estimate/run/history/materialization 상태를 맡고, `sqlLogic.ts`가 SQL 검증/자동완성/format helper를, `queryAiService.ts`가 SQL 초안 생성 요청을, `SqlPreviewTable.tsx`, `SqlResultChart.tsx`, `SqlDatasetRow.tsx`가 결과 표·위젯·데이터셋 표시를 맡는다. `SqlChartConfigurator.tsx`는 bounded 결과와 선택 데이터셋을 `DashboardDatasetOption`으로 변환하고 Dashboard `WidgetConfigPanel`을 합성한다. `SqlAiWriterDialog.tsx`는 shadcn `Popover`, `Bubble`, `Collapsible`로 Nessie prompt와 적용 상태를 구성하고, `SqlJobWizardDialog.tsx`는 compatibility 결과 기반 Job의 다단계 설정을 맡는다. Trino 결과 표는 cursor 기반 원격 페이지를 사용하며 `Dialog` 전체 보기에서도 같은 페이지 계약을 유지한다.
