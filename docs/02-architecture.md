...
EKS Continuous 실행은 `ASKLAKE_CONTINUOUS_SPARK_RUNNER=kubernetes`일 때 전용 SparkApplication gateway가 Spark Operator API로 `Python` cluster-mode application을 생성한다. application은 Job ID와 worker attempt ID label을 갖고, digest-pinned `ASKLAKE_SPARK_KUBERNETES_IMAGE`, service account, private runtime-document S3 prefix를 모두 요구한다. Continuous control worker가 Spark 제출 문서를 만들 때도 `asklake-spark-runtime` Secret의 JDBC URL/user/password를 `TRINO_ICEBERG_JDBC_*` alias로 주입한다. SparkApplication spec에는 값을 평문으로 넣지 않고 `ASKLAKE_SPARK_KUBERNETES_RUNTIME_SECRET_NAME`의 `secretKeyRef`로만 전달한다. MSK Serverless `9098` source는 `SASL_SSL`/`AWS_MSK_IAM` Kafka client option을 사용하고, runtime image에 bake한 shaded IAM JAR을 SparkApplication dependency로 명시한다. 구분자 기반 record parser는 `properties.position` 같은 source path를 중첩 `StructType`으로 구성해 leaf token의 타입 검증이 상위 struct null 때문에 전부 격리되지 않게 한다. terminal SparkApplication 교체는 새 제출 문서 생성, Iceberg credential 검증, output bucket 확인과 command 정리를 모두 통과한 뒤에만 기존 resource를 삭제한다. durable desired state가 `running`인데 현재 attempt가 사라진 경우에는 남은 마지막 report를 먼저 materialize한 뒤 같은 fencing token의 새 SparkApplication을 자동 생성한다. pause/stop command도 worker attempt token을 포함해 새 attempt가 이전 command를 적용하지 않게 하며, SparkApplication `COMPLETED`는 공통 runtime의 정상 종료 상태 `exited`로 정규화한다. 이 모드는 일반 유한 Spark batch runner의 `ASKLAKE_SPARK_RUNNER`와 분리되어 있다.
...
- 현재 운영 사이드바는 `파이프라인 구축`, `카탈로그`, `SQL 분석` 세 업무 메뉴만 노출한다. 기존 Dashboard, AI/Semantic legacy route와 Admin direct route는 `/jobs`로 복귀시키며, 계정 드롭다운의 프로필·로그아웃은 인증 shell 기능으로 유지한다. 각 화면 본문은 생성 같은 업무 action과 실제 콘텐츠만 소유하며 상세·편집 route의 고유 헤더는 별도로 유지한다. SQL 데스크톱 workspace와 editor는 기존 대비 1.5배 높이 토큰을 사용하고 1180px 이하에서는 기존 자동 높이 반응형 계약을 유지한다. SQL의 주요 action과 탭은 텍스트 label을 접근성 이름의 source of truth로 사용하고 장식 glyph를 반복하지 않는다. 이 표현 변경은 Dataset 선택, query, panel 접힘 상태나 API payload를 변경하지 않는다.
