...
- Phase 14는 Phase 13의 target인 Frontend Nginx와 FastAPI를 각각 `frontend:80`, `fastapi:8080` ClusterIP Service와 Deployment로 정의하고, 같은 Backend image/runtime identity를 쓰는 `trino-result-collector` 1-replica Deployment를 함께 소유한다. 세 workload는 General NodePool에만 배치하며 Collector는 HTTP/Service/Ingress와 Kubernetes API token을 갖지 않는다. ECR digest, runtime ConfigMap/Secret, Foundation과 NodePool, B의 다중 replica·Continuous 분리 증거가 완성되기 전에는 렌더하지 않는다. Airflow/Trino coordinator/Spark workload와 HPA/PDB 수치는 별도 lifecycle이며 실제 rollout/restart/Query Run terminal 증거 전에는 EKS 전환 완료가 아니다. 상세 기준은 [Phase 14 Frontend·FastAPI·Collector Workload](eks-phase-14-web-workloads.md)를 따른다.
