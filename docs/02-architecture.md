...
현재 FastAPI가 직접 소유하는 영역은 ETL, Run, Catalog hydrate, Catalog lineage fallback, SQL query compatibility runtime, SQL derived dataset 저장, Dashboard card/list, Dashboard draft/published runtime이다. Issue #488 Phase 1은 Compose 내부 Trino 482 coordinator, Iceberg JDBC catalog, MinIO S3 warehouse와 Catalog `queryEngineTable` physical mapping을 추가했다. Phase 2는 Trino HTTP adapter와 canonical Query Run persistence/compiler를 추가했다. Query Result Phase 1은 일반 Trino result page를 private MinIO gzip object에 저장하고 PostgreSQL에는 page metadata/checksum/manifest만 남긴다. Query Result Phase 2는 `trino-result-collector` worker가 DB lease를 얻어 Trino continuation을 수집하므로 browser polling은 persisted status를 읽기만 한다. collector lease가 만료되면 다른 worker가 마지막 durable continuation부터 다시 이어받으며, continuation URI는 page metadata에도 기록해 retry가 중복 page를 만들지 않게 한다. Query Result Phase 3은 run/retention-bound HMAC opaque cursor로 result page API를 보호하고 SQL 분석 UI에 result storage 상태를 표시한다. Query Result Phase 4는 `GET /api/query/runs`로 현재 사용자가 제출한 실행 이력 요약을 제공하고, SQL 분석 화면은 저장된 run을 다시 열어 cursor 결과를 재사용한다. 목록은 user ID 기반 submitter 단위로만 조회하며 ID 없는 legacy record에만 이름 fallback을 허용한다. 결과 retention은 result page에만 적용하므로 만료된 run도 SQL, 상태, 통계는 재확인할 수 있다. 재열기 시 개별 run의 Dataset 권한·차단·잠금 검사를 다시 수행한다. 기존 PostgreSQL row page는 migration read compatibility로만 유지한다. production Trino는 backend-only network와 HTTPS/password/file access control을 사용한다. 현재 API routing과 SQL runtime은 `TRINO_ENABLED` flag에 따라 전환된다. 목표 Trino 계약은 `docs/trino-query-run-contract.md`와 `docs/trino-query-result-storage-contract.md`를 따른다.

Query Engine 등록은 Dataset 표시명과 물리 table 이름을 분리한다. SQL 결과 Dataset은 이름 기반 안정적인 Dataset ID와 materialization별 충돌 없는 ASCII table 이름을 만들고, Catalog에 `pending`을 먼저 저장한 뒤 Iceberg CTAS terminal success와 `DESCRIBE` 검증이 모두 끝나야 `queryEngineStatus=available` 및 `queryEngineTable`을 공개한다. 실패 시 mapping은 공개하지 않고 `registration_failed`와 안전한 오류 코드만 저장하며 동일 materialization 조회로 검증을 재시도할 수 있다. `TRINO_ENABLED=true`에서는 mapping이 검증되지 않은 Dataset의 `permissions.canQuery`를 false로 계산해 frontend와 backend가 같은 기준으로 차단한다. 현재 Spark Parquet와 Kafka JSONL writer는 Iceberg table을 만들지 않으므로 거짓 mapping을 생성하지 않고 `unavailable`로 저장한다. 향후 writer가 `queryEngineVerified=true`와 검증된 mapping을 반환할 때만 SQL 대상으로 자동 승격한다.

Estimate & Guardrail phase는 Catalog size와 JOIN complexity로 실행 전 예상 처리량/시간을 계산한다. warning threshold를 넘으면 backend가 actor/query/dataset/TTL-bound confirmation token을 요구하고, optional hard byte limit은 서버에서 실행을 차단한다. 이 heuristic은 실제 Trino 처리량·시간·비용의 대체값이 아니라 실행 전 의사결정 보조이며, 완료 run stats가 source of truth다.

Estimate는 가능한 경우 `EXPLAIN (TYPE DISTRIBUTED)`의 Trino plan byte estimate를 우선 사용한다. plan endpoint가 일시적으로 실패하거나 estimate를 제공하지 않으면 Catalog heuristic으로 fallback하고, API/UI가 source를 명시해 두 값을 실제 비용으로 오해하지 않게 한다.
