...
- Phase 5는 Terraform state와 private runner가 정확한 stack으로 확인된 뒤 smoke가 SSM 이전 artifact/price 단계에서 실패해도 redacted completion receipt를 남긴다. cleanup plan은 smoke job과 분리해 먼저 artifact로 검토하고, apply는 `asklake-aws-staging-destroy` 보호 Environment에서 재계획 fingerprint가 일치할 때만 수행한다. 별도 TTL sweep은 state bucket의 엄격한 staging tag/`ExpiresAt`만 읽어 만료·비정상 stack을 발견하면 evidence와 실패 신호를 남기며, 자동 삭제 대신 기존 보호 destroy 승인으로 연결한다.
- Phase 6 handoff bundle은 smoke evidence, cleanup receipt, TTL audit을 같은 stack/source revision 기준으로 묶고 EMR Job Run·S3 report·price snapshot 위치만 redacted Markdown/JSON으로 전달한다. 이 bundle은 Phase 7 성능 승인과 production promotion을 명시적으로 차단한다.
