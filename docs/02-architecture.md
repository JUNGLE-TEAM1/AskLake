...
- 일반 Snapshot Spark runtime은 확정 schema projection을 `MEMORY_AND_DISK`로 materialize한 뒤 transform/quality/target write가 같은 lineage를 재사용한다. 직접 컬럼 복사와 `TRIM(CAST(<input> AS STRING))`으로 한정한 total·row-preserving SQL transform은 이전 row count를 재사용해 rule별 validation action을 만들지 않고, 그 밖의 SQL은 기존 검증 action을 유지한다. legacy Quality 규칙의 전체 실패 수와 rule별 실패 수는 하나의 aggregate action으로 계산하고, canonical Rule runtime도 rule 수에 비례해 source action을 늘리지 않는다. output frame과 quarantine frame을 포함한 cache는 성공·규칙 실패·예외 경로 모두에서 `unpersist`한다.
