...
배포용 opt-in smoke는 두 단계다. MSK smoke Job은 `asklake-msk-smoke` IRSA로 TLS/OAUTHBEARER metadata 조회만 확인한다. Spark smoke는 `asklake-spark` IRSA로 fixture topic의 `earliest`부터 실행 시점 `latest`까지 bounded read하고 전용 `iceberg.asklake.eks_mvp_fixture` table을 RDS JDBC catalog와 S3 warehouse에 replace commit한다. replay producer와 Continuous worker는 chart에 포함하지 않는다. `asklake-spark` ServiceAccount는 executor 관리를 위해 Kubernetes API token 자동 mount가 필요하다. 실제 MSK endpoint/topic, S3 bucket, RDS DSN, Secret 값, image digest와 IRSA 권한이 주입된 live smoke는 EKS foundation 완성 후 수행한다.
