...
- Phase 11 network foundation은 기본 `external` 경로에서 기존/shared VPC를 참조만 하고, 신규 MVP-owned EKS에 한해서만 전용 VPC와 최소 2개 AZ의 public/private subnet을 생성할 수 있다. dev는 전용 VPC와 단일 NAT를 적용해 EKS/MSK/RDS를 private subnet에 배치하고 MSK `9098`과 RDS `5432`를 EKS cluster security group source에만 허용했다. 일반 outbound용 단일 NAT와 기본 route는 유지하면서 `enable_s3_gateway_endpoint=true`일 때만 두 private route table에 S3 prefix-list route를 추가할 수 있다. 이 선택은 `private_egress_mode=hybrid`나 Interface Endpoint 생성을 요구하지 않으며 S3 traffic만 Gateway Endpoint로 우회한다. STS와 외부 API는 기존 NAT 경로를 유지한다. Pod enforcement는 Auto Mode Network Policy Controller와 NodeClass `DefaultAllow`로 활성화했으며 임시 deny policy가 실제 enforcement됨을 확인했다. 실제 workload allow/default-deny 정책은 B의 Service·port 계약 뒤 추가한다. 공개 ALB·DNS·ACM은 Phase 13 범위다. 상세 기준은 [Phase 11 VPC와 Private Network Foundation](eks-phase-11-network-foundation.md)을 따른다.
