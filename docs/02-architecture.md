...
- SQL 결과 저장 UI는 1회성 `Dataset으로 저장`과 `반복 Job 만들기`를 분리한다. compatibility 결과는 `useAskLakeData.createSqlDatasetJob`이 기존 `POST /api/etl/jobs`를 사용한다. Trino 반복 Job은 같은 `SqlJobWizardDialog`의 Trino mode에서 managed Iceberg/full-refresh를 명시하고 `POST /api/etl/sql-jobs`를 호출한다.
...
- SQL 분석 구현은 `SqlAnalysisPage.tsx`가 화면 상태와 큰 레이아웃 및 Trino estimate/run/history/materialization 상태를 맡고, `sqlLogic.ts`가 SQL 검증/자동완성/format helper를, `queryAiService.ts`가 SQL 초안 생성 요청을, `SqlPreviewTable.tsx`, `SqlResultChart.tsx`, `SqlDatasetRow.tsx`가 결과 표·위젯·데이터셋 표시를 맡는다. Trino 문법의 최종 판정은 `POST /api/query/validate`의 Trino parser/compiler 계약이며 frontend PostgreSQL parser는 UX 보조다. 실행 전 비용 평가는 Plan 추정과 Catalog 물리 크기 기반 값을 분리 보존하고 큰 값을 보수적 guardrail 기준으로 사용한다. 실행 이력은 결과 본문에 상시 렌더링하지 않고 실행 버튼 옆 모달에서 연다. `SqlJobWizardDialog.tsx`는 compatibility Job과 Trino 반복 Job mode를 함께 지원한다. Trino 결과 표는 cursor 기반 원격 페이지를 사용하며 현재 page만 정렬 대상으로 오해하지 않도록 원격 page에서는 client sort를 비활성화한다.
