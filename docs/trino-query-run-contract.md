...
production Trino는 backend-only internal network, HTTPS/password authentication, file-based read-only access control, separate Iceberg JDBC/MinIO credentials를 전제로 한다. TLS CA, keystore, password hash file은 서버 secret 경로에서 mount하며 repository에 저장하지 않는다. Phase 3부터 `TRINO_ENABLED=true`이면 `/api/query/runs` routing은 Trino runtime을 사용한다. frontend polling/pagination 전환은 후속 Phase다.
