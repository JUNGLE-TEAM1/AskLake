...
- SQL 분석 화면의 기본 `실행`은 원본 SQL을 최대 100행 subquery로 감싼 `mode=preview` Query Run이다. 작은 결과를 PostgreSQL에 저장해 표와 차트를 먼저 보여 준다.
- 실행 전에는 read-only SQL, 선택 Dataset context, 권한, 차단/잠금, 추정 정보를 검증한다. Preview도 같은 원본 Dataset을 Trino로 읽으므로 집계·정렬 쿼리는 입력 전체를 스캔할 수 있지만, 전체 출력 저장은 기다리지 않는다.
- `전체 보기` 또는 `CSV 다운로드`를 누를 때만 성공한 preview를 source로 `mode=run`을 시작하거나 재사용한다. 이 run은 원본 SQL 전체 결과를 S3-compatible page로 저장하고 브라우저는 cursor page만 조회한다.
...
SQL editor
  ...
  -> POST /api/query/runs (mode=preview, max 100 rows)
  ...
  -> PostgreSQL inline preview page
...
full view or CSV
  ...
  -> linked mode=run Query Run
  -> trino-result-collector continuation drain
  -> private S3-compatible result pages
  -> cursor page view or server-side CSV
```
...
- `POST /api/query/runs`는 public SQL 화면에서 기본 `mode=preview`, `previewLimit<=100`을 적용하고 `202 Accepted`와 `runId`, 최초로 저장된 실행 상태, 제출 시점 estimate snapshot을 반환한다. snapshot에는 예상 처리량/시간, source, risk, warning만 저장하며 confirmation token은 저장하거나 run response로 다시 반환하지 않는다.
- `POST /api/query/runs/{previewRunId}/full-results`는 성공한 preview의 SQL/context를 그대로 사용해 `mode=run`, `sourceRunId=previewRunId`를 생성한다. 같은 preview의 active run 또는 보관 중인 성공 run은 재사용한다.
...
- 실행/결과 milestone 시각은 UTC ISO 8601 optional field다. `stats.queryCompletedAt`은 Trino 완료를 최초 관측한 시각, `result.collectionStartedAt`은 collector가 결과 수집 추적을 시작한 시각, `firstPageAvailableAt`은 첫 durable page가 준비된 시각, `collectionCompletedAt`은 해당 run manifest가 `available`이 된 시각이다. Preview에서는 최대 100행 inline page 준비가 곧 완료이고, full run에서는 전체 object page 수집 완료를 뜻한다. 한 번 기록한 시각은 polling, retry, collector 재시작이나 lease takeover에서 덮어쓰지 않는다.
...
- SQL 분석 UI는 polling 응답의 queued/elapsed time, processed bytes/rows, peak memory를 실행 중과 완료 뒤에 함께 표시한다. 실행 전 estimate는 현재 SQL에서 자동 계산하는 참고값이고 실제 stats가 우선한다. estimate와 timeline은 editor 아래에 별도 card로 삽입하지 않고 결과 panel의 `실행 정보` view에서 함께 표시한다. Backend가 보존하는 estimate snapshot은 향후 또는 별도 client의 이력 재열기에도 사용하지만, 이번 화면에는 별도 최근 실행 선택 목록을 노출하지 않는다. Query 실행과 결과 수집은 별도 단계이며 부분 결과에는 `조회됨`이 아니라 `수집됨`을 사용한다.
...
- Preview 결과는 최대 100행 PostgreSQL inline page로 저장한다. Full run 결과만 private S3-compatible gzip page object에 저장하고 PostgreSQL에는 page metadata를 남긴다. 로컬은 MinIO, production은 사전 생성한 AWS S3 Query Result bucket과 EC2 instance profile을 사용한다. browser-independent collector lifecycle은 `docs/trino-query-result-storage-contract.md`를 canonical source로 둔다.
...
- UI는 결과 panel의 `실행 정보` view에 `Iceberg 메타데이터 기준` 스캔량과 현재 SQL에서 새로 계산한 `예상 실행 시간`을 표시한다. fallback일 때만 `보수적 추정`을 사용한다. 확인 모달은 조직의 bytes/time/concurrency 정책 임계치를 넘는 경우에만 사용한다. SQL editor의 높이, toolbar, textarea scroll 계약은 Trino 통합 전과 동일하게 유지한다.
...
- Frontend는 preview의 `실행 정보` view에서 `쿼리 실행`, `첫 결과 준비` 두 단계만 파생한다. 요청 접수와 `QUEUED`/`WAITING`/`PLANNING`/`STARTING`은 첫 단계의 phase label이다. Preview가 끝난 뒤 full run이 시작돼도 그 저장 진행률을 preview timeline에 합치지 않는다. 첫 page 자동 조회는 조회 가능한 최초 page가 없는 상태에서 한 번만 수행하고, 실패하면 해당 단계를 실패로 표시한다. 재시도 성공 시 page 행 수와 브라우저 표시 시간을 다시 측정해 완료로 전환한다. 이 view는 `차트 보기`, `데이터 미리보기`와 같은 결과 panel 높이 안에서 scroll하고 editor 아래 별도 sibling block을 만들지 않는다.
- 진행 bar는 해당 active 단계가 2초 이상 지속되고 실제 분자/분모를 확보했을 때만 표시한다. Preview 쿼리 단계는 Trino progress/driver/split만 사용한다. Full run의 `collectedRowCount / expectedRowCount`는 전체 보기/CSV 준비 상태에서만 사용하고 preview와 하나의 전체 퍼센트로 합치지 않는다.
- Result manifest의 `storageStatus`는 명시적으로 판정한다. `collecting`은 수집 중, `available`은 준비 완료, `unavailable`은 결과 저장 실패, `expired`는 결과 보관 만료다. Query가 끝났지만 `storageStatus`가 없으면 완료나 0행으로 간주하지 않고 `결과 준비 상태 확인 중`으로 유지한다. 실행 정보 view는 Trino runtime에서만 활성화하고 compatibility/mock 실행에는 적용하지 않는다. 실행 전후 editor를 축소하거나 높이를 바꾸지 않는다.
...
- SQL 화면은 성공 preview 뒤 `전체 보기`, server-side CSV 다운로드, `반복 Job 만들기` action을 제공한다. 전체 보기와 CSV는 full run을 요구하지만, 반복 Job 만들기는 full result 저장을 요구하지 않는다. 1회성 Iceberg materialization API는 운영/API 경로로 유지하지만 이 결과 화면에는 노출하지 않는다.
- 반복 SQL Job은 preview/result page를 입력으로 복사하지 않고 preview에서 검증한 SQL recipe만 저장한다. 각 수동/예약 Job Run에서 그 SQL을 전체 source Dataset에 다시 실행하는 `full_refresh` CTAS다.
...
Phase 1은 Trino single-node coordinator와 Iceberg JDBC catalog, S3-compatible warehouse 구성, Catalog `queryEngineTable` mapping schema와 backend configuration을 추가합니다. 로컬 root Compose는 MinIO를, production은 사전 생성한 AWS S3 Warehouse bucket과 instance profile을 사용합니다. `scripts/verify-deploy-dependencies.sh`는 Trino image availability도 확인합니다.
...
production Trino는 backend-only internal network, client/internal HTTPS, password authentication, file-based least-privilege access control, 별도 Iceberg JDBC identity와 Warehouse/Query Result bucket 최소 권한을 전제로 한다. `asklake-api`는 SELECT와 자기 query 실행/관리만, `asklake-materializer`는 `asklake` schema의 CTAS/검증에 필요한 권한만 가진다. TLS CA, keystore, bcrypt cost 8 이상 또는 PBKDF2 password hash file은 서버 secret 경로에서 mount하며 repository에 저장하지 않는다. PostgreSQL bootstrap은 기존 volume에도 JDBC role과 Iceberg metadata table 소유권을 멱등 반영하고, production S3 bucket은 배포 전에 생성해 EC2 instance profile로 readiness를 통과시킨다. 장기 AWS access key/secret은 저장하지 않는다. `TRINO_ENABLED=true`이면 `/api/query/runs` routing은 Trino runtime을 사용하고 frontend는 기본 preview polling과 cursor result page를 사용한다. Preview `LIMIT`은 backend가 compiled SQL을 감싼 subquery에 적용하며 사용자의 저장 SQL recipe는 바꾸지 않는다.

Phase 5는 legacy JSONL derived dataset 경로와 Trino run을 명확히 분리한다. SQL Job은 preview 또는 full result page를 재조합하지 않고, succeeded preview의 원본 SQL을 Iceberg CTAS statement로 materialize한다.
