...
이 EC2 demo 배포는 Issue #727의 일회성 EMR Serverless/MSK Serverless staging과 별개다. Phase 1 Terraform, Phase 2 output 변환기, Phase 3 수동 OIDC workflow와 Phase 4 private SSM smoke 실행기가 있어도 일반 `scripts/deploy.sh` 또는 애플리케이션 재배포는 이를 실행하지 않는다. staging IaC/smoke의 Phase, 비용·TTL과 수동 승인 경계는 [AWS Staging IaC와 실제 Smoke 자동화 계획](aws-staging-iac-smoke-plan.md)을 따른다.
