...
## Text Structuring Runtime Integration - 2026-07-11

- Catalog의 검색 대상과 주 자산은 dataset이다. model artifact는 dataset과 분리된 ETL runtime artifact이므로 검색 결과 아래의 전역 `모델 아티팩트` Panel을 제거했다.
- `model_artifacts` 저장소, `/api/catalog/models`, `/api/text-structuring/models`, ETL 변환 설정의 모델 선택 기능은 그대로 유지한다.
