...
- ETL 생성 wizard에서 소스, 출력 스키마, 저장 위치, 실제 저장 권한, 생성 준비 상태를 최종 확인한다.
...
- 다섯 개의 review section은 `etl-review-card`, `etl-review-card-header`, `etl-review-icon` 조합으로 구성된다.
- `권한 설정`은 담당자 자동 권한, 전체 사용자 조회, 대상별 허용 작업을 표시하고 `생성 준비 상태`는 단계별 ready/warning 결과만 표시한다.
...
- 접근성: 동일했던 `수정` 버튼 이름을 `기본 정보 수정`, `출력 스키마 수정`, `저장 위치 수정`, `권한 설정 수정`으로 구분한다. 여러 단계의 종합 결과인 `생성 준비 상태`에는 잘못된 단일 수정 경로를 만들지 않는다.

## Applied Improvements

- PageHeader 아래의 반복 안내 문구를 제거했다.
- Basic Information, Destination Settings, Permission Settings, Creation Readiness 아래의 보조 설명을 제거했다.
- 권한 표시와 생성 준비 검증을 한 카드에 섞지 않고 각각 독립 카드로 분리했다.
...
- 권한 설정 카드에 담당자·전체 사용자 조회·대상별 action이 표시되고, 생성 준비 상태에 `권한 설정`과 `저장 위치`가 별도 행으로 표시되는지 확인한다.
