...
상태 카드, 상태/실행 주기/소유자 컬럼 필터, 최근 실행 실패 Alert는 `GET /api/etl/jobs` 서버 query로 검증합니다. 명령 실행 뒤에는 API 응답의 해당 행과 facet만 즉시 갱신하고, 갱신된 행이 활성 query와 맞지 않을 때만 목록에서 제외합니다. 따라서 액션마다 table 전체를 loading 상태로 다시 조회하지 않습니다. 빠르게 필터를 연속 변경할 때는 마지막 요청만 화면에 반영합니다. 서버 검색과 pagination은 별도 API 확장 작업에서 추가합니다. 이 화면에는 임시 로컬 상태 필터를 제공하지 않습니다.
