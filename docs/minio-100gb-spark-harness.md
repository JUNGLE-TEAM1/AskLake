...
PostgreSQL timestamp columns are returned by the Node `pg` driver as JavaScript `Date` objects. The connector must serialize those cells with `toISOString()` before writing the Spark sample JSONL; `JSON.stringify(Date)` would preserve extra quote characters as part of the cell value and make Spark `Timestamp` casts fail.
