...
Snapshot Rule runtime은 transform-only Job에서 빈 Quality 단계를 별도 Spark action으로 평가하지 않는다. Job runner는 확정 schema projection과 지원되는 row-preserving transform 선두 prefix를 run 전용 Parquet materialization 경로에 먼저 쓴 뒤, 그 Parquet를 새 DataFrame으로 읽어 schema count/null, Rule, Quality, sample과 target write를 수행한다. 기본 `ASKLAKE_SPARK_STAGED_CACHE_MAX_BYTES=0`에서는 `sparkResources.cacheStorageLevel=NONE`, `materializationMode=run_scoped_parquet_staging`, `outputFrameCacheMode=staged_parquet_reuse`를 남긴다. 양수 한도 이하의 exact `materializationBytes`만 staged DataFrame을 `MEMORY_AND_DISK`로 persist하고 `outputFrameCacheMode=staged_parquet_memory_and_disk`를 남긴다. 크기 조회 실패, 한도 초과와 cache 준비 실패는 staging-only이며 raw source를 다시 읽지 않는다. 같은 Spark type의 `String`/`Long`/`Boolean` identity cast·copy·rename과 승인된 row-preserving SQL로만 된 canonical transform 선두 prefix는 staging 전에 적용하고 그 수를 `transform.preMaterializedTransformCount`로 남긴다. 직접 컬럼 복사와 `TRIM(CAST(<input> AS STRING))`으로 제한한 total·row-preserving SQL subset은 rule별 `count()` 없이 typed Column으로 컴파일하고 `transform.rowPreservingSqlExpressionCount`에 그 수를 남긴다. type-changing transform, 임의 SQL expression과 `SELECT`는 기존 validation action 및 오류 처리를 유지한다. legacy Quality rule은 전체 행·규칙별 실패·union 실패 수를 하나의 aggregate action으로 계산한다. `npm run verify:snapshot-rule-conformance`와 `npm run verify:spark-schema-contract`는 rule/필수 컬럼 수가 늘어도 action 수가 증가하지 않는지 확인한다. `npm run verify:snapshot-spark-pipeline`은 실제 JSONL 원본 물리 read가 staging-only와 staged-cache에서 각각 정확히 1회인지, staged Parquet가 생성되는지, success·quality failure·schema exception에서 staging이 정리되는지 검증한다.
...
하이브리드 실험에서는 `ASKLAKE_SPARK_STAGED_CACHE_MAX_BYTES`를 실행마다 명시하고
`materializationBytes`, `materializationSizeStatus`, `stagedCacheEligible`,
`stagedCacheDecisionReason`, `stagedCacheFallbackCount`, 실제
`cacheStorageLevel`을 함께 수집한다. Parquet byte와 executor cache 사용량은 같지
않으므로 한 번의 elapsed time으로 기본 한도를 확정하지 않는다. 동일 image와
resource shape에서 staging-only control과 staged-cache candidate를 반복하고,
원본 read 1회·행 수·정리 residue·JVM OOM/replacement를 먼저 통과한 결과에만
성능 비교를 적용한다.
