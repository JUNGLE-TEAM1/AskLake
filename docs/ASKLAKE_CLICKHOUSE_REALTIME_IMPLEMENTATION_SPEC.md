...
- PR02 repository pin은 `clickhouse/clickhouse-server:26.3.17.4@sha256:<repository-pin-redacted>`다. Kafka Connect base는 `confluentinc/cp-kafka-connect:8.2.2@sha256:<repository-pin-redacted>`, ClickHouse Sink는 v1.4.0 release asset과 SHA-256 `e146cf1205c3a15630fcdf667556da931cb3269e63b1f3f3bedf7dfd1ffc2cb0`로 고정한다. 정확한 full image digest는 `deploy/realtime-v2-provenance.json`을 canonical provenance로 사용하며 Markdown 증거에는 복사하지 않는다.
