...
`DELETE /api/etl/jobs/{jobId}`는 `delete` 권한과 idle 상태 검증을 통과한 뒤, Continuous runtime이 있으면 deterministic worker `terminate`를 먼저 수행한다. EKS Kubernetes runner는 연결된 `SparkApplication` 삭제 요청 후 실제 object 부재를 bounded polling으로 확인하며, 확인 실패 시 DB transaction을 rollback해 Job/runtime metadata를 보존한다. cleanup이 확인된 뒤에만 `producerJobId`가 해당 Job ID인 Catalog Dataset 등록과 그 Dataset의 permission/lock metadata를 같은 transaction에서 제거한다. 따라서 삭제된 Job이 만든 Dataset은 Catalog 목록과 SQL 분석의 Dataset 선택 목록에서 즉시 사라진다. 이 경로는 Kafka topic이나 Iceberg/S3/ClickHouse의 물리 결과를 삭제하지 않으며, 물리 purge는 별도 Catalog Dataset 삭제 API만 수행한다.
