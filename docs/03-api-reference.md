...
| `DELETE` | `/api/etl/jobs/{jobId}` | `delete` | Job의 운영 레코드를 제거하고 해당 Job이 게시한 Catalog Dataset은 자동 갱신 종료 상태로 유지. 물리 lake data 삭제는 포함하지 않음 | `docs/api-contract.md` |
...
`jobKind=trino_sql_materialization` Job의 `run`/`retry`/`cancelRun`은 Airflow/Spark가 아니라 Trino materializer와 durable collector를 사용한다. 매 Run은 고유 Iceberg table에 full-refresh CTAS하고 `DESCRIBE` 성공 후 안정적인 Catalog Dataset mapping을 교체한다. backend는 CTAS 요청 전에 unfinalized Run reservation을 commit한다. revision 자동 갱신은 Job row의 내부 claim ID·Run ID·source revision을 fencing 값으로 사용하며, 이 claim 또는 unfinalized Run이 있으면 수동 `run`/`retry`도 `409 CONFLICT`로 거절한다. Trino 상태가 terminal이어도 Catalog 교체와 Run finalization이 같은 transaction으로 확정될 때까지 다음 자동 Run을 제출하지 않으며, stale Run은 최신 Job/Catalog를 바꾸지 못한다. Trino 제출 수락 여부가 불명확한 경우에는 `TRINO_SUBMISSION_OUTCOME_UNKNOWN` 상태의 reservation을 보존해 자동 재제출을 막는다. 이 내부 claim 값은 public Job `refreshState` shape를 변경하지 않는다. 실패·취소는 이전 정상 mapping을 변경하지 않는다.
...
`DELETE /api/etl/jobs/{jobId}`는 `delete` 권한과 idle 상태 검증을 통과한 뒤, `producerJobId`가 해당 Job ID인 Catalog Dataset을 같은 transaction에서 삭제하지 않고 자동 갱신 종료 상태로 기록한다. Dataset은 source provenance, physical binding과 `available` 상태를 유지하며 `runtimeStatus="producer_deleted"`, `nextRefresh="예정 없음"`으로 바뀐다. 따라서 Catalog, SQL 분석, Dashboard에서는 기존 Dataset을 계속 조회·사용할 수 있지만 자동 갱신은 더 이상 예약되지 않는다. 이 경로는 Iceberg/S3/ClickHouse의 물리 데이터나 Catalog Dataset 자체를 삭제하지 않으며, Dataset 제거 또는 물리 purge는 별도 Catalog Dataset 삭제 API만 수행한다.
