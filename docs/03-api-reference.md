...
- EMR Continuous는 추가로 `ASKLAKE_EMR_SERVERLESS_CONTINUOUS_ENABLED=true`, MSK Runtime, EMR 7.1.0 이상, streaming entry point를 요구한다. application/entry point를 별도로 지정할 수 있고 빈 값은 공통 EMR 설정을 사용한다. 제출은 `STREAMING`, 시간당 실패 임계치 1~10, 실행 timeout 없음으로 고정하며 Kafka/IAM Maven package와 helper `--py-files` URI는 배포 env로 교체할 수 있다.
