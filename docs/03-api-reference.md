...
- Source/Schema/Create/Run 흐름은 live FastAPI adapter로 동작한다.
...
- frontend는 항상 Source connector, create/run/query/catalog/dashboard API를 실제 backend로 보낸다.
...
- Dashboard adapter는 FastAPI 응답을 source of truth로 사용하고 실패를 fixture로 대체하지 않는다.
- Target 저장경로 선택은 frontend가 S3를 직접 호출하지 않고 `GET /api/s3/buckets`, `GET /api/s3/prefixes` FastAPI를 통해 bucket/prefix만 조회한다. 서버는 `boto3`로 S3-compatible storage를 조회하며, 운영 배포에서는 `S3_ALLOWED_BUCKETS`를 명시해 노출 범위를 제한한다.
...
| `POST` | `/api/internal/airflow/spark-runs/{runId}/catalog` | Airflow service bearer token | 저장된 성공 Spark manifest를 manifest 포맷의 실제 output과 대조하고 Catalog에 멱등 반영. 브라우저 호출 금지 | `docs/api-contract.md` |
...
`sourceType: "SQL Result"` Job 생성에는 `sourceConfig`의 `SQL Run ID`, `Source Dataset ID`, `Reference Dataset IDs`, `Query`가 사용된다. FastAPI는 SQL Run snapshot을 source of truth로 삼아 query/context 변조를 거부하고, 실행 시점의 모든 성공 Catalog materialization을 `sqlExecution v1`로 내부 Spark manifest에 넣는다. 이 내부 계약은 브라우저 request 필드가 아니며 Preview `rows`, `rowCount`, `previewLimit`을 포함하지 않는다. 물리 segment가 없으면 실행 가능한 SQL Job으로 취급하지 않고 `503 SERVICE_UNAVAILABLE`을 반환한다.
...
Phase 3의 `publish_run_result` task는 `POST /api/internal/airflow/spark-runs/{runId}/catalog`에 `{ "jobId": "..." }`를 보낸다. backend는 저장된 `sparkResult.status=success`, manifest 포맷과 일치하는 실제 output object, Job의 `datasetId`를 검증한 뒤 같은 Run의 materialization과 lineage를 `catalog_datasets.payload`에 저장한다. 성공 response는 `status`, `runId`, `reconciledAt`, `dataset`을 반환하고 `JobRunSummary.taskStates.catalogResult`에도 같은 식별자와 결과를 보존한다.
...
| `GET` | `/api/catalog/datasets/{datasetId}/rows?limit=100&offset=0` | `query` | 성공 materialization 전체를 실제 저장 포맷별로 읽는 행 pagination | `docs/api-contract.md` |
...
| Source/Schema 연결 | `testSourceConnector` live adapter | `POST /api/etl/sources/test` |
...
| Lineage | 저장된 `LineageGraph` 또는 upstream 기반 fallback | `GET /api/catalog/datasets/{datasetId}/lineage` |
| SQL 분석 | `executeQueryPreview`, `executeQueryDraft` 호환 wrapper | `POST /api/query/runs` preview mode |
| Query AI 생성 | 선택 metadata를 포함해 FastAPI/OpenAI 호출 | `POST /api/query/ai-suggestions` |
...
| 대시보드 | FastAPI dashboard adapter | `GET /api/dashboards`, `POST /api/dashboards/query`, draft/published runtime APIs |
...
SQL 화면의 `처리 Job 생성` UI는 모달에서 만든 설정을 SQL Result 기반 `DraftPipeline`으로 변환해 기존 Job 생성 경로를 사용한다. localStorage dataset fallback은 사용하지 않으며 backend catalog persistence와 `GET /api/catalog/datasets` 응답을 source of truth로 둔다.
...
ETL graph의 source는 `SOURCE · <fileFormat|connectorType>`, 가운데 Job은 `PROCESS · SPARK`, target은 `<targetLayer> LAYER · <persistedFormat>`으로 표시한다. target engine은 Spark manifest와 Catalog에 기록된 실제 저장 포맷(`PARQUET`, `CSV`, `JSON`)을 사용한다. 화면의 상위 데이터셋 수에는 `PROCESS` node를 포함하지 않는다.
저장된 `lineageGraph`가 없는 구형 dataset은 `CatalogDataset.upstream`으로 fallback graph를 만들고, `CatalogDataset.downstream`은 별도 영향도 context로 분리할 수 있다.
