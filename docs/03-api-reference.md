...
MSK Serverless IAM endpoint(`:9098`)는 EKS Pod Identity의 AWS 자격 증명으로 OAuth bearer 토큰을 생성해 `SASL_SSL`로 연결한다. Source Preview의 임시 consumer group은 배포 IAM 계약과 동일한 `asklake-preview-*` 접두사를 사용한다.
