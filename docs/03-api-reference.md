...
- Dashboard adapter는 FastAPI 응답만 사용하며 404와 transport 실패를 사용자에게 명시한다.
...
RAG 역할 승인에서는 제목·본문 컬럼을 여러 개 선택할 수 있다. `문서 전체 임베딩`은 명시적인 PII 경고 확인 뒤 제외 컬럼을 본문으로 전환하고 식별자·메타데이터도 본문과 겹쳐 포함한다. 선택한 필드는 이름이 붙은 하나의 parent document로 합쳐지고, 긴 문서는 의미 경계를 유지하는 bounded chunk로 나뉜 뒤 전부 임베딩된다. `GET /api/catalog/datasets/{datasetId}/rag/jobs`는 색인·재색인 작업의 실제 stage, 관측 가능한 경우에만 계산한 진행률, 문서/청크/실패 수, embedding provider/model, 검증과 alias 활성화 상태를 반환한다.
