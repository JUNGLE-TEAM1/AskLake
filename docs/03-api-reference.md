...
ClickHouse V2 은퇴 전에 저장된 Catalog payload의 `physicalBindings`에는 현재 지원하지 않는 `serving/clickhouse` entry가 남을 수 있다. 목록·상세 read path는 원본 JSONB를 수정하지 않고 현재 계약에 맞는 유효한 `archive/trino` binding만 응답에 투영한다. 은퇴 binding, 형식이 잘못된 entry, list가 아닌 legacy 값은 Dataset 전체 조회를 실패시키지 않으며, `queryEngineTable`이 유효하면 기존 `archive/trino` fallback을 유지한다.
