...
Phase 4에서는 parent가 lock commit 뒤 batch child `run`, realtime child `startContinuous`를 batch → realtime 순서로 internal command path에서 요청하고, node의 `producerRunId`와 status를 응답 `activeTreeRun`에 반영한다. child command는 matching tree run ID와 fencing token이 없으면 여전히 `409 CONTINUOUS_SQL_DEPENDENCY_CONFLICT`다. 하나라도 시작되지 않으면 `409 CONTINUOUS_SQL_DEPENDENCY_UNAVAILABLE`로 parent worker 시작 전 실패하며 tree lock은 해제된다. revision 대기와 direct Kafka consumer 제거는 Phase 5 범위다.
