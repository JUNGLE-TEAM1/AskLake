...
CLICKHOUSE_STATIC_VERIFY_MAX_MEMORY_BYTES=536870912
CLICKHOUSE_STATIC_VERIFY_MAX_THREADS=2
CLICKHOUSE_STATIC_LOAD_MIN_FREE_BYTES=2147483648
CLICKHOUSE_MEMORY_LIMIT=6g
...
- `CLICKHOUSE_STATIC_VERIFY_MAX_MEMORY_BYTES`, `CLICKHOUSE_STATIC_VERIFY_MAX_THREADS`: cache miss에서 정렬 JOIN key의 exact duplicate 검사가 사용할 query memory와 thread hard limit다. 검증 완료 cache hit에서는 scan 자체를 생략한다.
- `CLICKHOUSE_STATIC_LOAD_MIN_FREE_BYTES`: 새 static table 적재 후에도 남겨야 하는 ClickHouse disk reserve다. 예상 임시 적재량을 더한 free-space가 부족하면 `CLICKHOUSE_STATIC_DISK_LOW`로 적재 전에 실패한다.
- `CLICKHOUSE_MEMORY_LIMIT`, `CLICKHOUSE_MEMORY_RESERVATION`: production Compose ClickHouse container의 host 보호용 hard/reservation memory다. backend Settings가 아니라 Compose resource 설정이다.
...
- opt-in `servingMode=clickhouse`: `clickhouseTarget: { engine: "clickhouse", database, table }`만 사용한다. `storagePath`, `icebergTarget`, `checkpointPath`를 함께 보내면 `422`다. 정적 S3/Iceberg relation은 Job 시작 때 참조 열만 Trino page로 읽어 snapshot-scoped ClickHouse local table에 고정한다. Dataset·snapshot·schema·mapping·참조 열·JOIN key identity가 같은 검증 완료 table은 `asklake_static_cache_registry`를 통해 resume와 다른 Job에서 재사용한다. Kafka Engine은 메시지를 `RawBLOB`으로 받고 Dataset의 `recordParsing/schemaColumns`에 따라 typed raw table로 변환한 뒤 JOIN output table에 연속 반영한다. 이 mode는 `PINNED_AT_START`만 지원한다.

Job 응답은 `servingMode`와 mode별 `outputTarget`을 반환한다. ClickHouse 결과 Dataset은 Catalog에 `storageFormat=clickhouse`와 `clickhouseTable`을 기록하며 Dataset row와 Dashboard widget API는 output table을 `FINAL`로 읽는다. 일반 Trino SQL mapping은 만들지 않으며, Catalog 권한 계산도 이 Dataset에 Trino registration을 요구하지 않아 `canQuery=true`와 `queryEngineRequired=false`를 유지한다. plan relation의 `cacheHint`는 서버가 Catalog 통계와 안전 한도로 계산한 실행 hint이며 client가 임의로 지정하는 입력이 아니다. active Run 응답은 generation과 `fencingTokenHash`만 포함하며 fencing token 원문은 반환하지 않는다.

`clientRequestId`가 같은 create 재시도는 request fingerprint가 같을 때 기존 Job을 반환한다. Catalog 표시 이름과 `output.datasetName`은 사용자용 이름이므로 다른 `datasetId`끼리 같아도 충돌하지 않는다. 반대로 같은 `output.datasetId`를 다른 Job이 소유하거나 이미 Catalog에 존재하면 `409`다.

ClickHouse Kafka consumer의 broker 연결·coordinator·timeout 계열 오류는 원하는 상태가 `running`인 동안 `observedState=recovering`으로 노출하고 1초 control-plane reconciliation이 같은 generation과 consumer group을 계속 확인한다. broker와 ClickHouse가 다시 poll하면 자동으로 `running`으로 돌아간다. JSON parsing, field count, schema/type 변환처럼 같은 record에서 반복되는 결정적 오류는 `failed`다. Job/Run은 PostgreSQL, Kafka offset은 Job ID 기반 고정 consumer group, raw/output identity는 `(kafka_partition,kafka_offset)` `ReplacingMergeTree`에 보존된다.

기존 Spark Kafka Continuous worker도 종료된 실패 세션 뒤 runtime heartbeat가 다시 `starting|running`으로 회복되면 새 session row를 연다. Job의 현재 `running` 상태와 과거 실패 session이 동시에 현재 상태처럼 표시되지 않으며, 새 session의 소비·적재 수는 복구 시점의 누적 counter를 baseline으로 사용한다.

ClickHouse publication commit이 남아 있는데 Catalog row만 삭제된 경우, 다음 reconciliation은 새 Kafka event나 새 revision을 요구하지 않고 같은 publication identity로 Catalog row를 복원한다. 기존 revision과 partition cursor는 증가시키거나 되감지 않는다. 정적 Dataset의 snapshot 변경은 실행 중 Run을 바꾸지 않으며, `stop` 후 새 `start` 또는 `recover`로 만들어진 새 generation이 최신 snapshot identity의 새 cache table을 선택한다.

지원 SQL, Catalog relation metadata, lifecycle, error stage와 publication 계약은 `docs/realtime-2026/contracts/continuous-sql-v1.md`를 따른다. 기능 비활성은 `409 CONTINUOUS_SQL_DISABLED`, SQL/metadata validation은 안정적인 `CONTINUOUS_SQL_*` code와 `422`, 잘못된 transition/idempotency 충돌은 `409`다. ClickHouse 시작 단계의 disk reserve 부족은 `503 CLICKHOUSE_STATIC_DISK_LOW`, 검증 registry와 immutable table 불일치는 `503 CLICKHOUSE_STATIC_CACHE_CORRUPT`, source 한도 초과는 `503 CLICKHOUSE_STATIC_LOAD_MAX_ROWS`로 실패하며 Kafka worker가 준비됐다고 표시하지 않는다. ClickHouse Dashboard time dimension은 `day|month|year`, aggregation은 `count|sum|avg|min|max`를 지원하며 source column을 내부 table alias로 한정해 output alias와 같은 이름이어도 SQL ambiguity가 생기지 않아야 한다.
