...
- Target 화면은 기본정보, 태그, 파티션, 컬럼 최소 규칙, 샘플 프리뷰, 테스트 실행, Source -> Transform -> Target 라인리지 단위로 구성된다.
...
- Test Run은 mock async validation이며 필수 metadata, JSON parsing, 사용 컬럼 0개, partition 컬럼 사용 제외 상태를 실패로 처리한다.
