...
현재 P1 API의 `Auth` 값은 계약상 TBD다. Create flow의 Permission 입력값과 `owner` 표시는 governance/identity metadata이며, 실제 접근 제어는 임시 actor header와 resource별 `permissionGrants`를 기준으로 확장 중이다. Catalog/SQL/Job/Dashboard runtime의 공통 권한 계약은 `docs/api-contract.md`의 Permission/Governance 용어를 따른다.
