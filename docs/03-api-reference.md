...
Issue #500 Phase 1은 같은 command endpoint에 `startContinuous`, `pauseContinuous`, `resumeContinuous`, `stopContinuous`를 추가한다. 이 명령은 `executionMode: "continuous"` Kafka Job에만 적용하며, Phase 1에서는 worker control intent와 runtime record만 저장한다. Continuous worker가 구현되기 전 응답의 `processingResult.controlPlaneOnly`는 `true`이고, Kafka 소비가 시작됐다는 뜻이 아니다. Continuous Job은 장기 실행 Spark query와 checkpoint로 상태를 복원하므로 `run`/`retry` Snapshot command와 섞어 사용할 수 없다. 상세 request/response와 충돌 정책은 [Kafka Continuous Ingestion Contract](kafka-continuous-ingestion-contract.md)를 따른다.
