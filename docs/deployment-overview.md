...
일반 EC2 자동 배포 GitHub Actions workflow는 후속 phase에서 추가한다. Issue #727의 승인형 AWS staging plan/apply/artifact/smoke/destroy와 read-only TTL audit workflow는 별도 검증 환경 제어면이며 이 EC2 배포를 호출하거나 대체하지 않는다.
