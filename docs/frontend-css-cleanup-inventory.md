...
| `frontend/src/styles/sql.css` | 591 | B | `부분 정리됨` | #468 2차 정리에서 `PanelHeader`, `FieldGroup`, `NativeSelect`, `Separator`, shadcn Table 기본 surface까지 적용해 2,547줄에서 591줄로 축소. responsive workspace, dark SQL editor, tree 연결선/hover positioning, autocomplete 위치, result overflow, embedded Dashboard 크기 같은 도메인 layout CSS만 유지한다. |
