...
- 기존 서울 리전 EC2 `<ec2-instance-redacted>`(`asklake-prod`, `t3.xlarge`)와 Elastic IP, 기본 VPC의 public subnet을 재사용했다. 새 EC2, NAT Gateway, VPC endpoint는 만들지 않았다.
...
- 외부 `<public-endpoint-redacted>/`는 HTTP 200, `<public-endpoint-redacted>/api/health`는 DB 연결을 포함해 `ok=true`, status 200을 반환했다.
...
| 2026-07-13 | Phase 1 S3 round-trip | `codex/aws-s3-storage-mode` / `02b479a3` | `__asklake_phase1/02b479a3/phase1-smoke.csv`, 121 bytes, data 5 rows | Raw put/list/get/checksum/delete와 Output readiness 성공 | SHA-256 `<digest-redacted>`; Raw·Output test prefix 최종 empty | 없음 |
...
| 2026-07-13 | Phase 2 EC2·IAM baseline | `codex/aws-s3-storage-mode` / `a203ebf6` | 기존 `asklake-prod`, `AskLakeEC2SSMRole`, Raw·Output 2개 bucket | 최소 권한 inline policy 적용, IAM simulation과 network guardrail 통과 | baseline Raw list는 AccessDenied; policy 적용 후 host SSM `<uuid-redacted>` 성공 | VPC endpoint는 후속 보안 강화 선택 사항 |
| 2026-07-13 | Phase 2 host·container S3 | EC2 `<ec2-instance-redacted>` | Raw 85 bytes, SHA-256 `<digest-redacted>` | host와 Docker bridge가 Role로 Raw read·Output write/delete 성공, Raw write 거부 | container SSM `<uuid-redacted>`; cleanup SSM `<uuid-redacted>`; Raw·Output `__asklake_phase2/` 최종 empty | 서버 repo dirty 상태 보존, 실제 배포는 Phase 3 이후 |
| 2026-07-13 | Phase 3 release bootstrap | `codex/aws-s3-storage-mode` / `d157792e` | 기존 dirty repo와 server secret, 새 `/opt/asklake-release` | clean checkout, release env mode 600, S3 계약과 내부 token 준비, static AWS key absent | checkout SSM `<uuid-redacted>`; env SSM `<uuid-redacted>` | SSH 기반 `deploy.sh` transport는 미검증 |
| 2026-07-13 | Phase 4 Production Compose | EC2 `<ec2-instance-redacted>` / `d157792e` | 격리 project `asklake-release`, Raw·Output AWS S3 | preflight·S3 readiness·build·up 성공, 10개 장기 container healthy·restart 0 | preflight SSM `<uuid-redacted>`; deploy SSM `<uuid-redacted>`; final audit `<uuid-redacted>` | frontend npm audit 1 moderate·1 high는 dependency backlog |
| 2026-07-13 | Phase 5 partial health | `<public-endpoint-redacted>` | public frontend와 `/api/health` | 외부 HTTP 200, API `ok=true`, DB `ok=true`, 내부·외부 health 성공 | health SSM `<uuid-redacted>`; 로컬 외부 curl도 200 | DAG import·Spark runtime·최근 error log·DB restart 지속성 미검증 |
| 2026-07-14 | Issue #707 1GB source-scan benchmark | `fix-#707-scan-reduction` / `7392bf53` | `click_events_1gb.jsonl` 972,951,366 bytes, 3,112,448 rows, 8 Spark ranges | input/output 3,112,448행, raw full scan 7→3회, Spark 101.997→70.538초(-30.8%), NetworkIn 약 6.83→2.92 GiB(-57.3%), CPU 평균 84.82%·최대 100%, memory 최대 34.92% | Run `<run-redacted>`; deploy SSM `<uuid-redacted>`; submit SSM `<uuid-redacted>`; 24 FileScanRDD lines / 8 ranges SSM `<uuid-redacted>`; local raw 1초 export `output/benchmarks/issue-707/<run-redacted>/cloudwatch-1s.json` | 3회 예산은 현재 단일 cast transform-only Snapshot 기준이며 다중 transform/quality action batching은 별도 확장 필요 |
