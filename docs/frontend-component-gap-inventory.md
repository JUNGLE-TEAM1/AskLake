...
| --- | --- | --- | --- | --- |
| 전체 | 화면 상단 masthead + 아이콘 + 설명 + actions | `부분 해결` | `PageHeader` | Jobs/ETL에 이어 #384에서 Catalog/SQL/Dashboard list 상단 헤더를 `PageHeader`로 맞춤. `PageHeader`는 route별 icon/title/description class slot을 제공하며, `/jobs`의 64px icon, 36px title, 20px description과 동일한 위치 기준을 다른 list page 폴리싱 때 적용한다. Module placeholder, ETL schedule standalone, Dashboard runtime/compact header는 route 성격이 달라 후속 판단. |
...
| 전체 | metric summary card grid | `부분 해결` | `MetricCard`, shadcn `Button` composition | 읽기 전용 수치는 `MetricCard`를 유지한다. #440에서 Jobs 현황은 클릭 가능한 현재 상태 필터이므로 shadcn `Button` composition으로 분리했고, 최근 실행 실패는 `Alert`로 이동했다. Dashboard runtime/ETL detail metric류는 상호작용 여부에 따라 후속 판단. |
| 전체 | filter/search toolbar | `부분 해결` | `FilterToolbar` | #369에서 Jobs/Dashboard list의 toolbar body/search/actions를 1차 공통화. #375에서 Catalog 검색/태그/checkbox filter와 SQL 분석 테이블 검색까지 `FilterToolbar` 계열로 확장. #420에서 Catalog sort menu는 `DropdownMenu`로 전환. #422에서 `FilterToolbarSearch`/`FilterToolbarInput`/`FilterToolbarCheckbox` 내부를 `InputGroup`/`Checkbox` primitive로 바꾸고, Dashboard sort/filter menu도 `DropdownMenu` 기준으로 전환. Catalog tag/chip 시각 상태는 후속 `Chip`/`TagList` 후보로 유지. |
| 전체 | tag/chip/status row | `부분 해결` | `Chip`, `TagList`, `StatusBadge` | #385에서 Jobs status/owner/tag, ETL target tag, Dashboard row tags/status, Dashboard list 상태 meta에 적용. #482에서 Catalog dataset status와 SQL preflight/result 실행 상태를 Jobs 기준 `StatusBadge`로 연결했다. 일부 ETL data chip은 후속 판단. |
...
| 전체 | dense settings form | `부분 해결` | `SettingsPanel`, `FormFieldGroup`, `NativeSelectField`, shadcn `Field`/`Input`/`NativeSelect`/`InputGroup`/`Textarea`/`Checkbox` | #389에서 Dashboard config shell을 1차 적용했고, #391에서 WidgetConfigPanel chart/table select, S3/DB picker toolbar, ETL source/schedule field, SQL materialize field까지 확장. #414에서 ETL rule builder/target/permission form label/select wrapper를 추가 전환. #418에서 ETL source/rule/schedule/target/permission의 대표 raw input/select와 S3/DB picker 검색 control을 shadcn primitive로 1차 교체. #419에서 SQL editor/materialize, Dashboard widget config, assistant, runtime inline widget control의 대표 raw textarea/input/checkbox/button을 primitive로 2차 교체. 기존 form wrapper는 route QA 전까지 compatibility layer로 유지한다. color picker 세부 layout은 후속. |
...
| 전체 | tree/list hybrid selector | `부분 해결` | `TreePanel`, `TreeHoverCard`, `TreeView`, `TreeRow`, Kibo UI Tree | #401/#416/#421에서 wrapper, hover card, row shell과 MUI 제거를 진행. #468에서 SQL branch/table/column row를 Shadcnblocks/Kibo line tree로 교체하고 SQL row CSS를 제거. S3/ETL/Dashboard engine과 route별 density는 유지. |
...
| DataTable stacked cell | title/subtitle, run summary처럼 두 줄 이상을 쌓는 table cell | `부분 해결` | `DataTableStackedCell`, `DataTableCellPrimary`, `DataTableCellSecondary` | #440에서 Jobs 목록의 작업명, 타깃, 실행 결과, 마지막 실행 셀에 적용. 다른 table은 역할과 interaction이 같을 때만 확대한다. |
...
| Dashboard runtime topbar | title edit + publish/draft/share/refresh actions | `부분 해결` | `RuntimeTopbar`, `ActionGroup`, shadcn `Input`/`Button` | #385에서 runtime edit toolbar는 `ActionGroup`으로 전환. #419에서 title edit는 기존 primitive 유지 기준으로 compact size를 보정. publish/share topbar와 dirty state shell은 화면 전용으로 유지. |
...
| 2026-07-09 | #419에서 SQL/Dashboard runtime/config의 대표 textarea/input/checkbox/button을 shadcn primitive로 교체하고, Dashboard tab/title edit compact 보정을 추가. color picker와 form wrapper 제거는 후속으로 유지. |
...
| 2026-07-11 | #523 후속에서 ReUI `c-timeline-10` deployment-log pattern에 맞춰 실행 단계 card border를 제거하고 제목+Badge, 메타+소요시간, 완료 시각 구조로 조정했다. 단계 timing이 없을 때는 UI가 임의 값을 만들지 않는다. |
| 2026-07-11 | #523 실행 이력 filter를 현재 Run status facet과 count 기준으로 구성하고, 실행 ID/실행 시간 column에 TanStack sorting을 적용했다. 상태 label은 `runStatusMeta`를 filter와 badge가 공유한다. |
