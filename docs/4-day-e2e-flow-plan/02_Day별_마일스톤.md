...
| 사용자 관점 결과물 | Source/Schema/Rule/Schedule/Permission/Target 값을 채운 뒤 Review에서 `파이프라인 생성`을 누르면 ETL 목록에 새 Job이 보이고 Catalog에 새 Dataset이 보인다. |
| 구현해야 하는 코드 결과물 | `draftPipeline` contract, Source/Schema/Rule/Schedule/Permission/Target step output, Review summary, `createPipelineDraft` adapter, `jobs/datasets` prepend, selected state 갱신 |
...
| 티켓 | 작업 | 맡기 좋은 사람 | 완료 기준 |
|---|---|---|---|
| A0 | `draftPipeline` 타입/fixture/step output shape 고정 | A 고정 | 모든 step이 같은 draft object를 읽고 쓴다. |
| A1 | Source Connection 상태 UI | A 또는 C | 연결 테스트 mock 후 success/error가 보인다. |
...
| 티켓 | 작업 | 맡기 좋은 사람 | 완료 기준 |
|---|---|---|---|
| A6 | Job command adapter | A 고정 | command 응답이 `job`, `run`, `dagSteps`로 정규화된다. |
| A7 | 실행 버튼 상태 | A 또는 C | 실행 중 버튼 disabled/loading이 보인다. |
| A8 | Run store | A 고정 | `runsByJobId`, `selectedRun`이 같은 Run ID를 가진다. |
| A9 | 실행 이력 UI | A 또는 C | Run row에 상태/시간/row 요약이 보인다. |
| A10 | DAG step UI | A 또는 C | 현재 step과 실패 step이 DAG에 보인다. |
...
| 티켓 | 작업 | 맡기 좋은 사람 | 완료 기준 |
|---|---|---|---|
| A11 | 중복 클릭 방지 | A 또는 C | 생성/실행 버튼을 빠르게 눌러도 요청이 1번만 처리된다. |
| A12 | 실패 rollback | A 고정 | 실패 후 입력값과 이전 Job/Run 상태가 유지된다. |
| A13 | Error Toast 문구 | A 또는 C | 422/500/timeout별 안내가 다르게 보인다. |
| A14 | step별 empty/error | B/C 지원 가능 | Source/Schema/Quality/Schedule 각 단계가 비어도 앱이 멈추지 않는다. |
