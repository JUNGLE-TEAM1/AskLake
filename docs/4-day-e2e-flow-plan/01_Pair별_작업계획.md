...
Pair A는 볼륨이 크다. 2명만 고정으로 처리하지 말고, B/C가 빨리 끝나면 아래 모듈 단위로 합류할 수 있게 쪼갠다.

| 모듈 | 범위 | 주 담당 | B/C 합류 가능성 |
|---|---|---|---|
| A0 Draft Contract | `draftPipeline`, demo fixture, step별 output shape | A 고정 | 합류 전 필수 기준 |
| A1 Source Connection | Source type 선택, 연결 정보, 연결 테스트 mock, success/error 상태 | A 또는 C | C 합류 좋음 |
| A2 Schema Inference | CSV/DB/JSON schema preview, 컬럼 타입, 선택 schema 저장 | A 또는 B | B 합류 좋음 |
| A3 Transform Recipe | rule/recipe step 추가/삭제, 변환 요약, output schema preview | A 또는 B | B 합류 가능 |
| A4 Quality Check | validation rule, pass/fail summary, quality score | A 또는 B | B 합류 좋음 |
