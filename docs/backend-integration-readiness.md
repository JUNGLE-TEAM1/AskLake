...
| SQL 분석 | Phase 2는 local/prod Compose의 Trino 482 + Iceberg baseline 위에 Trino client protocol adapter, AST physical table compiler, server-side bounded result pages, run ownership/audit/nextUri validation을 제공한다. Query AI는 SQL 초안을 생성한다. 현재 `/api/query/runs`와 `sql_runs.payload` snapshot/DuckDB Preview는 전환 전 호환 구현으로 남아 있다 | API routing migration, UI polling, real coordinator E2E |
