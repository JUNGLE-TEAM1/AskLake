...
EKS Spark runtime image는 S3A, Iceberg, PostgreSQL JDBC와 Spark SQL Kafka runtime JAR을 build stage에서 `/opt/spark/jars`에 고정한다. Docker build는 네 root JAR의 존재를 직접 검사하고 `verify:production-spark-contract`는 dependency POM과 image copy 계약이 제거되면 실패한다.
