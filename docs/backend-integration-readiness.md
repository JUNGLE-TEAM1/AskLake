...
| Preview 실행 | 자동 SQL 점검 통과 후에만 mock result 생성, preview는 최대 100 rows 제한 안내 | `POST /api/query/runs` preview mode 또는 `POST /api/query/previews` |
...
| 결과 Lake 저장 | Preview 성공 후 생성 대상 이름/설명/태그/레이어/RAG 여부를 받아 mock Catalog Dataset을 생성하고, operation 진행률/예상시간을 표시하며, mock mode에서는 localStorage fallback으로 유지 | `POST /api/catalog/derived-datasets` |
...
| 새 Lake Dataset 저장 | Preview runId/source dataset/query와 dataset metadata를 기반으로 datasets state에 prepend하고 mock mode에서는 `asklake.derivedDatasets`에서 재hydrate하며 SQL 화면 context는 유지 | `POST /api/catalog/derived-datasets` 또는 `POST /api/etl/jobs` |

SQL 실행 백엔드는 반드시 read-only guard를 둬야 합니다. 현재 frontend preflight는 데모 안전장치이며, backend 전환 시 같은 기준을 서버 validation과 query runtime에서 재검증해야 합니다. Preview 실행은 원본 SQL을 바꾸지 않고 서버 쪽에서 row limit을 적용하는 흐름으로 분리해야 합니다. SQL 결과로 만든 derived dataset은 `lineageGraph`에 source dataset lineage와 derived node/column edge를 포함해야 합니다. 현재 derived dataset 생성 진행률은 mock operation adapter가 제공하며, backend 전환 시 `operationId` 기반 polling 또는 progress endpoint로 대체합니다. Join builder와 join key recommendation은 이번 범위에서 제외합니다.
