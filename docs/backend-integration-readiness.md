...
| Rule | versioned canonical `rules[]` compiler, legacy transform/quality adapter, create/update/review 사전 검증, pass-through output schema, 구조화된 compilation issue. Schema UI의 rename/cast/default/null guard 명시 직렬화와 실제 Snapshot runtime 기반 bounded Preview 연결 | Continuous streaming compiler |
