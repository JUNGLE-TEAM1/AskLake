...
| SQL 분석 | Phase 3은 `TRINO_ENABLED=true`에서 `/api/query/runs`를 Trino submit/status/result cursor/cancel API로 routing한다. Phase 2의 AST compiler, bounded result pages, run ownership/audit/nextUri validation을 사용한다. `false`에서는 DuckDB compatibility API를 유지한다 | frontend polling/pagination migration, real coordinator E2E |
