...
- `scripts/deploy.sh smoke`는 배포된 backend container에서 Spark REST endpoint 도달성, Redpanda Kafka metadata, 위 Trino/Iceberg readiness를 함께 확인한다. 이 runtime smoke는 user Job/Catalog/Kafka fixture를 생성하지 않으며 `TRINO_ENABLED=true` profile에서만 실행한다.
