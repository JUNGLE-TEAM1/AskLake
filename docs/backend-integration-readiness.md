...
Kafka Continuous dataset의 published widget은 PostgreSQL `dataset_freshness`, `dataset_revision_commits`, `dataset_kafka_partition_cursors`, `dashboard_widget_results`를 사용한다. backend는 `dataPath`, 완료 manifest, 두 경로의 실제 `_SUCCESS`, canonical offset 범위와 batch identity를 확인한 뒤 revision을 올린다. topic·partition watermark로 과거 commit 전체를 읽지 않고 같은 stream offset의 재반영과 부분 겹침을 거절한다. legacy 범위 migration은 version marker로 한 번만 실행한다. stream과 최대 1,000행 단위 자체 manifest를 가진 quarantine replay는 별도 commit 종류로 관리한다. replay의 Catalog 반영 실패 결과는 일반 runtime refresh에서 재조정하며 Catalog 성공 후에만 runtime 카운터를 더한다. 같은 revision이면 S3를 다시 읽지 않고, 바뀌었을 때 전체 누적 기준 `count`/`sum`/`avg`만 연속 delta를 병합한다. `min`/`max`, table, snapshot, revision gap, calculation version 변경, 10,000 group 초과에서는 active data 전체 재계산으로 fallback한다. 최근 N분·슬라이딩 시간창은 이번 범위에서 지원하지 않는다. published `/dashboards/{dashboardId}`는 backend 권장값 `clamp(triggerIntervalSeconds * 500, 5000, 60000)`에 dataset ID 기반 0~10% deterministic jitter를 더해 polling하고 hidden tab에서 중지하며 실패 시 이전 결과를 유지한다.
