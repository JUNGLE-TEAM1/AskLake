...
| Job command | Kafka Snapshot Job은 `POST /api/etl/jobs/{jobId}/commands`의 run/retry로 fixed range ingest를 실행하고, non-Kafka Job은 Airflow DAG Run을 접수. Continuous Kafka Job은 execution mode/runtime control record와 start/pause/resume/stop control intent를 저장. Prod-like Compose는 `redpanda:9092` broker를 제공 | Issue #500 Spark worker 연결, durable checkpoint runtime, lag/heartbeat 상태, target append |
