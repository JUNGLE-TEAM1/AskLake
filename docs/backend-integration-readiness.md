...
| Job command | Kafka Snapshot Job은 `POST /api/etl/jobs/{jobId}/commands`의 run/retry로 fixed range ingest를 실행하고, non-Kafka Job은 Airflow DAG Run을 접수 | Issue #500 Continuous Kafka start/pause/resume/stop, durable checkpoint runtime, lag/heartbeat 상태 |
