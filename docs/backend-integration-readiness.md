...
| SQL 분석 | Phase 1은 local/prod Compose의 Trino 482 single-node coordinator, Iceberg JDBC catalog, MinIO S3 warehouse와 Catalog `queryEngineTable` mapping schema를 제공한다. Query AI는 SQL 초안을 생성한다. 현재 `sql_runs.payload` snapshot/DuckDB Preview는 전환 전 호환 구현으로 남아 있다 | backend Trino adapter, Query Run migration, 결과 retention |
