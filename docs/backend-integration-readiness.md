...
| Target 저장경로 선택 | `GET /api/s3/buckets`, `GET /api/s3/prefixes`로 S3 bucket/prefix를 서버에서 lazy 조회하고 `target.storagePath` string에 반영. Local은 MinIO, EC2 prod는 AWS S3/IAM Role을 사용하며 둘 다 서버 `S3_ALLOWED_BUCKETS` allowlist를 적용 | 서비스별 IAM 분리와 credential rotation 운영 정책 |
...
- Object storage mode: local root Compose는 `ASKLAKE_OBJECT_STORAGE_PROVIDER=minio`와 MinIO endpoint/static local credential/path-style을 사용한다. EC2 prod Compose는 `ASKLAKE_OBJECT_STORAGE_PROVIDER=aws`, `AWS_REGION`, `S3_FORCE_PATH_STYLE=false`를 사용하며 MinIO service나 static AWS key를 포함하지 않는다. backend/Node/Spark/DuckDB/Trino는 EC2 instance profile IAM Role/default credential chain을 공유한다. raw/output/Iceberg warehouse/Query Result bucket은 배포 전에 만들고 `aws-s3-readiness`가 read bucket list와 write bucket put/head/delete를 통과해야 backend/Trino가 시작된다. AWS에서는 bucket 자동 생성을 허용하지 않고 `TRINO_RESULT_STORAGE_AUTO_CREATE_BUCKET=false`를 유지한다.
