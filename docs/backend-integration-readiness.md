...
| SQL 분석 | Phase 5는 Trino run과 legacy JSONL derived dataset 경로를 분리하고, succeeded run의 compiled SQL을 Iceberg CTAS로 materialize할 statement builder를 제공한다. `TRINO_ENABLED=true` UI는 run submit/status polling/cursor page 결과/취소를 사용하며 기본 SQL 초안에 preview `LIMIT`을 넣지 않는다 | CTAS submit/status/Catalog-lineage-dashboard source API, real coordinator E2E |
