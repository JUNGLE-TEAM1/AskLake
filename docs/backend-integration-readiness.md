...
- `npm run verify:aws-staging-workflows`는 Phase 3의 manual-only OIDC plan/apply/artifact/destroy, 검토/re-plan fingerprint, apply-only 보호 Environment, actual quota/확인 문자열 gate, cleanup의 Budget/quota 독립성, private artifact 제거, conditional/checksummed JAR bundle과 fail-closed Continuous 활성화를 AWS side effect 없이 검증한다. `AWS Staging Contract Checks`가 PR에서 같은 계약과 Terraform mock을 credential 없이 실행한다. workflow의 실제 실행과 S3/MSK/EMR smoke는 Phase 4에서 별도 증거가 필요하다.
