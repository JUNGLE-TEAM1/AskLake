...
Continuous worker의 non-empty publication은 최신 정상 projection row를 최대 20개까지 `sampleRows`로 report/manifest에 보존한다. Backend는 이를 Catalog preview와 진단용 bounded metadata로 갱신하되 Dashboard runtime은 성공한 물리 materialization을 조회한다. sample이 없는 구버전 report/retry는 기존 Catalog sample을 보존하며, bounded sample은 전체 Lake 데이터나 materialization row count를 대체하지 않는다. Catalog payload의 `sourceExecutionMode: "continuous"`, `sourceKind: "kafka"`, `dashboardSyncIntervalMinutes`를 자동 대상/주기 판정에 사용하고, 명시적 provenance나 주기가 없는 legacy payload는 `sourceRunId`의 `continuous:` prefix와 5분으로 보완한다. `sourceKind: "kafka"`만으로 Kafka Snapshot을 포함하지 않는다.
