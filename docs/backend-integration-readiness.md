...
| SQL 분석 | Phase 4 SQL 화면은 `TRINO_ENABLED=true` run submit/status polling/cursor page 결과/취소 UI를 사용하며, 기본 SQL 초안에 preview `LIMIT`을 넣지 않는다. `false`에서는 DuckDB compatibility API를 유지한다 | Trino materialization/dashboard source 전환, real coordinator E2E |
