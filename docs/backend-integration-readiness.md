...
| SQL 분석 | Trino Query Run과 Iceberg CTAS materialization API가 연결되어 있으며, `TRINO_ENABLED=true` UI는 run submit/status polling/cursor page 결과/취소를 사용하고 기본 SQL 초안에 preview `LIMIT`을 넣지 않는다. private MinIO result page storage, durable collector, signed cursor, estimate/confirmation guardrail이 구현됐다. | production TLS/ACL E2E, org quota/actual plan-cost integration |
