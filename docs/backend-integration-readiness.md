...
| SQL 분석 | DuckDB compatibility snapshot과 Trino Query Run을 분리 지원. Trino mode는 canonical `/api/query/validate`, idempotent submit, durable collector, signed-cursor 결과 page, server-side CSV, Iceberg CTAS 등록과 반복 full-refresh SQL Job을 제공한다. ETL Job의 backend-owned `icebergTarget`, 일반 Spark와 Kafka Snapshot의 native Iceberg commit, 공통 snapshot/warehouse/`DESCRIBE`/`$files` 검증 adapter도 제공한다. 실행 평가와 timeline은 기존 SQL editor를 변경하지 않고 결과 panel의 `실행 정보` view에 표시한다. | Kafka Continuous Iceberg writer 연결([계획](iceberg-writer-migration-plan.md) Phase 4), old SQL Job table retention/cleanup policy, org quota 고도화 |
