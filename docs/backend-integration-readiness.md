...
- `npm run verify:storage-layout-contract`는 MinIO/AWS provider-neutral root, Node/Python 결과 일치, 명시 경로와 legacy bucket 호환, artifact 경로, Job별 checkpoint, retention 설정, production local path 차단, 안전한 object-storage 오류를 확인한다.
