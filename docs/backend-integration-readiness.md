...
| 영역 | 현재 상태 | 남은 범위 |
| --- | --- | --- |
| 수집/처리 목록 | `GET /api/etl/jobs` hydrate. `GET /api/etl/jobs/{jobId}`는 source/schema/rule/permission/target 원본 metadata까지 반환해 수정 hydrate 기반으로 사용 | 삭제, 수정 저장 API와 수정 UI |
