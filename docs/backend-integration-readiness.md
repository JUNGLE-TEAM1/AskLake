...
| Target 저장경로 선택 | `GET /api/s3/buckets`, `GET /api/s3/prefixes`로 S3 bucket/prefix를 서버에서 lazy 조회하고 `target.storagePath` string에 반영. 명시 경로는 보존하고 자동 경로는 Storage Layout V1의 output bucket/base prefix/environment/dataset/layer 계약을 사용한다. 로컬은 MinIO, EC2 prod compose는 실제 AWS S3와 instance profile IAM Role/default credential chain을 사용 | bucket lifecycle 실제 적용 자동화, 서비스별 IAM 분리와 credential rotation 고도화 |
...
| Job command | Kafka Snapshot Job은 fixed range ingest를 실행하고, non-Kafka Job은 Airflow DAG Run을 접수한다. Continuous Kafka Job은 long-running Spark worker를 제어하며, S3A checkpoint contract fingerprint, `_SUCCESS` + Rule/offset manifest, Catalog 복구, partition lag/throughput/schema/Rule report, Kafka timestamp→target commit P50/P95/P99, quarantine replay와 compaction을 제공한다. EMR opt-in은 DB admission reservation, application/workload slot·queue, actor/project quota, 자원/비용 projection을 StartJobRun 앞에 적용한다. Phase 7은 부하·장애 plan, local opt-in runner, versioned evidence/SLO와 billed resource 비용 판정 report를 제공한다. Phase 8은 격리 Shadow 결과·관측·승인·롤백을 판정하고 운영 후보 Runtime 배포를 승인된 리포트와 현재 commit에 묶는다 | pause/cancel의 실제 Airflow/Spark interrupt, async Airflow maintenance scheduling, compaction retention switch, 실제 AWS VPC/MSK/EMR 반복 부하·CloudWatch/Cost Explorer 기반 SLO 승인과 실제 운영 전환 |
...
| Auth / Admin | httpOnly `asklake_session` cookie 기반 local login/signup/session/logout, 현재 사용자 profile, admin 사용자·그룹·permission grant·governance control API 연결. Frontend는 session actor 확인 이후 보호 route와 hydrate를 시작하고, admin actor에게만 관리 콘솔과 EMR 실행 용량 탭을 노출 | 운영 IdP/SSO, production session hardening, Alembic migration |
...
Rule/target 변경의 빠른 검증은 `npm run verify:dataset-identity`, `npm run verify:rule-compiler`, `npm run verify:rule-preview`, `npm run verify:spark-schema-contract`, `npm run verify:snapshot-spark-pipeline`, `npm run verify:kafka-target-projection`, `npm run verify:target-mode-contract` 순서로 실행한다. `verify:dataset-identity`는 서로 다른 한글/slug-collision target의 Job·dataset ID 분리와 정확히 같은 target의 append 재사용을 격리 SQLite metadata DB에서 확인한다. Spark schema contract는 필수 컬럼별로 원본을 다시 읽지 않고 하나의 집계 action으로 모든 null/cast 실패 컬럼을 식별한다. JSON/JSONL runtime은 승인된 source path로 명시적 reader schema를 구성해 schema inference scan을 만들지 않고 dotted nested path를 target alias로 펼친다. Kafka Snapshot Job bridge는 확정 schema와 compiler output schema를 전달하고 direct JSONL/Catalog metadata를 동일 projection으로 생성한다. `npm run verify:kafka-review-scheduled-ingest`는 실제 Job create/command, 물리 S3 JSONL, Catalog schema까지 이 계약을 end-to-end로 검증한다.
...
- Object storage mode: local root Compose는 MinIO endpoint/static local credential/path-style을 사용한다. 저장된 Source endpoint가 host loopback이면 backend 연결에는 그대로 사용하고 Docker Spark에는 `MINIO_ENDPOINT_IN_DOCKER`를 전달하며, 외부 MinIO endpoint는 바꾸지 않는다. EC2 prod Compose는 실제 AWS S3만 사용하고 MinIO service나 static AWS key를 포함하지 않는다. Backend/Spark/DuckDB/Trino result storage가 EC2 instance profile IAM Role/default credential chain을 공유한다. 일반 ETL `spark_job_run.py`를 포함한 Spark entrypoint는 공통 provider-aware S3A builder를 사용한다. Storage Layout V1은 batch/Continuous의 data, checkpoint, manifest, quarantine, log reference를 같은 provider-neutral root에 배치하고 Job별 checkpoint 충돌을 방지한다. 명시 경로는 Node/Python이 같은 canonical percent encoding을 만들고 malformed percent/UTF-8, decoded slash, 빈 segment와 traversal을 거부한다. 기존 Continuous Job은 persisted checkpoint에서 root를 복구하며 서로 다른 output/checkpoint root는 시작하지 않는다. AWS Spark REST 실행은 MinIO credential resolution을 호출하지 않으며, MinIO REST 실행에서만 worker가 상속한 application credential 일치를 검증한다. `TRINO_ENABLED=false`에서는 Raw/Output readiness만 필요하고, `true`에서는 사전 생성한 Warehouse와 Query Result bucket의 read/write/delete readiness도 통과해야 backend와 Trino worker가 시작된다. Production frontend Target 기본값은 `ASKLAKE_SPARK_OUTPUT_BUCKET`에서 주입하며, 저장된 legacy `s3a://asklake-output/...` 경로는 실행과 Catalog 확정 시 현재 Output bucket으로 정규화하되 명시적인 custom bucket은 보존한다. Retention env는 정책 계약만 제공하며 실제 bucket lifecycle 자동 적용은 아직 하지 않는다.
...
- Issue #727 Phase 2는 apply 뒤 `terraform output -json`을 `npm run aws-staging:render-runtime`에 pipe해 위 EMR/MSK/S3 값을 stack별 private env로 생성한다. 공개 가능한 인접 manifest에는 broker endpoint 대신 개수와 SHA-256만 남고 두 파일은 mode `0600`/Git ignore로 관리한다. converter는 account/region/application/role/bucket/capacity와 Terraform sensitive wrapper를 fail-closed 검증한다. Phase 3 artifact workflow는 필수 Spark Kafka/MSK IAM JAR와 전체 bundle SHA-256을 `If-None-Match`로 올리고 S3 checksum/size/metadata를 다시 확인한 뒤에만 Continuous를 활성화한다. private env는 staging S3의 실행별 runtime prefix로 전달한다.
- Issue #727 Phase 4는 같은 runtime prefix의 checksum runner bundle을 private SSM EC2에서 실행해 S3/MSK/EMR Batch/Continuous pause-resume evidence를 만든다. Phase 5는 state/runner가 확인된 stack에 completion receipt를 남기고 보호 destroy Environment에서 Terraform teardown receipt를 남기며, TTL sweep은 state tag/expiry를 audit해 수동 destroy가 필요한 stack을 실패 신호로 남긴다. 제품 API나 기본 Runtime은 바꾸지 않는다.
...
- `npm run verify:aws-staging-contract`는 AWS side effect 없이 Issue #727 Phase 0의 서울 리전, dedicated private VPC, S3 state lock/versioning/encryption, GitHub OIDC/IAM, budget/TTL, 16 vCPU application cap과 100만 건 기능 smoke 정합성 기준을 고정한다. 이 명령의 성공만으로 AWS 준비 완료를 주장하지 않는다.
- `npm run verify:aws-staging-terraform`은 Phase 1의 별도 state bootstrap과 network/S3/IAM/MSK Serverless/EMR Serverless/CloudWatch/Budget/private SSM runner module을 정적 정책, `fmt`, `validate`, credential 없는 mock plan으로 검증한다. 실제 AWS plan/apply와 smoke는 수행하지 않는다.
- `npm run verify:aws-staging-runtime`은 Phase 2 Terraform output의 stack/account/region/application/role/bucket/capacity/sensitive wrapper를 private env와 redacted manifest로 안전하게 연결한다.
- `npm run verify:aws-staging-workflows`는 Phase 3~5의 OIDC plan/apply/artifact/smoke/destroy와 scheduled TTL audit, 검토/re-plan fingerprint, 보호 Environment, actual quota/확인 문자열 gate, evidence-before-cleanup, private artifact 제거와 SSM 경계를 AWS side effect 없이 검증한다.
- `npm run verify:aws-staging-smoke`는 Phase 4 evidence의 exact count, lag/quarantine 0, Batch row, 두 Continuous attempt/Job Run, checkpoint/S3 report와 price/resource snapshot을 fail-closed로 검증한다. 실제 workflow 실행과 AWS evidence는 GitHub/AWS 외부 설정 후 별도로 필요하다.
- `npm run verify:aws-staging-lifecycle`는 Phase 5 Terraform state tag/expiry sweep evidence와 자동 삭제 금지, smoke evidence 이후 cleanup 순서를 검증한다.
- `npm run verify:aws-staging-handoff`는 Phase 6 smoke/cleanup/TTL evidence를 같은 stack/source revision으로 묶고 Phase 7 blocker와 redaction을 유지하는지 검증한다.
