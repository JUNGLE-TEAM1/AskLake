...
- `scripts/deploy.sh smoke`는 배포된 backend container에서 Spark REST endpoint 도달성, Redpanda Kafka metadata, 위 Trino/Iceberg readiness를 함께 확인한다. Spark/Kafka probe는 bounded retry를 사용하며 이 runtime smoke는 user Job/Catalog/Kafka fixture를 생성하지 않는다. `TRINO_ENABLED=true` profile에서만 실행한다.
- `ASKLAKE_RUN_PRODUCTION_JOB_E2E=true scripts/deploy.sh job-smoke`는 일반 batch, Kafka Snapshot, Kafka Continuous의 실제 production Job 경로를 고유 fixture로 확인한다. 시작 시 Job 생성 전에 `asklake-production-smoke/<suffix>/_permission-probe` Put/Delete로 Raw fixture lifecycle 권한을 fail-fast 검증한다. 각 target의 Iceberg table을 `DESCRIBE`/`SELECT count(*)`로 대조하고 Catalog `queryEngineStatus=available`을 확인한다. Snapshot은 committed range의 후속 0건 run도 검사한다. Continuous cleanup은 `stopped`와 `failed`를 모두 terminal 상태로 취급한다. 생성한 S3 source object, Kafka topic, Job, Catalog dataset, Iceberg table만 cleanup하며 기본 배포에 자동 포함하지 않는다. AWS Raw source는 읽기 전용을 유지하되, smoke fixture lifecycle에 필요한 `asklake-production-smoke/*` prefix만 `s3:PutObject`/`s3:DeleteObject` 예외로 둔다.
