...
| SQL 분석 | DuckDB compatibility snapshot과 Trino Query Run을 분리 지원. Trino mode는 canonical `/api/query/validate`, idempotent submit, durable collector, signed-cursor 결과 page, server-side CSV, 완료된 full result의 bounded server chart aggregation, Iceberg CTAS 등록과 반복 full-refresh SQL Job을 제공한다. 차트는 현재 100행 page가 아니라 private full-result page 전체를 서버에서 집계하고 최대 500개 group만 반환한다. ETL Job의 backend-owned `icebergTarget`, 일반 Spark/Kafka Snapshot/Kafka Continuous의 native Iceberg commit과 공통 `$refs` main snapshot/warehouse/`DESCRIBE`/exact `$snapshots.summary` 검증 adapter도 제공한다. Continuous maintenance 결과도 같은 current/exact snapshot 계약으로 Trino 재검증한다. 실행 평가와 timeline은 기존 SQL editor를 변경하지 않고 결과 panel의 `실행 정보` view에 표시한다. | old SQL Job table cleanup policy, org quota와 조직별 retention policy 고도화 |
...
- Catalog 목록/상세/lineage, legacy DuckDB compatibility snapshot, SQL read-only guard, derived dataset 생성/재조회/lineage를 한 번에 확인한다. Trino 전용 lifecycle은 `tests.test_trino_query_chart`, `verify:trino-query-foundation`, `verify:query-engine-registration`, `verify:trino-result-storage`, `verify:trino-collector-resilience`, `verify:trino-submission-guard`가 분리 검증한다.
...
| 결과 차트 | 성공한 preview에서 full run을 시작하거나 재사용한다. 완료 뒤 backend가 전체 private result page를 순차 집계해 최대 500개 group만 반환하며 현재 100행 page는 차트 원본으로 사용하지 않는다. | `POST /api/query/runs/{previewRunId}/full-results`, `POST /api/query/runs/{runId}/chart` |
