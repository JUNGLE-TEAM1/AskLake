...
| SQL 분석 | Trino Query Run과 Iceberg CTAS materialization API가 연결되어 있으며, `TRINO_ENABLED=true` UI는 run submit/status polling/cursor page 결과/취소를 사용하고 기본 SQL 초안에 preview `LIMIT`을 넣지 않는다. Query Result Phase 0은 backend collector와 private MinIO result page storage로 전환하는 contract를 확정했다 | MinIO result storage migration, durable collector/recovery, signed cursor, estimate/guardrail, production TLS/ACL E2E |
