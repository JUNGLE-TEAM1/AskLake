...
- `PYTHONPATH=. .venv/bin/python -m unittest tests.test_etl_service_module_boundaries tests.test_scheduling -v`는 `etl_service.py` 7,600줄 상한, schedule·Job projection·record parsing 모듈의 역방향 façade import 금지, 기존 공개 함수 re-export identity와 schedule 동작을 검증한다. 상세 경계는 `docs/refactor-2026/contracts/etl-service-module-layout.md`를 따른다.
