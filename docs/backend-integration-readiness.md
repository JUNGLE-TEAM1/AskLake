...
- Issue #727 Phase 4는 같은 runtime prefix의 checksum runner bundle을 private SSM EC2에서 실행해 S3/MSK/EMR Batch/Continuous pause-resume evidence를 만든다. Phase 5는 success/failure evidence export 뒤 Terraform teardown receipt를 남기며, TTL sweep은 state tag/expiry를 audit해 수동 destroy가 필요한 stack을 실패 신호로 남긴다. 제품 API나 기본 Runtime은 바꾸지 않는다.
