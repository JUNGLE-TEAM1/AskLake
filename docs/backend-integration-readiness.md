...
- Issue #727 Phase 2는 apply 뒤 `terraform output -json`을 `npm run aws-staging:render-runtime`에 pipe해 위 EMR/MSK/S3 값을 stack별 private env로 생성한다. 공개 가능한 인접 manifest에는 broker endpoint 대신 개수와 SHA-256만 남고 두 파일은 mode `0600`/Git ignore로 관리한다. converter는 account/region/application/role/bucket/capacity와 Terraform sensitive wrapper를 fail-closed 검증한다. Phase 3 artifact workflow는 필수 Spark Kafka/MSK IAM JAR와 전체 bundle SHA-256을 검증해 불변 S3 prefix에 올린 뒤에만 Continuous를 활성화하고, private env는 staging S3의 실행별 runtime prefix로 전달한다.
...
- `npm run verify:aws-staging-workflows`는 Phase 3의 manual-only OIDC plan/apply/artifact/destroy, 보호 Environment, actual quota/확인 문자열 gate, private artifact 제거, checksum JAR bundle과 fail-closed Continuous 활성화를 AWS side effect 없이 검증한다. workflow의 실제 실행과 S3/MSK/EMR smoke는 Phase 4에서 별도 증거가 필요하다.
