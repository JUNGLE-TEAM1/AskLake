...
| 기능 | 현재 동작 | 필요한 백엔드 |
| --- | --- | --- |
| SQL 점검 | SQL/context 변경 시 frontend가 자동으로 empty/read-only/unknown table과 기본 구문 오류를 검사하고 inline message 표시. 문자열 리터럴 내부 mutation keyword는 무시하고 comma-separated table도 context 검증 대상에 포함 | backend SQL guard와 query validation response |
