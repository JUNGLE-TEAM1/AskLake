...
| Permission/Governance | Create flow의 `owner`, `permissionSummary`, `permissionRoles`는 metadata로 저장/표시. Job/Dataset/Dashboard 응답은 optional `createdBy`/`createdByProfile` 표시 metadata를 받을 수 있음. Catalog/SQL/Job API는 아직 grant 기반 접근 제어를 하지 않음. Dashboard 삭제만 owner/admin 임시 header 검사 | `permissionGrants`, actor context, dataset/SQL/job/dashboard 공통 permission check |
