...
| Dashboard | FastAPI dashboard card/list와 draft/published runtime API 연결. `GET /api/dashboards/{dashboardId}/published/data`가 published widget의 연결 dataset 최신 sample을 일괄 조회하며 Dashboard `view`와 Dataset `query` 권한을 함께 강제. Published 화면은 10초 self-scheduling polling, hidden-tab 중단, 중복 요청 방지, 오류 시 마지막 성공 chart 유지까지 연결. 프론트는 기존 runtime API의 404 local fallback 유지 | 공유 링크/API, export API, production-scale dashboard query/cache, cross-pair E2E QA |
...
Continuous worker의 non-empty publication은 최신 정상 projection row를 최대 20개까지 `sampleRows`로 report/manifest에 보존한다. Backend는 이를 Catalog sample로 갱신해 Published Dashboard polling이 대용량 Parquet root를 반복 다운로드하지 않게 한다. sample이 없는 구버전 report/retry는 기존 Catalog sample을 보존하며, bounded sample은 전체 Lake 데이터나 materialization row count를 대체하지 않는다.
