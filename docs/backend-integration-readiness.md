...
ClickHouse Continuous JOIN은 기존 Iceberg 경로를 대체하지 않는 dual-mode opt-in이다. `CONTINUOUS_SQL_JOIN_ENABLED=true`와 `CLICKHOUSE_CONTINUOUS_JOIN_ENABLED=true`인 `servingMode=clickhouse` Job만 Kafka Engine → raw `ReplacingMergeTree` → JOIN materialized view → output `ReplacingMergeTree` 경로를 사용한다. Kafka Engine은 `RawBLOB` 원문을 받고 저장된 공백 레코드/JSON schema 계약으로 typed raw row를 만든다. S3/Iceberg 정적 relation은 시작 시 SQL 참조 열만 exact snapshot에서 Trino page로 적재하고 `PINNED_AT_START`로 고정하며 동일 snapshot은 resume에서 재사용한다. 새 generation은 최신 snapshot identity를 다시 고정하고 이전 cache를 stale snapshot 대신 쓰지 않는다. raw input offset 수와 JOIN output 행 수를 분리해 Catalog revision을 발행하고 Dashboard는 JOIN된 output만 읽는다. pause/resume과 backend/ClickHouse restart는 고정 consumer group과 raw/output/static을 보존하며, 같은 Run 중 Spark/Iceberg 자동 fallback은 하지 않는다. broker 연결 오류는 `recovering`에서 자동 복구하고 JSON/field-count 오류는 `failed`로 유지한다. Catalog row만 삭제되면 기존 publication/revision으로 row를 복원한다. `npm run verify:clickhouse-kafka-join`은 실제 local Kafka·ClickHouse·PostgreSQL에서 Job 생성/시작, raw-text INNER JOIN, Catalog, published Dashboard 10개 widget type, pause 중 미소비와 resume 후 queued event 반영, offset duplicate 제거를 검증한다. `npm run verify:clickhouse-static-cache-refresh`는 실제 Iceberg snapshot 교체와 cache identity 분리를 검증한다.
...
Spark Kafka Continuous runtime이 일시 실패 session 이후 같은 checkpoint에서 다시 heartbeat를 보고하면 control plane은 이전 terminal session을 현재 상태로 재사용하지 않고 새 running session을 만든다. 새 session counter는 복구 직전 누적값을 baseline으로 삼아 Job 상태·현재 session 상태·소비량이 서로 모순되지 않는다.
