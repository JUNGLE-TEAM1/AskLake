...
- Issue #727 Phase 4는 같은 runtime prefix의 checksum runner bundle을 private SSM EC2에서 실행해 S3/MSK/EMR Batch/Continuous pause-resume evidence를 만든다. 제품 API나 기본 Runtime은 바꾸지 않고 `asklake.aws-staging-smoke-evidence.v1`만 GitHub/S3 증적으로 남긴다.
...
- `npm run verify:aws-staging-workflows`는 Phase 3/4의 manual-only OIDC plan/apply/artifact/smoke/destroy, 검토/re-plan fingerprint, 보호 Environment, actual quota/확인 문자열 gate, cleanup 독립성, private artifact 제거와 SSM 경계를 AWS side effect 없이 검증한다.
