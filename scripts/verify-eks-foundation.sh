...
required_files=(
  ...
  "$ROOT_DIR/infra/eks/helm/asklake-runtime-config/Chart.yaml"
  "$ROOT_DIR/infra/eks/helm/asklake-runtime-config/values.schema.json"
  "$ROOT_DIR/infra/eks/helm/asklake-runtime-config/templates/configmap.yaml"
  "$ROOT_DIR/infra/eks/values/workloads/runtime-config.test.example.yaml"
  ...
  "$ROOT_DIR/scripts/lib/verify-eks-backend-rollout-boundary.sh"
  "$ROOT_DIR/scripts/test-eks-continuous-process-boundary.sh"
  ...
  "$ROOT_DIR/scripts/prepare-eks-runtime-config-values.sh"
  "$ROOT_DIR/scripts/verify-eks-runtime-config-release.sh"
  "$ROOT_DIR/scripts/deploy-eks-runtime-config-release.sh"
  ...
  "$ROOT_DIR/backend/scripts/run_eks_phase6_bounded_e2e.py"
  "$ROOT_DIR/scripts/run-eks-day16-phase6-bounded-e2e.sh"
  "$ROOT_DIR/scripts/retain-eks-phase6-spark-evidence.sh"
  ...
  "$ROOT_DIR/scripts/test-eks-day18-operations-runbook.sh"
  "$ROOT_DIR/scripts/run-eks-day18-backend-rollout-round-trip.sh"
  "$ROOT_DIR/scripts/test-eks-day18-backend-rollout-round-trip.sh"
)
...
bash "$ROOT_DIR/scripts/test-eks-day18-operations-runbook.sh"
bash "$ROOT_DIR/scripts/test-eks-day18-backend-rollout-round-trip.sh"
...
bash -n "$ROOT_DIR/scripts/prepare-eks-runtime-config-values.sh"
bash -n "$ROOT_DIR/scripts/verify-eks-runtime-config-release.sh"
bash -n "$ROOT_DIR/scripts/deploy-eks-runtime-config-release.sh"
...
runtime_config_render="$(mktemp)"
helm template asklake-runtime-config "$ROOT_DIR/infra/eks/helm/asklake-runtime-config" \
  --namespace asklake-dev \
  -f "$ROOT_DIR/infra/eks/values/workloads/runtime-config.test.example.yaml" >"$runtime_config_render"
[[ "$(grep -c '^kind: ConfigMap$' "$runtime_config_render")" -eq 1 ]] || {
  echo "runtime ConfigMap chart must render exactly one ConfigMap" >&2
  exit 1
}
...
bash -n "$ROOT_DIR/scripts/lib/verify-eks-backend-rollout-boundary.sh"
bash -n "$ROOT_DIR/scripts/test-eks-continuous-process-boundary.sh"
...
bash "$ROOT_DIR/scripts/test-eks-continuous-process-boundary.sh"
