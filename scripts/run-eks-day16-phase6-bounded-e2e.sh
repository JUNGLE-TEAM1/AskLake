...
for command in git jq kubectl; do command -v "$command" >/dev/null 2>&1 || fail "missing required command: $command"; done
...
kubectl wait --for=condition=complete "job/$name" -n "$NAMESPACE" --timeout=160m >/dev/null || fail "Phase 6 bounded E2E Job did not complete"
