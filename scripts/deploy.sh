...
health_check() {
  ...
  for attempt in $(seq 1 "$HEALTH_RETRIES"); do
    ...
    if "$CURL_BIN" -fsSI --max-time 20 "$url" >/dev/null; then
      printf 'Checking backend: %s%s (attempt %s/%s)\n' "$url" "$HEALTH_PATH" "$attempt" "$HEALTH_RETRIES"
      if health_payload="$("$CURL_BIN" -fsS --max-time 20 "${url}${HEALTH_PATH}")"; then
        if printf '%s' "$health_payload" | health_payload_ready; then
          ...
          if ai_health_payload="$("$CURL_BIN" -fsS --max-time 20 "${url}${AI_HEALTH_PATH}")"; then
