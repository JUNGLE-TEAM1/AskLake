...
function validateDistributedTrinoEvidenceVersion(receipt, expectedDeploymentCommit, schemaVersion, expectedWorkerCount) {
  ...
  if (expectedDeploymentCommit !== null && receipt.deploymentCommit !== expectedDeploymentCommit) {
    throw new Error('deploymentCommit does not match the deployed dev commit');
  }
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) {
  ...
  if (!path || !expectedDeploymentCommit) {
    console.error('usage: ASKLAKE_TRINO_DEPLOYMENT_COMMIT=<deployed-dev-sha> verify-eks-trino-distributed-evidence.mjs [--historical-v2] <redacted-receipt.json>');
    process.exit(2);
  }
  try {
