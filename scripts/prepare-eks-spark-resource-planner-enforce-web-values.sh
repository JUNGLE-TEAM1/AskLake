...
RUNTIME_VALUES="${ASKLAKE_RUNTIME_CONFIG_VALUES:-$ROOT_DIR/infra/eks/values/workloads/dev.spark-resource-planner-enforce.runtime-config-values.json}"
OUTPUT="${ASKLAKE_WEB_VALUES:-$ROOT_DIR/infra/eks/values/workloads/dev.spark-resource-planner-enforce.web.private-values.json}"
BUILDER="$ROOT_DIR/scripts/build-eks-spark-resource-planner-enforce-web-values.mjs"
...
for values in "$BASE_WEB_VALUES" "$RUNTIME_VALUES"; do
  [[ -s "$values" ]] || fail "private base Web or enforce runtime values are missing"
...
jq -e --slurp '
  ...
  and ($candidate.backend.runtimeConfigRevision | test("^sprp-enforce-[0-9a-f]{16}$"))
  and $candidate.backend.runtimeConfigRevision !=
    ($base.backend.runtimeConfigRevision // "")
' "$BASE_WEB_VALUES" "$temporary" >/dev/null || \
  fail "enforce Web candidate changed more than backend.runtimeConfigRevision"

revision="$(jq -r '.backend.runtimeConfigRevision' "$temporary")"
mv "$temporary" "$OUTPUT"
chmod 600 "$OUTPUT"
trap - EXIT
printf 'spark_resource_planner_enforce_web_values=prepared runtime_revision=%s\n' "$revision"
