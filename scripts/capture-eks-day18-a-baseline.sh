...
pair1_sha="$(git -C "$ROOT_DIR" rev-parse origin/pair1)"
source_contains_pair1=false
git -C "$ROOT_DIR" merge-base --is-ancestor "$pair1_sha" "$source_sha" && source_contains_pair1=true
...
baseline="$(jq -n \
  ...
  --arg sourceSha "$source_sha" --arg pair1Sha "$pair1_sha" \
  --argjson sourceContainsPair1 "$source_contains_pair1" \
  ...
  --argjson externalEc2Count "$external_ec2_count" --argjson externalEc2Healthy "$external_ec2_healthy" '
  {
    ...
    source: {sha: $sourceSha, pair1Sha: $pair1Sha, containsPair1: $sourceContainsPair1},
