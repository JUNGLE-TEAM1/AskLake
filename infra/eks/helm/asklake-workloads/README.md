...
This command is a render example, not authorization to apply. Before an actual upgrade, follow `docs/eks-realtime-kafka-v1-rollout.md`: change the EC2 worker from `CONTINUOUS_WORKER_SCOPE=all` to `continuous_sql`, prove the EC2 Kafka process/lease claim is zero, write the matching PostgreSQL durable owner claim, approve one exact generation-scoped `msk_realtime_topics`/`msk_realtime_consumer_groups` pair, and verify the Backend `continuous-runtime` and Spark `checkpoints` S3 prefixes. The EKS Deployment requires `backend.enabled=true`, uses `CONTINUOUS_WORKER_SCOPE=kafka`, passes the owner/generation to the worker, and explicitly disables V2 flags/consumer ownership. Kafka Connect, ClickHouse, and Keeper are not rendered by this V1 component.
