...
export async function signup(payload: SignupRequest): Promise<AuthUserResponse> {
  if (apiConfig.useMock) {
    const user = { ...mockAdminUser, displayName: payload.displayName, email: payload.email, profile: { ...mockAdminUser.profile, displayName: payload.displayName, email: payload.email } };
    writeMockSession(user);
    return { user };
  }
