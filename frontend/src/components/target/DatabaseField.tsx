...
function DatabasePicker({
  ...
  const [databases, setDatabases] = useState<TargetDatabaseOption[]>(() => mergeCurrentDatabase([], value));
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  const [query, setQuery] = useState("");
  const [selectedName, setSelectedName] = useState(value.trim());
  ...
  const loadDatabases = async () => {
    ...
    try {
      ...
      const nextDatabases = mergeCurrentDatabase(result.databases, value);
      setDatabases(nextDatabases);
      setSelectedName((currentName) => currentName || nextDatabases[0]?.name || "");
    } catch (loadError) {
      setDatabases((currentDatabases) => mergeCurrentDatabase(currentDatabases, value));
