  ...
  return (
    <div className={cn("user-identity flex min-w-0 items-center gap-2.5 text-left", className)} {...props}>
      <Avatar size={size}>
        ...
        <AvatarFallback className="bg-slate-100 font-medium text-slate-700 ring-1 ring-slate-200">
          {getIdentityInitials(name, avatarInitials)}
        </AvatarFallback>
      </Avatar>
      <div className="grid min-w-0 gap-1">
        <span className={cn("truncate text-lg font-medium text-slate-800", nameClassName)} data-slot="user-name" title={name}>
          {name}
        </span>
        {secondary && (
          <span className={cn("truncate text-base font-normal text-slate-500", secondaryClassName)} data-slot="user-secondary" title={secondary}>
            {secondary}
          </span>
        )}
      </div>
