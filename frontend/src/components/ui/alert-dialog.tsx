...
>(({ className, ...props }, ref) => (
  <AlertDialogPortal>
    ...
    <AlertDialogPrimitive.Content
      className={cn(
        "fixed left-1/2 top-1/2 z-50 grid w-[min(calc(100vw-2rem),30rem)] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-lg border border-slate-200 bg-white p-6 text-slate-950 shadow-lg outline-none",
        className,
      )}
      ref={ref}
      {...props}
...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Description className={cn("text-sm leading-6 text-slate-500", className)} ref={ref} {...props} />
));
