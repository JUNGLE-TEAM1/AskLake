...
import { AlertTriangle, ArrowRight, BarChart3, Check, ChevronDown, Database, FileText, Filter, Loader2, Pencil, Plus, RefreshCw, Save, Search, ShieldCheck, Sparkles, Table2, Trash2, X } from "lucide-react";
...
function ModelWorkflowNav({ activeTab, onChange }: { activeTab: Tab; onChange: (tab: Tab) => void }) {
  const steps: Array<{ id: Tab; number: number; title: string }> = [
    ...
    { id: "analysis", number: 2, title: "업무 정의" },
...
function SchemaCard({ dataset, catalogDatasets }: { dataset: SemanticDataset; catalogDatasets: CatalogDataset[] }) {
  ...
  return <details className="semantic-real-schema-disclosure"><summary><span><Table2 /> 컬럼 {schema.length}개 보기</span><ChevronDown /></summary><div className="semantic-real-schema-table"><div className="header"><span>컬럼</span><span>타입</span><span>샘플</span></div>{schema.map((column) => <div key={column.name}><code>{column.name}</code><span>{column.dataType}</span><small>{column.sampleValues.slice(0, 2).join(" · ") || "샘플 없음"}</small></div>)}</div></details>;
}
...
function AnalysisDefinitionsTab({ model, catalogDatasets, onSaveMetrics, onSaveDimensions }: {
  ...
  const [definitionView, setDefinitionView] = useState<"metrics" | "dimensions">("metrics");
  useEffect(() => setDefinitionView("metrics"), [model.id]);
  return (
    <Tabs className="semantic-real-tab-content" value={definitionView} onValueChange={(value) => setDefinitionView(value as "metrics" | "dimensions")}>
      <TabsList className="semantic-real-definition-switch" aria-label="업무 정의 종류">
        <TabsTrigger value="metrics"><BarChart3 /><span><strong>계산 규칙</strong></span><em>{model.metrics.length}</em></TabsTrigger>
        <TabsTrigger value="dimensions"><Filter /><span><strong>분류 기준</strong></span><em>{model.dimensions.length}</em></TabsTrigger>
      </TabsList>
      <TabsContent className="semantic-real-definition-content" value="metrics"><MetricsSection model={model} catalogDatasets={catalogDatasets} onSave={onSaveMetrics} /></TabsContent>
      <TabsContent className="semantic-real-definition-content" value="dimensions"><DimensionsSection model={model} catalogDatasets={catalogDatasets} onSave={onSaveDimensions} /></TabsContent>
    </Tabs>
  );
}
    ...
    {!profile ? <Card size="none" className="semantic-real-rag-empty"><Sparkles /><strong>AI 검색을 아직 준비하지 않았습니다.</strong><p>AI가 컬럼을 살펴보고 검색할 문장과 식별자를 먼저 추천합니다.</p><Button type="button" disabled={busy !== null} onClick={onClassify}><Sparkles /> 검색할 내용 자동 추천</Button></Card> : <>
      ...
      <details className="semantic-real-rag-setup" open={!hasServingIndex}>
        ...
        <div className="semantic-real-rag-setup-body">
          <div className="semantic-real-card-heading"><div><h3>AI가 읽을 내용 확인</h3><p>본문, 제목, 식별자만 확인하면 됩니다. 임베딩과 색인 같은 기술 설정은 시스템이 처리합니다.</p></div><div className="semantic-real-role-heading-actions"><Button variant="outline" type="button" onClick={includeWholeDocument}><FileText /> 제외 필드 빼고 모두 포함</Button><Button variant="outline" type="button" disabled={busy !== null} onClick={onClassify}><RefreshCw /> 다시 추천</Button></div></div>
          <div className="semantic-real-embedding-guide"><strong>AI가 읽을 컬럼 {embeddedFieldCount}개</strong><span>문서 제목과 본문은 검색 대상에 자동 포함됩니다.</span>{!hasIdentifier && <span className="is-warning">원본 행을 구분할 식별자를 하나 선택해 주세요.</span>}</div>
          <div className="semantic-real-role-table"><div className="header"><span>컬럼</span><span>타입</span><span>추천 이유</span><span>검색에서의 역할</span><span>본문 포함</span></div>{schema.map((column) => {
            ...
            return <div key={column.name}><code>{column.name}</code><span>{column.dataType}</span><small>{recommendation?.reason ?? "업무 정의 또는 사용자 선택"}</small><Select value={role} onValueChange={(value) => selectRole(column.name, value)}><SelectTrigger aria-label={`${column.name} 검색 역할`} className="semantic-real-role-select" size="sm"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="body">검색할 문장</SelectItem><SelectItem value="title">문서 제목</SelectItem><SelectItem value="metadata">필터 조건</SelectItem><SelectItem value="identifier">행 식별자</SelectItem><SelectItem value="excluded">사용 안 함</SelectItem></SelectContent></Select><label className="semantic-real-embedding-toggle"><Checkbox checked={alwaysEmbedded || Boolean(embeddedColumns[column.name])} disabled={alwaysEmbedded || cannotEmbed} onCheckedChange={(checked) => setEmbeddedColumns((current) => ({ ...current, [column.name]: checked === true }))} /><span>{cannotEmbed ? "제외됨" : alwaysEmbedded ? "자동 포함" : "함께 읽기"}</span></label></div>;
          })}</div>
      ...
      <details className="semantic-real-technical-details"><summary><span><strong>작업 기록과 색인 데이터</strong><small>문제가 있을 때만 확인하는 기술 정보입니다.</small></span><ChevronDown /></summary><div><RagJobHistory datasetId={selectedDatasetId} refreshToken={jobsRefreshToken} /><DocumentPreview profile={profile} documents={previewDocuments} /></div></details>
    </>}
  </div>;
}
