    ...
    <Panel asChild>
      <aside className="sql-schema-panel">
        <section className="grid min-w-0 gap-2" aria-label="선택 테이블">
          <PanelHeader
            actions={<Badge size="sm" variant="default">{selectedDatasets.length}개 선택됨</Badge>}
            bordered={false}
            className="min-h-0 p-0"
            icon={<Table2 size={16} />}
            title="선택 테이블"
          />
          <Panel className="grid max-h-[min(520px,56vh)] min-w-0 gap-0 overflow-y-auto max-[1240px]:max-h-[min(320px,42vh)] max-[860px]:max-h-[min(320px,46vh)]">
...
function SelectedSchemaColumnList({
  ...
  return (
    <Panel className="grid min-h-[180px] content-start gap-0 overflow-y-auto p-2 max-[860px]:max-h-[52vh]" variant="muted">
