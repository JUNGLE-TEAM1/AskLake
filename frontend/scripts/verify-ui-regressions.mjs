...
const checks = [
  {
    ...
    patterns: [
      /className=\{cn\(styles\.workspace, "grid min-w-0 content-start gap-3"\)\}/,
      /className=\{cn\(styles\.queryPanel, "grid gap-4 p-5"\)\}/,
      /<SqlResultsPanel/,
      ...
      /\{showTrinoExecutionProgress && \([\s\S]*<TrinoExecutionTimeline/,
      ...
      /const applyChartConfig = \(nextConfig: SqlChartConfig\) => \{[\s\S]*if \(!visibleResult\) return;[\s\S]*\/api\/query\/runs\/\$\{visibleResult\.runId\}\/visualization/,
    ],
    forbiddenPatterns: [
      ...
      /className="sql-result-panel/,
      /\{visibleResult \? \([\s\S]*<SqlResultsPanel/,
      /statusContent=/,
      /emptyStateContent=/,
      /progressContent=/,
      /title="실행 상태 확인 중"/,
      /title="전체 결과 수집 중"/,
      /title="결과 대기 중"/,
      /const applyChartConfig = \(nextConfig: SqlChartConfig\) => \{[\s\S]*if \(!resultDraft\) return;/,
    ],
  },
  {
    name: "SQL result panel keeps the shared dev toolbar, table, and dialog layout",
    file: "src/pages/sql/SqlResultsPanel.tsx",
    patterns: [
      /className=\{`\$\{styles\.resultPanel\} grid gap-4 p-5`\}/,
      /className=\{styles\.resultToolbar\}/,
      /aria-label="차트 보기"[\s\S]*차트 보기/,
      /aria-label="데이터 미리보기"[\s\S]*데이터 미리보기/,
      /<SqlChartEmptyState \/>/,
      /className=\{styles\.resultScroll\}[\s\S]*scrollbars="both"[\s\S]*type="always"/,
      /className="min-h-0"[\s\S]*scrollbars="both"[\s\S]*type="always"/,
      ...
      /<SqlPreviewTable[\s\S]*remotePagination=\{remotePagination\}/,
      /emptyState\?: SqlResultsEmptyState;/,
      /<PanelHeader bordered=\{false\}[\s\S]*title="결과 대기 중"/,
    ],
    forbiddenPatterns: [
  ...
  {
    name: "SQL compatibility and Trino pages share the same DataTable renderer",
    file: "src/pages/sql/SqlPreviewTable.tsx",
    patterns: [
      /import \{[\s\S]*DataTable,[\s\S]*type DataTableRemotePagination,[\s\S]*\} from "@\/components\/ui\/data-table";/,
      /<DataTable/,
  ...
  {
    name: "SQL charts identify a paged Trino source and reuse Dashboard rendering",
    file: "src/pages/sql/SqlResultChart.tsx",
    patterns: [
      /function getSqlResultChartLabel\(resultDraft: SqlResultDraft\)/,
      /resultDraft\.rangeStart == null \|\| resultDraft\.rangeEnd == null/,
      /현재 결과 \$\{resultDraft\.rangeStart\.toLocaleString\(\)\}-\$\{resultDraft\.rangeEnd\.toLocaleString\(\)\}행/,
      /label: getSqlResultChartLabel\(resultDraft\)/,
      /<WidgetRenderer widget=\{widget\} \/>/,
    ],
  },
  {
    name: "SQL remote results reuse the shared DataTable pagination slot",
    file: "src/components/ui/data-table.tsx",
    patterns: [
      /export type DataTableRemotePagination = \{/,
      /remotePagination\?: DataTableRemotePagination;/,
      /\{\(paginationOptions \|\| remotePagination\) && \(/,
      /remotePagination\?\.rangeLabel/,
    ],
  },
  {
  ...
  },
  {
    ...
    patterns: [
      ...
      /accessScope,[\s\S]*permissionSummary: buildPermissionSummary\(accessScope\)/,
    ],
    forbiddenPatterns: [
      /eyebrow="처리 작업"/,
  ...
  {
    name: "SQL page derives the dataset viewport from the shared shell dimensions",
    file: "src/pages/sql/SqlAnalysisPage.module.css",
    patterns: [
      /--sql-page-header-height:\s*76px;/,
      /--sql-page-row-gap:\s*12px;/,
      /--sql-dataset-panel-height:\s*clamp\([\s\S]*var\(--layout-topbar-height\)[\s\S]*var\(--layout-footer-min-height\)[\s\S]*var\(--layout-page-padding-block-start\)[\s\S]*var\(--layout-page-padding-block-end\)[\s\S]*var\(--sql-page-header-height\)[\s\S]*var\(--sql-page-row-gap\)[\s\S]*860px[\s\S]*\);/,
      /\.datasetPanel\s*\{[\s\S]*position:\s*sticky;[\s\S]*height:\s*var\(--sql-dataset-panel-height\);[\s\S]*max-height:\s*var\(--sql-dataset-panel-height\);[\s\S]*overflow:\s*hidden;/,
      /\.workspace\s*\{[\s\S]*height:\s*auto;[\s\S]*min-height:\s*var\(--sql-workspace-min-height\);[\s\S]*max-height:\s*none;[\s\S]*grid-template-rows:\s*max-content auto;/,
      /\.resultScroll\s*\{[\s\S]*height:\s*clamp\(320px, 48dvh, 460px\);/,
      /@media \(max-width: 1180px\)[\s\S]*\.datasetPanel\s*\{[\s\S]*position:\s*static;[\s\S]*height:\s*min\(680px, 72dvh\);[\s\S]*max-height:\s*72dvh;/,
      /@media \(max-width: 1180px\)[\s\S]*\.workspace,[\s\S]*\.collapsed \.workspace\s*\{[\s\S]*grid-column:\s*1;/,
    ],
    forbiddenPatterns: [
      /--sql-workspace-height/,
      /height:\s*clamp\(820px/,
    ],
  },
  {
    name: "SQL editor and result content preserve their internal scroll viewports",
    file: "src/styles/sql.css",
    patterns: [
      /\.sql-result-body[\s\S]*min-height:\s*0;[\s\S]*overflow:\s*hidden;/,
      /\.sql-result-scroll[\s\S]*height:\s*100%;[\s\S]*min-height:\s*0;/,
  ...
  },
  {
    ...
    file: "src/styles/sql.css",
    patterns: [
      /\.sql-run-timeline\s*\{[^}]*display:\s*grid;[^}]*gap:\s*6px;[^}]*padding:\s*12px 0;/s,
    ...
    forbiddenPatterns: [
      /\.sql-run-timeline\s*\{[^}]*border-(?:top|bottom):/s,
    ],
  },
  {
  ...
  {
    name: "Trino SQL uses canonical validation and uses the shared SQL Job wizard",
    file: "src/pages/sql/SqlAnalysisPage.tsx",
    patterns: [
  ...
  },
  {
    ...
    file: "src/pages/sql/SqlAnalysisPage.tsx",
    patterns: [
      /<main className=\{cn\(styles\.workspace[\s\S]*contextCollapsed && \([\s\S]*className=\{styles\.contextRailButton\}/,
    ],
  },
  {
    ...
    patterns: [
      /\.page-body\.sql-body\s*\{[^}]*padding:\s*var\(--layout-page-padding-block-start\) var\(--layout-page-padding\) var\(--layout-page-padding-block-end\);/s,
    ],
  },
  {
