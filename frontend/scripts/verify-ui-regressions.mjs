...
const checks = [
  ...
  },
  {
    ...
    patterns: [
      ...
      /source\.metadata/,
      /ragSourceFieldSummary\(source\.sourceFields\)/,
  ...
  {
    name: "Semantic RAG job history uses the backend completion gate and honest progress contract",
    file: "src/components/semantic/RagJobHistory.tsx",
    patterns: [
      /normalize\(job\.status\) === "ready"[\s\S]*normalize\(job\.stage\) === "ready"[\s\S]*normalize\(job\.validationStatus\) === "passed"[\s\S]*normalize\(job\.activationStatus\) === "committed"[\s\S]*Boolean\(job\.completedAt\)/,
      /job\.isComplete === true/,
      /job\.progressDeterminate !== true/,
      /progress === null \? \(/,
      /rag-job-history-indeterminate/,
      /isSuccessfulJob\(job\) \|\| isFailedJob\(job\) \|\| isCancelledJob\(job\)/,
      /normalize\(job\.status\) === "failed"/,
      /normalize\(job\.status\) === "canceled"/,
      /onLatestJobSettled\(latestJob\)/,
      /lastSuccessfulJobs\.some\(\(job\) => !isTerminalJob\(job\)\)/,
      /settledDedupeRef\.current\.datasetId !== requestDatasetId/,
      /clearTimer\(\);[\s\S]*requestController\?\.abort\(\)/,
    ],
    forbiddenPatterns: [
      /단계 기준 진행률/,
  ...
  {
    name: "Semantic RAG job type mirrors determinate and nullable backend progress",
    file: "src/services/semanticApi.ts",
    patterns: [
      /isComplete: boolean;/,
      /progressDeterminate: boolean;/,
      /progressPercent: number \| null;/,
      /export async function listRagJobs/,
      /rag\/jobs\?limit=30/,
    ],
    forbiddenPatterns: [
  ...
  {
    name: "Semantic RAG sensitive embeddings require explicit confirmation for fields and whole documents",
    file: "src/pages/semantic/SemanticLayerPage.tsx",
    patterns: [
      /isSensitiveRagRole\(role\)/,
      /setPendingSensitiveEmbedding\(\{ column, role \}\)/,
      /위험을 이해하고 포함/,
      /민감정보\/PII가 VectorDB 검색 본문에 저장되고 유사도 검색 결과에 노출/,
      /includeSafeDocumentFields/,
      /requestWholeDocumentEmbedding/,
      /confirmWholeDocumentEmbedding/,
      /문서 전체 임베딩/,
      /위험을 이해하고 문서 전체 포함/,
      /role === "excluded" \? "body" : role/,
      /schema\.map\(\(column\) => \[column\.name, true\]\)/,
      /role === "body" \|\| role === "title"/,
      /const RAG_APPROVAL_COLUMN_LIMIT = 256/,
      /schema\.length > RAG_APPROVAL_COLUMN_LIMIT/,
      /if \(!profile \|\| schemaLimitExceeded\) return/,
      /전체 포함과 역할 승인을 차단했습니다/,
      /uniqueDisplayValues\(document\.sourceColumns\)/,
      /ragSourceFieldSummary\(source\.sourceFields\)/,
    ],
    forbiddenPatterns: [
      /schema\.map\(\(column\) => \[column\.name, roles\[column\.name\] !== "excluded"\]\)/,
      /slice\(0,\s*50\)/,
      /RAG_APPROVAL_COLUMN_LIMIT = 50/,
    ],
  },
  {
