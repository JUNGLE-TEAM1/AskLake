...
const checks = [
  ...
  },
  {
    ...
    patterns: [
      ...
      /\.sql-editor-surface\s*\{[\s\S]*height:\s*clamp\(276px, 36vh, 480px\);[\s\S]*min-height:\s*276px;/,
      /\.sql-editor-surface textarea\s*\{[\s\S]*height:\s*100%;[\s\S]*min-height:\s*0;[\s\S]*max-height:\s*none;[\s\S]*resize:\s*none;[\s\S]*overflow:\s*auto;/,
      /\.sql-editor-input-wrap\s*\{[\s\S]*height:\s*100%;[\s\S]*min-height:\s*0;[\s\S]*overflow:\s*hidden;/,
  ...
  {
    name: "SQL editor restores defaults only when the base dataset changes",
    file: "src/pages/sql/SqlAnalysisPage.tsx",
    patterns: [
      /const updateQuery = \(nextQuery: string\) => \{[\s\S]*setQuery\(nextQuery\);[\s\S]*resetResultState\(\);/,
      /사용자가[\s\S]*입력한 SQL이 바뀌었다는 이유로 빈 편집 내용을 다시 덮어쓰면 안 된다\./,
      /\}, \[baseDataset\?\.id\]\);/,
    ],
    forbiddenPatterns: [
  ...
  {
    name: "Catalog sample viewer loads real materialized rows with bounded paging controls",
    file: "src/pages/catalog/CatalogPage.tsx",
    patterns: [
      /getCatalogDatasetRows\(dataset\.id, \{ limit: pageSize, offset \}\)/,
      /최신 성공 버전 \$\{latestSuccessfulRun\.runId\}/,
      /aria-label="샘플 데이터 새로고침"/,
      /onClick=\{\(\) => setOffset\(0\)\}[\s\S]*처음/,
      /onClick=\{\(\) => setOffset\(lastOffset\)\}[\s\S]*마지막/,
    ],
  },
  {
    name: "Catalog sample viewer keeps column headers visible while rows scroll",
    file: "src/styles/catalog.css",
    patterns: [
      /\.catalog-sample-table th\s*\{[\s\S]*position:\s*sticky;[\s\S]*top:\s*0;/,
    ],
  },
  {
