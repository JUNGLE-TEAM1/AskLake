...
const checks = [
  ...
  {
    name: "SQL Job creation preserves wizard settings in the live pipeline draft",
    file: "src/hooks/useAskLakeData.ts",
    patterns: [
      /summary: request\.job\?\.permissionSummary/,
      /label: request\.job\?\.scheduleLabel/,
      /summary: request\.job\?\.scheduleSummary/,
      /compression: request\.job\?\.compression/,
      /partitionColumns: request\.job\?\.partitionColumn/,
      /storagePath: request\.job\?\.storagePath/,
      /description: request\.dataset\.description/,
    ],
  },
  {
  ...
  {
    name: "Catalog row pagination shows a retry state instead of a mismatched fallback page",
    file: "src/pages/catalog/CatalogPage.tsx",
    patterns: [
      /const \[rowsRetryKey, setRowsRetryKey\] = useState\(0\);/,
      /const rows = rowsResult\?\.rows \?\? \[\];/,
      /rowsError \? "불러오기 실패"/,
      /setRowsRetryKey\(\(key\) => key \+ 1\)/,
      /\) : rowsError \? \(/,
    ],
    forbiddenPatterns: [
      /rowsResult\?\.rows \?\? dataset\.sampleRows/,
      /저장된 미리보기 데이터를 표시합니다/,
    ],
  },
  {
    name: "Dashboard list routes draft cards to the draft runtime",
    file: "src/pages/dashboard/DashboardPage.tsx",
    patterns: [
  ...
  {
    name: "Dashboard lazy runtime fallback has stable height and visible progress",
    file: "src/pages/dashboard/DashboardPage.tsx",
    patterns: [
      /function DashboardRuntimeModuleLoading\(\)/,
      /className="grid min-h-\[520px\] place-items-center text-slate-600"/,
      /<Spinner className="size-5" \/>/,
      /<Suspense fallback=\{<DashboardRuntimeModuleLoading \/>\}>/,
    ],
  },
  {
    name: "S3 picker waits for the server allowlist before loading prefixes",
  ...
  {
    name: "Authentication check blocks the login route until session resolution",
    file: "src/App.tsx",
    patterns: [/if \(!authChecked\) \{\s*return <div className="workspace-route-loading"/s],
    forbiddenPatterns: [/if \(!authChecked && activeFlow !== "login"\)/],
  },
  {
    name: "Continuous Kafka controls expose pause and resume by runtime state",
    file: "src/pages/ingest/JobsPages.tsx",
    patterns: [
      /kind: "pauseContinuous", label: "일시정지"/,
      /runtimeStatus === "paused"/,
      /kind: "resumeContinuous", label: "스트림 재개"/,
      /action\.kind === "pauseContinuous"\) return <Pause/,
      /action\.kind === "resumeContinuous"\) return <Play/,
    ],
  },
  {
