...
const checks = [
  ...
  },
  {
    ...
    patterns: [
      ...
      /child\.type !== "option"/,
      /WidgetSelectModeContext = createContext<"combobox" \| "select">\("combobox"\)/,
      /selectMode === "select"[\s\S]*?<SelectTrigger[\s\S]*?<SelectContent>/,
      /const selectValue = value \|\| widgetEmptySelectValue;/,
      /!hasEmptyOption && \(!nextValue \|\| nextValue === widgetEmptySelectValue\)/,
